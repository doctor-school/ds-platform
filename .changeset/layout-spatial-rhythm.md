...
- **Semantic spacing-role tokens** over the §03 4px scale, surfaced as named Tailwind utilities via the `--spacing-<role>` `@theme` namespace: `inset` (`p-inset`), `stack` (20px mobile / 28px desktop — `space-y-stack-sm layout:space-y-stack`; the mobile 20 is a recorded owner Stage-B decision, 2026-07-06, superseding the canvas gap:0), `section` (`space-y-section`), `controls` (`gap-controls`), `inline` (`gap-inline`), `gutter` (`px-gutter` / `-mx-gutter` bleed), `day-band` (0 / bleed).
