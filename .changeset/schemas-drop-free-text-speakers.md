...
seam), the speaker projection entry loses its `source` discriminator and the
expert identity is no longer optional, and `legacySpeakerId` is off the
`event_experts` link schemas.
