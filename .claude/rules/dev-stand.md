...
- **Migration serialization across parallel branches.** The Drizzle journal (`packages/db/migrations/meta/_journal.json`) is linear — two open PRs each carrying a migration conflict non-auto-mergeably (journal + snapshot). Before `drizzle-kit generate` or adding any migration, list the in-flight migration PRs — `gh pr list --state open --json number,files --jq '.[] | select([.files[].path] | any(startswith("packages/db/"))) | .number'` — and keep ONE migration in flight repo-wide; a later branch waits, or rebases and regenerates after the first lands. With two product tracks (academy ↔ doctor showcase) on one shared backend this is the norm, not the exception.
