...
**This stand is the agent's own loop, not the owner's.** Stage-B handbacks go to a per-PR staging slot (AGENTS.md §10, `tools/staging/README.md` → «Stage-B: the owner-facing stand»); a `localhost` URL reaches the owner only when the staging box is unreachable AND the ask says so out loud.
...
- **Migration serialization across parallel branches.** The Drizzle journal (`apps/api/drizzle/meta/_journal.json`) is linear — two open PRs each carrying a migration conflict non-auto-mergeably (journal + snapshot). Before `drizzle-kit generate` or adding any migration, list the in-flight migration PRs — `gh pr list --state open --json number,files --jq '.[] | select([.files[].path] | any(startswith("packages/db/"))) | .number'` — and keep ONE migration in flight repo-wide; a later branch waits, or rebases and regenerates after the first lands. With two product tracks (academy ↔ doctor showcase) on one shared backend this is the norm, not the exception.
...
- **Playwright payloads stay out of the lead's context.** Interactive browser verification (MCP playwright) runs inside a dispatched subagent; the lead receives only the verdict + screenshot paths (return contract: `apps/docs/content/agent-discipline.md` → Dispatch, models and context). For repeatable checks prefer a scripted Playwright spec (`@playwright/test` is a dependency of `apps/portal` / `apps/showcase`) printing a compact pass/fail.
- **Live-verify pre-flight and failure evidence are lead-owned.** Run `pnpm dev:status`; follow the documented recovery when a service is down. Before declaring startup blocked, record (1) the exact failed/denied call and returned reason, (2) the local recipe path and applicable step, (3) compiled entrypoint/static/public readiness, and (4) the task process's exit/log plus PID/worktree ownership of the responding listener. A health response from a foreign PID is not task readiness. A generic policy rejection proves only the reported denial; an unknown cause stays unknown. Preserve real denials and diagnose read-only rather than changing wrappers to evade them. Transfer an action to the owner only when its actual prerequisite requires owner input.
...
- **Cross-track shared surfaces.** `packages/schemas`, `packages/db/src/schema` and the generated `packages/api-client` serve BOTH product tracks (academy ↔ doctor showcase). A wave touching them first checks the other track's open-PR touch-sets (`gh pr list --state open --json number,files,labels`) and sequences behind them instead of colliding; the migration-serialization rule in «Rules for agents» applies doubly here, because a schema change usually drags a migration with it.
