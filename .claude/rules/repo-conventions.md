...
**Title taxonomy.** Feature `[Академия][NNN] <English title>` / `[Витрина][NNN] …` (Type Feature, carries Start/Target dates); EARS `[NNN] EARS-k: …`; release gate `gate: <milestone name>`; epic `epic: …` (all Type Task). RU only in the milestone name and the track prefix. Canon: board-design spec §7.1.
