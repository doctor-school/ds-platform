...
- **Versioning & release train:** changesets (user-facing PR → `pnpm changeset`; unsure → **major**). **The agent** ships prod via `pnpm deploy:prod`, which cuts the `release-*` tag + Release at the **deployed SHA** (Option A; D+B trigger; no "operator") — detail `repo-conventions.md` + `/deploy` skill + spec §10.
...
- **No untracked seam / scaffold (F-22).** A scaffold, stub, fake, or fail-closed seam standing in for a real deliverable is decision-debt; a code comment is not a tracked obligation. **Significance threshold:** an Issue with a real-dependency done-criterion ONLY when it blocks/critical-paths a product deliverable, is user-visible or a prod risk (security/data), or must precede next release; else a `DEBT.md` line. Detail: `open-ears-issues` 3a.
- **Orchestration is the default execution mode.** Implementation/execution work dispatches to subagents by default; going inline requires **naming which sanctioned carve-out applies** — an unnamed inline edit is a visible violation. Closed carve-out list (the `#700-M1` dispatch-guard hook's WARN set): **(i)** read-only recon/scope framing; **(ii)** **≤2 consecutive lead main-tree mutations** (hook WARNs at 3); **(iii)** a skill's declared `mode:inline` step within its size cap (`do-feature-iteration` RED/GREEN/REFACTOR share the ≤2-mutation cap; else dispatch). Nothing else: an impl-heavy/to-merge session opens with a **dispatch**. Detail: memory `feedback_orchestrate_by_default_feature_completion`.
...
- **UI from the design system — adopt before bespoke.** All UI is built from `@ds/design-system`: styling **only** via tokens (arbitrary Tailwind values are lint-blocked, §5); **interactive elements and their hover/active/focus states come from its primitives**, never hand-assembled from token utilities (tokens-clean ≠ state-correct — #818; guard #828), and before writing **any** bespoke page / form / element you run the `build-ui-from-design-system` gate — inventory the package, then **search the approved registry whitelist** (official shadcn · Origin UI · Intent·Jolly · Kibo) and report the result; bespoke is a last resort, recorded in the PR. Our product code is **proprietary** (`UNLICENSED`) at any repo visibility — adopt MIT/permissive third-party freely (preserve notices); proprietary/paid registries need a license **and** a private repo, else pattern-only. **Canvas-derived UI:** vendor **every** canvas the surface renders into `design-source/` first (DesignSync) (incl. composed units) and build from the files, not issue-body prose; before Stage-B the lead runs the **eyes-on render + interaction-state parity check** (both breakpoints × both themes; canon: `build-ui-from-design-system` → Stage B — #512–514/#818). Canon: ADR-0013; memory `feedback_registry_research_before_bespoke_ui`, `feedback_import_design_source_before_building`.
- **UI design is approved before it's built — and re-confirmed live before merge.** For a `user-facing` surface the _look_ + existing-behavior changes are product decisions, not architecture calls the lead settles alone. **Notification emails/SMS are user-facing surfaces too (#869).** Stage A: before implementation present a **design proposal with research findings + 2-3 concrete options** and get explicit **product-owner choice**. Stage B: the rendered, branded result is **re-confirmed by the product owner on the LIVE stand before merge** — keep the stand **up** until that verdict; an **unanswered Stage-B question BLOCKS the merge**. **Batched carve-out:** an owner-designated **batched Stage-B gate Issue** defers live Stage-B to that gate (stands stay up); child PRs merge on Mode-a + green CI, each PR body naming `Stage-B: batched at #<gate>`; silent deferral is a violation. **Lead self-cert carve-out:** a behavioral-only user-facing change with NO new/changed visual surface may record `Stage-B: N/A (no visual surface) — lead-certified` instead of an owner GO — ONLY with explicit owner autonomous-merge auth AND a recorded lead live-verify; never to bypass a pending design question on a PR that changes the look. Canonical home of the **complete Stage-A/Stage-B procedure**: skill `build-ui-from-design-system` → Design-approval gate; memory `feedback_ui_design_product_approval`.
- **Verify UI live before "done".** Any UI-checkable feature MUST be driven in the **actual running UI** (Playwright, live dev-stand) before done — build/typecheck/lint/Mode-a are necessary, **not** sufficient (live-gated E2E absent from CI is not a substitute). Drive **every field kind × surface** (reject + accept, error language + timing); a user-facing dev placeholder is a banned stub. Detail: memory `feedback_verify_ui_on_live_stand`, `feedback_verify_every_field_kind_every_surface`.
