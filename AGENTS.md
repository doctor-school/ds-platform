...
- **UI design is approved before it's built.** For a `user-facing` surface the _look_ is a product (taste) decision, not an architecture call the lead settles itself. Before entering implementation (board → In Progress / branch) you present a **design proposal** — the registry-research shortlist (candidate blocks/screens) and/or a wireframe, brand noted — and get explicit **product-owner approval**; a rendered, branded preview screenshot is re-confirmed before merge. Two-stage gate detail: `build-ui-from-design-system`; memory `feedback_ui_design_product_approval`.
