...
- **Cross-front capability reuse before invention.** Before a frontend builds any component or product behaviour, inventory sibling frontends and shared packages and record the current owner plus canonical target. The same event feed, card, calendar, filter, query/URL codec, live-state resolution, room-entry policy, state machine, or other cross-front behaviour has **one canonical core implementation**: extract an app-local precedent into the appropriate shared package/module, then let each app add only a thin host projection for its own defaults, authorization/targeting, response envelope, route, copy and composition. Direct app-to-app imports, copied query/state logic and copy/fork UI are forbidden; divergence needs an explicit ADR-backed product or technical reason. Canon: ADR-0013 A1 + `build-ui-from-design-system`.
