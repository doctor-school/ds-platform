...
- **Versioning & release train:** changesets (user-facing PR → `pnpm changeset`; unsure → **major**). A `Version Packages` merge cuts a `release-*` tag + GitHub Release; **the agent** ships prod via `pnpm deploy:prod` (records a GitHub Deployment) — **when** it ships is the D+B trigger policy (releasable-unit Done → agent ships standing-auth classes, escalates riskier ones; no human "operator"). Detail: `repo-conventions.md` + `/deploy` skill + release-cycle spec §10.
