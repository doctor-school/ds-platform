...
- **UI from the design system — adopt before bespoke.** All UI is built from `@ds/design-system`: styling **only** via tokens (arbitrary Tailwind values are lint-blocked, §5), and before writing **any** bespoke page / form / element you run the `build-ui-from-design-system` gate — inventory the package, then **search the approved registry whitelist** (official shadcn · Origin UI · Intent·Jolly · Kibo) and report the result; bespoke is a last resort, recorded in the PR. Our product code is **proprietary** (`UNLICENSED`) at any repo visibility — adopt MIT/permissive third-party freely (preserve notices); proprietary/paid registries need a license **and** a private repo, else pattern-only. **Canvas-derived UI:** vendor the `.dc.html` into `design-source/` first (DesignSync) and build from the file, not issue-body prose (a lossy transcription); verify the render against it element-by-element before Stage-B — the #512–514 miss passed every gate while diverging from the canvas. Canon: ADR-0013; memory `feedback_registry_research_before_bespoke_ui`, `feedback_import_design_source_before_building`.
- **UI design is approved before it's built — and re-confirmed live before merge.** For a `user-facing` surface the _look_ is a product (taste) decision, not an architecture call the lead settles itself. Stage A: before implementation you present a **design proposal with the research findings + 2-3 concrete options** (registry shortlist / wireframe, brand noted) and get explicit **product-owner choice** — a subagent-authored "research-backed" standard that merely cites sources does **not** satisfy this. Stage B: the rendered, branded result is **re-confirmed by the product owner on the LIVE stand before merge** — keep the stand **up** until that verdict, and an **unanswered Stage-B approval question BLOCKS the merge** (never screenshot-only, never after-the-fact). Two-stage gate detail: `build-ui-from-design-system`; memory `feedback_ui_design_product_approval`.
