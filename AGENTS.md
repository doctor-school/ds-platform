...
<!-- Startup budget: each file ≤200 lines/25 KB, each harness total ≤30 KB including portable discipline (`pnpm lint:instruction-budget`). Codex loads AGENTS.md; Claude adds CLAUDE.md. Scoped .claude/rules references are on-demand via §0, never implicitly imported by Codex. -->

On-demand conventions and dev-stand rules: §0; task procedure: §3. Use established evidence per portable discipline.
...
Every session: identify the requested outcome and apply portable discipline’s proportionate path; repository changes use the task-kind procedure below.
...
For repository-changing work, first reply opens with the «План сессии» block (format: `apps/docs/content/agent-discipline.md` → Session plan), then: kind, track (`track:*`), active artifact (Issue #N / spec path / ADR section), skill dispatched.
...
- **UI is approved, then driven by the agent, then re-confirmed live by the owner.** `user-facing` look + behavior (notification emails/SMS too) are product decisions, not lead calls. Stage A: reuse a valid recorded approval; anything new/unresolved needs research + options and an owner choice. DRIVE any UI-checkable feature in the running UI (Playwright, local stand); build/typecheck/lint/Mode-a are necessary, not sufficient. Cover journeys + dependent states, reject + accept, error language/timing; a user-facing dev placeholder is a banned stub. Stage B: a per-PR slot (§9), never localhost, raised ONLY with a **Stage-B request** (PR comment + the same chat message, never a handoff prompt), kept until the verdict — unanswered BLOCKS the merge; a deferral ⇒ down now + «re-raise after X»; a new head ⇒ a NEW request. Batched/behavioral-self-cert routes: `build-ui-from-design-system`; markers: repo-conventions → pre-merge gates.
