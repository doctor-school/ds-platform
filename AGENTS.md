...
- **UI design is approved before it's built — and re-confirmed live before merge.** For a `user-facing` surface the _look_ + existing-behavior changes are product decisions, not architecture calls the lead settles itself. **Notification emails and SMS are user-facing surfaces too:** their copy and templates (incl. IdP-rendered message texts) pass the same Stage-A/Stage-B gates as screens — shipping a vendor's bundled default text on a user-visible channel is a silent Stage-A bypass (#869). Stage A: before implementation you present a **design proposal with the research findings + 2-3 concrete options** and get explicit **product-owner choice** — a subagent-authored source-citing "standard" does **not** satisfy this. Stage B: the rendered, branded result is **re-confirmed by the product owner on the LIVE stand before merge** — keep the stand **up** until that verdict, and an **unanswered Stage-B approval question BLOCKS the merge** (never screenshot-only). **Carve-out:** an owner-designated **batched Stage-B gate Issue** defers the owner's live Stage-B to that gate (stands stay up) — child PRs merge on Mode-a + green CI, each PR body naming `Stage-B: batched at #<gate>`; a silent deferral is still a violation. Canonical home of the **complete Stage-A/Stage-B procedure**: skill `build-ui-from-design-system` → Design-approval gate; memory `feedback_ui_design_product_approval`.
