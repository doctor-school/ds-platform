...
6. **Background dispatches are checkpointed; report only observed artifacts.** A `run_in_background` impl dispatch gets a lead-side liveness probe (worktree `git log`/`status` after a bounded interval; still-clean ≈10 min in = kill + re-dispatch with a tighter brief) — never "wait for the notification". Owner-facing status states only observed artifacts (commit / PR # / verdict); downstream steps are phrased as plan. Briefs carry the edit-first + ≤15-tool-call research-budget clauses (memory `feedback_orchestration_brief_full_lint_before_pr` 1k/1l — the #728 123K zero-output burn).
