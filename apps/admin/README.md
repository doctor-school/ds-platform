...
`e2e/` translates `007-scenarios.feature` to the admin surface via **playwright-bdd** — the full operator arc (create → publish → configure stream → open → close → archive) plus the invalid-transition, closed-provider-enum, МСК-no-drift, and non-admin-refusal branches. Like the portal e2e (#131) it is a **manual, dev-stand-gated** gate (NOT in CI): the session bootstrap `throw`s without the stand env.
