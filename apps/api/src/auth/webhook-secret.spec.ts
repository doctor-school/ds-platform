...
describe("webhookSecretMatches — #119 constant-time webhook auth", () => {
  it("fails closed when the expected secret is unset (undefined)", () => {
  ...
  it("rejects when the provided secret is undefined", () => {
    expect(webhookSecretMatches(undefined, "s3cret")).toBe(false);
  });

  it("rejects a mismatching secret of equal length", () => {
    expect(webhookSecretMatches("s3cret", "S3CRET")).toBe(false);
  });

  it("does not throw and returns false on a length mismatch (no length leak)", () => {
  ...
  it("handles multibyte/unicode secrets by byte comparison", () => {
