...
import {
  ...
  EventProjectAdminListQuerySchema,
...
// 012 EARS-6 (#1288) — the event↔project relationship admin surface (012-design
...
//
// There is deliberately NO `PATCH` route. `event_projects` carries no mutable
// attribute (012-design §2 ER: the row is exactly the pair plus its lifecycle
// envelope), unlike `project_experts` (`role`) or `project_partners`
// (`is_primary`). §5.1's "PATCH attributes with the join ETag" is a generic
// statement over the five joins and is vacuous for this one — a PATCH here
// could only accept an empty body and bump a version. The ETag / If-Match
// contract still applies in full: it rides the two lifecycle transitions.
//
...
const ROUTE_CREATE = "/v1/admin/event-projects";
const ROUTE_TRANSITION = "/v1/admin/event-projects/:id/:transition";
const SCOPE = "taxonomy.event-projects";
...
export class EventProjectsAdminController {
  ...
  /**
   * EARS-6 / EARS-15 — the relationship list. Either endpoint may scope it
   * (`eventId` / `projectId`), `status` filters explicitly, and retired
  ...
  @Authz({
    ...
    check: "fast-path",
    audit: "none",
    tests: ["EARS-6", "EARS-15", "EARS-16"],
  })
  list(
    @Query() rawQuery: Record<string, string>,
  ): Promise<EventProjectAdminList> {
    const parsed = EventProjectAdminListQuerySchema.safeParse(rawQuery);
    if (!parsed.success) throw validationFailed(parsed.error.issues, "query");
    return this.relations.list(parsed.data);
  }

  /**
   * EARS-6 — `POST /v1/admin/event-projects`. Relates one project to one event.
   * Requires a canonical UUID `Idempotency-Key`; there is no `If-Match` because
   * there is no prior version to assert. Answers 201 with the detail body, the
   * row's `ETag` and a `Location`.
   *
  ...
  async create(
    ...
    const parsed = CreateEventProjectRequestSchema.safeParse(readJsonBody(req));
  ...
  /** EARS-6 — detail by stable id, retired relationships included (§5.1). */
  @Get(":id")
  @Authz({
    access: "authenticated",
    roles: ["platform_admin"],
  ...
  /**
   * EARS-6 / §3.1 — the lifecycle-impact PREVIEW. Answers what retiring (or
  ...
  /** EARS-6 — withdraw the relationship, retaining its row and its id (§3.1). */
  @Post(":id/retire")
  @HttpCode(200)
  @Authz({
    access: "authenticated",
  ...
  /**
   * EARS-6 — put the SAME relationship back into effect. A restore is an
   * UPDATE of the retained row, never a re-insert: the id a sponsor report or
   * an audit trail already cites keeps pointing at this relationship.
   */
  @Post(":id/restore")
  @HttpCode(200)
  @Authz({
  ...
  private async confirmTransition(
    ...
    const path = `/v1/admin/event-projects/${id}/${transition}`;
    const outcome = await this.idempotency.begin({
      key,
      scope: SCOPE,
      actorId: actorSub(req),
