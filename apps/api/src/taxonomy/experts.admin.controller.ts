...
// 012 EARS-2 (#1284) — the expert admin surface (012-design §5.1). The contract
// is the one the project vertical established, unchanged: authorization is
// feature 011's dedicated MFA-verified admin session plus CSRF double-submit,
// and the route guard requires `platform_admin` BEFORE validation, idempotency,
// upload or handler (EARS-16). There is no per-mutation live IdP revalidation
// and no step-up in 012 (§5.3).
//
...
// at most one `photo` file part. Multipart WITHOUT a file is 415 — it is not a
...
/** The only file part name an expert accepts — `cover` belongs to a project. */
...
export class ExpertsAdminController {
  // Explicit @Inject tokens — see the note in `experts.service.ts`: the
  ...
  constructor(
    @Inject(ExpertsService) private readonly experts: ExpertsService,
   ...
   * case-insensitive `q` (LD-6 trigram-indexed over name and slug), explicit
  ...
  @Authz({
    ...
    check: "fast-path",
    audit: "none",
    tests: ["EARS-2", "EARS-15", "EARS-16"],
  })
  list(@Query() rawQuery: Record<string, string>): Promise<ExpertAdminList> {
  ...
  async create(
    ...
    const { detail, etag } = await this.experts.create({
      payload: parsed.data,
      photo: file,
      lease: outcome.lease,
    });
    void reply.header("etag", etag);
    void reply.header("location", `/v1/admin/experts/${detail.id}`);
    return detail;
  }
  ...
  async detail(
    ...
    const { detail, etag } = await this.experts.detail(id);
    void reply.header("etag", etag);
    return detail;
  }

  /**
   * EARS-2 — `PATCH /v1/admin/experts/:id`. Requires the target `If-Match`
  ...
  async update(
    ...
    const parsed = UpdateExpertRequestSchema.safeParse(payload ?? {});
    if (!parsed.success) {
      throw new TaxonomyError(
        "VALIDATION_FAILED",
        "invalid expert payload",
    ...
    if (file && parsed.data.mediaAction === "clear") {
      throw new TaxonomyError(
        ...
        'a photo file and mediaAction: "clear" cannot be combined',
      );
    }

    const route = "/v1/admin/experts/:id";
    const outcome = await this.idempotency.begin({
      key,
      scope: "taxonomy.experts",
      actorId: actorSub(req),
    ...
    const { detail, etag } = await this.experts.update({
      id,
      payload: parsed.data,
      photo: file,
      expectedVersion,
   ...
   * plus at most one `photo` file; multipart with NO file is 415, and a second
  ...
  private async readAuthoringRequest(
    ...
    if (!isMultipart) {
      ...
      if (contentType && !contentType.includes("application/json")) {
        throw new TaxonomyError(
          ...
          "use application/json, or multipart/form-data with one payload part and one photo file",
        );
      }
    ...
    for await (const part of req.parts()) {
      if (part.type === "file") {
        ...
        if (part.fieldname !== PHOTO_PART) {
          conflict ??= `unexpected file part "${part.fieldname}"; an expert accepts only "${PHOTO_PART}"`;
          continue;
        }
        if (file) {
          conflict ??= "more than one photo file part";
          continue;
        }
        if (body.length > MAX_IMAGE_BYTES) {
          ...
          oversize ??= `the photo exceeds the ${MAX_IMAGE_BYTES}-byte limit`;
          continue;
        }
        file = {
