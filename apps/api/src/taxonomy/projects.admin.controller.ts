...
export class ProjectsAdminController {
  ...
  /**
   * EARS-5 / §5.1 — `POST /v1/admin/projects/:id/publish`. `draft → published`,
   ...
   * WITHDRAWS from the public surface, and a publish withdraws nothing. A
   * mandatory 428 on a purely additive move would be ceremony, not a safeguard.
   */
  @Post(":id/publish")
  @HttpCode(200)
  @Authz({
    ...
    revalidate: "live",
    audit: "low-stakes",
    tests: ["EARS-1", "EARS-5", "EARS-16", "EARS-17"],
  })
  async publish(
    @Param("id") id: string,
    @Req() req: FastifyRequest,
    ...
    const route = "/v1/admin/projects/:id/publish";
    const outcome = await this.idempotency.begin({
      key,
      scope: "taxonomy.projects",
      actorId: actorSub(req),
      ...
      fingerprint: this.idempotency.fingerprint({
        method: "POST",
        path: `/v1/admin/projects/${id}/publish`,
        payload: {},
        ifMatch: rawIfMatch,
      }),
    });
    ...
    const { detail, etag } = await this.projects.publish({
      id,
      expectedVersion,
      lease: outcome.lease,
    });
