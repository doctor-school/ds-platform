...
7. **Security baseline (§5.5):** rate limiting per-user/IP/ASN, enumeration protection (idempotent responses on login/reset), SMS toll-fraud protection (global budget circuit-breaker + IP/ASN limits), RF-accessible CAPTCHA (Yandex SmartCaptcha), account-lockout policy, refresh token theft detection. **Cookie default — `__Host-` prefix per app**; cross-app SSO continuity via OIDC silent re-auth (ADR-0001 Amendment A2, supersedes A1.1).
...
**Requirement:** every REST/RPC endpoint of the backend carries classification metadata — required role(s), Cerbos vs fast-path policy check, audit requirement, test coverage. The source of truth is TS annotations on NestJS controllers + an aggregated table in `apps/api/docs/endpoint-authz-matrix.md`. CI gate `tools/lint-endpoint-authz` validates that every decorated endpoint has full metadata; missing metadata → CI fail.

**Matrix row structure:**

| Field            | Description                                                                                            |
| ---------------- | ------------------------------------------------------------------------------------------------------ |
| `endpoint`       | HTTP method + path, or RPC name                                                                        |
| `required_roles` | Minimum role(s) for access (`guest`, `doctor`, `expert`, `platform_admin`, …)                          |
| `auth_check`     | `fast-path` (JWT only) or `cerbos` (full policy eval)                                                  |
| `object_attrs`   | object-level checks (e.g. `course.author_id == actor.id` for `course.update`)                          |
| `audit`          | `none` / `low-stakes` / `high-stakes` — determines whether an entry in `auth_audit_events` is required |
| `test_coverage`  | link to e2e / unit test coverage                                                                       |
...
| Cookie security                    | Backend           | `Secure; HttpOnly; SameSite=Lax; __Host-` prefix per app; **no tokens in localStorage**. Cross-app continuity via OIDC silent re-auth (ADR-0001 Amendment A2, supersedes A1.1). Full session security profile — ADR-0001 §6 (single source of truth). |
...
| Web storage                        | HttpOnly + Secure + SameSite=Lax cookie with `__Host-` prefix. **Not localStorage.** **Every app (portal, admin, promo, docs, cms) holds its own host-only cookie**; cross-app continuity via OIDC silent re-auth (see §7.5). Full security profile — ADR-0001 §6 + Amendment A2 (single source of truth).                                                                |
...
| MFA-elevated session               | Separate claim `acr=mfa-fresh`, TTL 30 min (reduced from 1h); admin actions require fresh MFA. Forward reference: the formal step-up authentication contract (when re-MFA is required, which endpoints, TTL) — **ADR-0001 Amendment A4 (step-up authentication)** + the `step_up_required` matrix field in `2026-05-18-ds-platform-endpoint-authorization-matrix-design`. |
...
3-year retention (PRD §31). List of events that **must** be written to `auth_audit_events`:
...
After reversing Amendment A1 (DSO-63 #2), cross-app login continuity between portal, admin, promo, docs, cms is **not a shared cookie on `.doctor.school`**, but OIDC silent re-auth at the IdP.
