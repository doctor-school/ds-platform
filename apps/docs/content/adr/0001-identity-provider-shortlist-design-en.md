...
**Requirement:** every REST/RPC endpoint of the backend carries classification metadata — required role(s), the policy-check kind (engine-neutral: role-only fast-path vs `policy`, where the role alone is not sufficient), audit requirement, test coverage. The source of truth is TS annotations on NestJS controllers + an aggregated table in `apps/api/docs/endpoint-authz-matrix.md`. CI gate `tools/lint-endpoint-authz` validates that every decorated endpoint has full metadata; missing metadata → CI fail.
...
| `auth_check`     | `none` (public), `fast-path` (JWT/role only), or `policy` (role necessary but not sufficient — **engine-neutral**, two sub-modes: with `object_attrs` the ABAC predicate evaluates through `IPolicyEngine` (ADR-0002 §3.2; Cerbos by default), without it the classified handler enforces the resource-scoped domain rule in-service; full contract in the matrix design spec §3) |
...
- **Headless for credentials:** all login/register/reset/MFA/magic-link forms — on our domain (`doctor.school`, `academy.doctor.school`, mobile native), sending JSON to the IdP headless API and receiving a next-step JSON response. The user never sees the IdP UI.
...
| ------------------ | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Email + password   | All web frontends                                                          | bcrypt/argon2 (IdP-managed), policy: ≥8 chars, length only — no character-class rule, set explicitly in the IdP (HIBP check deferred, DSO-26); **enumeration protection** on login (idempotent response, see §5.5)           |
...
| ---------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rate limiting — per-user           | IdP / API gateway | Auth surface: 10 attempts / 15 min / `(email \| phone)`, then a generic throttle (the native 30-min lockout / CAPTCHA still applies separately). A **successful** login or password-reset-complete **forgives** (clears) the per-user window for that identifier, so a legitimate forgot-password → login recovery flow is never throttled mid-journey (the per-IP / per-ASN windows are NOT forgiven). |
