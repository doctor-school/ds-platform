...
Zitadel предоставляет нативный headless-first **v2 Session API** (gRPC + REST) — явное resource-oriented создание сессий и переходы по шагам. Покрывает login / register / password reset / magic-link / SMS-OTP / MFA prompts (magic-link строится кастомом поверх session API — см. ADR-0001 §8 known trade-offs).
...
**Решение:** IdP для DS Platform — **Zitadel** (81.6% vs Authentik 75.8% по 7 взвешенным дифференциаторам). Обоснование, AGPL-дисциплина и сводка отвергнутых кандидатов — см. ADR-0001 §8.
