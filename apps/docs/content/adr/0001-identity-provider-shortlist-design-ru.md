...
7. **Security baseline (§5.5):** rate limiting per-user/IP/ASN, enumeration protection (idempotent responses на login/reset), SMS toll-fraud защита (global budget circuit-breaker + IP/ASN limits), RF-доступная CAPTCHA (Yandex SmartCaptcha), account-lockout policy, refresh token theft detection. **Cookie default — `__Host-` prefix per app**; cross-app SSO continuity через OIDC silent re-auth (ADR-0001 §6).
8. **Выбор IdP** — закрыт в §9 (Zitadel; ADR-0001 §8, DSP-209).
...
- **Финальный выбор IdP** — закрыт в §9 (Zitadel; ADR-0001 §8, DSP-209).
...
**Требование:** каждый REST/RPC endpoint backend'а имеет classification metadata — required role(s), вид policy-проверки (engine-neutral: role-only fast-path vs `policy`, где роли одной недостаточно), audit requirement, тест-покрытие. Источник правды — TS-аннотации NestJS controllers + сводная таблица в `apps/api/docs/endpoint-authz-matrix.md`. CI gate `tools/lint-endpoint-authz` валидирует, что каждый decorated endpoint имеет полную metadata; missing metadata → CI fail.
...
| `auth_check`     | `none` (public), `fast-path` (JWT/role only) или `policy` (роль необходима, но недостаточна — **engine-neutral**, два sub-mode: при `object_attrs` ABAC-предикат оценивается через `IPolicyEngine` (ADR-0002 §3.2; по умолчанию Cerbos), без него classified handler применяет resource-scoped правило in-service; полный контракт — matrix design spec §3) |
...
- **Outbox в IdP** — у Zitadel нет встроенного outbox; компенсируем через retry на стороне webhook-receiver: 5xx ответ = Zitadel повторяет с exponential backoff (Zitadel actions).
- **Reconciliation cron** — раз в час backend опрашивает Zitadel admin API (выборка пользователей с фильтром `?last_modified__gte=`), сверяет с `users` mirror, чинит drift. Любой drift событие пишется в `audit_events` как `idp_sync_drift_detected`.
...
- **Headless для credentials:** все формы login/register/reset/MFA/magic-link — на нашем домене (`doctor.school`, `academy.doctor.school`, mobile native), шлют JSON в IdP headless API, получают next-step JSON. UI IdP пользователь не видит.
...
- **Zitadel Login v2 — рассмотрен и отклонён для credentials** (ADR-0001 §2): сократил бы кастомный UI-код, но форсирует redirect-хоп на auth-субдомен, что противоречит выбору headless-inline для credentials (redirect принят только для social). Зафиксировано, чтобы не пере-обсуждалось.

### 4.2. По провайдерам

Zitadel предоставляет нативный headless-first **v2 Session API** (gRPC + REST) — явное resource-oriented создание сессий и переходы по шагам. Покрывает login / register / password reset / magic-link / SMS-OTP / MFA prompts (кастомом строится только транспорт кликабельной ссылки magic-link поверх session API — одноразовый секрет нативен; см. ADR-0001 §8 known trade-offs).
...
| ------------------ | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Email + password   | Все web-фронты                                                          | bcrypt/argon2 (IdP-managed), policy: ≥8 символов, только длина — без требования символьных классов, задаётся явно в IdP (проверка HIBP отложена, DSO-26); **enumeration protection** на login (idempotent response, см. §5.5) |
...
| ---------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rate limiting — per-user           | IdP / API gateway | Auth-поверхность: 10 попыток / 15 мин / `(email \| phone)`, далее generic-throttle (нативный 30-мин lockout / CAPTCHA применяются отдельно). **Успешный** login или password-reset-complete **прощает** (очищает) per-user-окно для этого identifier, поэтому легитимный поток forgot-password → login не троттлится посреди восстановления (per-IP / per-ASN окна НЕ прощаются). |
...
| Cookie security                    | Backend           | `Secure; HttpOnly; SameSite=Lax; __Host-` prefix per app; **никаких токенов в localStorage**. Cross-app continuity — OIDC silent re-auth (ADR-0001 §6). Полный session security profile — ADR-0001 §6 (single source of truth).                                                                                                                                                   |
...
### 6.1. Реализация (Zitadel)

- VK ID, Yandex ID, Max — generic OAuth2 source через config + **PKCE обязателен** (RFC 7636).
- Telegram Login — НЕ OAuth2, требует custom Zitadel Action, валидирующий HMAC-подписанный callback от Telegram Login Widget. ~50 строк Go + webhook.
...
| Web storage                        | HttpOnly + Secure + SameSite=Lax cookie с `__Host-` prefix. **Не localStorage.** **Каждое приложение (portal, admin, promo, docs, cms) держит свою host-only cookie**; cross-app continuity — через OIDC silent re-auth (см. §7.5). Полный security profile — ADR-0001 §6 (single source of truth).                                        |
...
| `auth.session.rejected`              | user_id (или null), reason (wrong_cookie/pending_ref/fingerprint_mismatch/expired/role_without_mfa/csrf_mismatch), tier, ip, ts |
...
**Forward reference — `auth.session.rejected`** определяется feature-спекой **011** (`apps/docs/content/specs/features/011-admin-session-2fa/`, Event Model → Events; design §8a). Это **новое событие внутри существующего класса `auth.session`**, не новый класс: admin-роут отказал в запросе, причина — в `reason`. 011 также вводит metadata-дискриминатор `tier` (`tier: "admin"`), чтобы строки admin-тира оставались отделимы от строк портала врача на общих классах `auth.session.*` / `auth.login.*` — та же идиома «различение внутри класса по атрибуту», которую уже используют `method`, `reason` и `by_admin` выше. Эта строка и есть регистрация; семантика принадлежит определяющей спеке (прецедент `ai_dual_llm` из ADR-0009/ADR-0010). Владелец канонической таксономии `identity-auth-rbac-design §7.3` несёт ту же регистрацию.
...
Cross-app login continuity между portal, admin, promo, docs, cms — **не shared cookie на `.doctor.school`** (shared cookie через trust-zone boundary отвергнут per ADR-0001 §6 — same-origin XSS или subdomain takeover скомпрометировали бы admin-сессию), а **OIDC silent re-auth у IdP**.
...
| Hash в Directual              | Zitadel                               |
| ----------------------------- | ------------------------------------- |
| bcrypt                        | ✅ Native                             |
| argon2                        | ✅ Native                             |
| PBKDF2                        | ✅ Native                             |
| scrypt                        | ⚠️ Migration plugin                   |
| SHA-256 без соли / самописное | ❌ — option «magic-link reset» forced |
...
| Sanctions ужесточение → Zitadel commercial support отозван     | Self-host остаётся (OSS), но без vendor support | Зафиксировано в ADR как known risk; fork-ready strategy                           |

### 10.2. Open questions (закрываются вне DSO-25)

1. Финальный выбор IdP — закрыт в §9 (Zitadel; ADR-0001 §8, DSP-209).
