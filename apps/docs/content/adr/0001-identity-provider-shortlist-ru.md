...
**Дата:** 2026-06-02 (текущая редакция; полная история эволюции — в `git log`).
...
- **Zitadel Login v2 — рассмотрен и отклонён для credentials.** Zitadel поставляет self-hosted MIT Login v2 (Next.js поверх Session API), который сократил бы объём кастомного UI-кода, но требует redirect-хопа на auth-субдомен — что противоречит выбранным выше seamless inline-формам для _credentials_ (redirect-модель принята только для _social_). Headless inline-формы сохраняют выбранный UX, а auth-примитивы в любом случае остаются нативными. Зафиксировано, чтобы выбор не пере-обсуждался; пересмотр потребовал бы ревизии этого раздела.
...
- Email/phone enumeration protection: idempotent responses на login/reset/register с timing delta ≤50ms. Это (вместе с rate-limit выше) — **наш** backstop, а не единственная линия защиты: Zitadel неоднократно выпускал обходы login-UI enumeration, где его собственная защита «ignore unknown usernames» давала сбой — флаг соблюдался непоследовательно (CVE-2024-41952), страница «select account» (CVE-2025-57770) и endpoints password-reset + Login UI V2 (CVE-2026-23511). **Definition of Done для любого деплоя Zitadel: пин на релиз, пропатченный против всех трёх — ≥ 4.9.1 (ветка v4) или ≥ 3.4.6 (ветка v3).**
...
**Решение.** IdP для DS Platform — **Zitadel** (2026-05-25, DSP-209).
...
- Magic-link: кастомом строится только транспорт кликабельной ссылки поверх session API — сам одноразовый секрет нативен (`otp_email`); кастомная часть — это обёртка-ссылка и её доставка по email (не core-фича Zitadel, GitHub #2075). Security-review обязателен для link-формы.
...
**Операционный fallback.** Keycloak при критических проблемах с Zitadel в v1 (наиболее зрелая OSS-альтернатива).
