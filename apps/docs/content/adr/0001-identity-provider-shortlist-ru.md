...
- **Authelia.** Wrong category — это forward-auth proxy для защиты сервисов за nginx/Traefik, не full IdP. Нет self-signup, magic-link, SMS-OTP, social OAuth client, admin UI для users. Может использоваться отдельно для защиты internal tooling BBM (Plane / Grafana / GlitchTip), но не для user-facing DS Platform identity.
