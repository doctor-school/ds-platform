...
| `AuditModule`         | Global interceptor                    | Append-only audit log 3 years; 23 auth events from IdP + domain events                                                                                                                                                                                                                         |
...
- CSP headers — at the nginx level (zone-specific, see frontend-stack-design §3.2 + ADR-0001 A1.2).
...
- Web: JWT in `HttpOnly + Secure + SameSite=Lax + __Host-` cookie (ADR-0001 §7 security baseline + Amendment A1.1; admin/api direct = **Host-, SSO cookie portal↔promo = **Secure-).
...
| **Redis HA (sessions + idempotency)**                 | **v1: single-node Redis + AOF + daily RDB snapshot** (ADR-0003 §8 Amendment A2). Sessions live in the IdP (ADR-0001 Amendment A2), not in Redis. Idempotency keys — in Postgres (ADR-0003 §7.4). HA trigger (Sentinel ≥3 nodes) — >1000 active concurrent users OR >1 unplanned restart/month (ADR-0003 §8 + ADR-0012 OQ-T2) |
...
| Redis down (v1 single-node)                 | Cache-bypass (degraded p95, re-fetch from Postgres); BullMQ non-critical jobs pause, critical jobs replay from Postgres outbox (ADR-0003 §7.4); sessions — unaffected (IdP-side); alert. Recovery from AOF + daily RDB snapshot |
...
| **OQ11. v1 availability target — 99.0% or 99.5%**     | **CLOSED 2026-05-18 (DSO-59, ADR-0002 Amendment A1)** — 99.0% v1 single-AZ; v2 HA trigger via OQ-D7 ADR-0003.                                                        |
