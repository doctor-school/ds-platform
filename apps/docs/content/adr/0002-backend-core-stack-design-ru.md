...
| `AuditModule`         | Global interceptor                    | Append-only audit-лог 3 года; 23 auth-события из IdP + domain-события                                                                                                                                                                                                                      |
...
> **Forward-ref:** policy-enforcement middleware `endpointAuthzGuard` (validation, что каждый decorated endpoint имеет полную authz-metadata из matrix), CI gate `tools/lint-endpoint-authz` — см. **`2026-05-18-ds-platform-endpoint-authorization-matrix-design`**.
...
- Web: JWT в `HttpOnly + Secure + SameSite=Lax + __Host-` cookie (ADR-0001 §7 security baseline + Amendment A1.1; admin/api direct = **Host-, SSO cookie portal↔promo = **Secure-).
...
| **Redis HA (sessions + idempotency)**    | **v1: single-node Redis + AOF + daily RDB snapshot** (ADR-0003 §8 Amendment A2). Sessions хранятся в IdP (ADR-0001 Amendment A2), не в Redis. Idempotency keys — в Postgres (ADR-0003 §7.4). HA-триггер (Sentinel ≥3 узла) — >1000 active concurrent users ИЛИ >1 unplanned restart/мес (ADR-0003 §8 + ADR-0012 OQ-T2) |
...
| **Availability**                          | uptime SLO                                           | 99.0% (ADR-0002 Amendment A1)        | 99.5% (HA trigger) | 99.95%        |
...
| Redis down (v1 single-node)                 | Cache-bypass (degraded p95, re-fetch из Postgres); BullMQ non-critical jobs паузятся, критичные replay'ятся из Postgres outbox (ADR-0003 §7.4); sessions — без потерь (IdP-side); алерт. Восстановление из AOF + daily RDB snapshot |
...
| **OQ11. v1 availability target — 99.0% или 99.5%**    | **CLOSED 2026-05-18 (DSO-59, ADR-0002 Amendment A1)** — 99.0% v1 single-AZ; v2 HA trigger через OQ-D7 ADR-0003.                                                                |
...
| module-readme-required | PR трогает `src/modules/X/*.controller.ts`/`*.service.ts` → README в diff (или override label) — **manual review, не machine check** |
