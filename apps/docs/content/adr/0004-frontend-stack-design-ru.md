...
4. **Admin/CMS framework:** Refine + custom REST data provider → NestJS API + Cerbos access provider + custom Authentik/Zitadel auth strategy. Без headless CMS как замены backend.
...
9. **Promo content source:** Payload CMS v3 content-only в `apps/cms`, Postgres `cms.*` namespace в shared Postgres-instance из ADR-0003. Custom Lexical features для inline SSOT-glossary insertions. MCP server для AI-агентов. Custom Auth Strategy → Authentik/Zitadel.
...
- **Payload Auth Strategy concrete implementation** — Phase 0 implementation после выбора IdP в DSO-25 spike (Authentik vs Zitadel).
...
Полный security profile cookie (TTL, rotation, CSRF, fingerprint binding) — **ADR-0001 §6 + Amendment A2** (single source of truth). Этот документ — implementation reference только для Next.js-specific деталей (см. §3.2.1).

#### 3.2.1. Cross-app SSO via OIDC silent re-auth — frontend implementation (DSO-63 #2, ADR-0001 Amendment A2)

> **History note:** ранее (до 2026-05-18) §3.2.1 описывал shared cookie `__Secure-ds_session` на `.doctor.school` (ADR-0001 Amendment A1.1). По результатам внешней валидации архитектуры (DSO-63 #2) shared cookie признан архитектурным дефектом и отменён. Текущая модель — host-only per app + OIDC silent re-auth. См. ADR-0001 §6 + Amendment A2 для полного обоснования.
...
- **Auth provider:** custom (~50 строк) → Authentik/Zitadel JWT с two-tier validation (JWT fast-path + introspect для high-stakes, наследие ADR-0001 §6).
...
| Authentik/ADR-0001 интеграция          | High   | +3 (auth provider)     | +3               | +2                  | +3            |
...
- Маркетолог логинится через Authentik/Zitadel (как везде в DS Platform).
- JWT-token проверяется в Payload через custom strategy (~50 строк TS).
- Payload-сессия mapped на пользователя из Authentik (Payload local user mirror через webhook outbox из ADR-0002 §5).
...
Trade-off — operational complexity (-1): Payload это новый Next.js app (`apps/cms`) + Payload migrations рядом с Drizzle (в разных schema namespace) + custom Authentik Auth Strategy ~50 строк. Это плата за остальные 5 побед.
...
│   ├── auth-shared/            # Authentik JWT integration, JWKS, two-tier validation
...
- JWT auth-flow через Authentik/Zitadel/Keycloak (выбор финализируется Phase 0 spike, DSO-25).
- **Host-only `__Host-` cookie per app** (portal, admin, promo, docs, cms) — каждое приложение имеет свой scope. Полный security profile — ADR-0001 §6 + Amendment A2.
...
| OQ-F12    | **Payload native auth fallback**                                                                                                           | DSO-25 spike (Authentik vs Zitadel) показал, что headless API не поддерживает clean custom-strategy pattern для Payload v3. Consequence: Payload использует native auth (отдельный user store для маркетинг-team), SSO между cms и portal/admin **ломается** — маркетолог имеет отдельный логин/пароль. Mitigation: email-mirror на Authentik users через webhook outbox (ADR-0002 §5), 2FA enforce'ится в Payload native auth |
...
- **Payload Auth Strategy concrete implementation** — Phase 0 implementation после DSO-25 IdP-spike (Authentik vs Zitadel).
...
| Payload auth integration с Authentik сложнее планов                      | Medium                     | Phase 0 spike (~2 дня) подтверждает feasibility; fallback — Payload native auth + email-mirror на portal users                                                                                                                                                                                                                                                                                              |
