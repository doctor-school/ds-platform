...
- Auth provider (~50 строк) → Zitadel JWT
...
SSOT enforcement через custom Lexical features («Insert glossary term» button в rich-text toolbar → relation reference, не литерал) + CI-lint на canonical terms. Custom Auth Strategy → Zitadel. MCP server для AI-агентов.
...
- Payload содержит свою migration-систему рядом с Drizzle (две migration-tool в codebase, оба в monorepo). **Namespace separation `cms.*` vs `public.*` — недостаточно как security/operational boundary**, это только naming convention. Mitigation — **Postgres role-level privilege separation**: `cms_owner` имеет USAGE only на `cms.*` schema, `app_owner` only на `public.*`. Это enforce'ится на уровне Postgres roles, не conventions. См. design spec §10.2.1; ADR-0003 §1 обязывает multi-role privilege separation наряду с single Postgres instance.
- Payload integration с Zitadel требует custom Auth Strategy ~50 строк. Не trivial, но прямой pattern.
...
| OQ-F4. Migration Payload → git-based flat-file CMS | Маркетинг scope сужается до 3-5 лендингов И inline-glossary не нужен                                                                                                                                                                                                                                   |
