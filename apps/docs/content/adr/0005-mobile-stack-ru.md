...
**Наследует:** ADR-0001 (Identity/Auth: IdP = Zitadel, закрыт по §8 / DSP-209), ADR-0002 (Backend: NestJS+REST+Centrifugo+BullMQ+Timeweb storage), ADR-0003 (Postgres17+Drizzle, Cerbos RBAC §5, Redis), ADR-0004 (Frontend: Next.js 15 App Router + 4 apps + Refine + Payload v3)
...
- ADR-0001 — IdP = Zitadel (закрыт по §8, DSP-209), OIDC/OAuth2; Cerbos RBAC живёт в ADR-0003 §5; refresh token TTL=14d (mobile per §6)
