...
| DB schema               | `packages/db/src/schema/**/*.ts` (Drizzle)                                                                                                                                                  | `apps/api/drizzle/*.sql` (миграции per ADR-0008 §2.3), `packages/db/erd.svg`                            | `pnpm db:generate` (drizzle-kit; `out: ../../apps/api/drizzle`); `pnpm db:erd` (introspect → render) | `drizzle-kit check`                           |
...
**CI posture (WARN v1, ADR-0007 §2.6).** Два wiring'а: (a) severity `warn` в `eslint.config.js` для editor/dev in-line feedback, едет в `lint`-job non-blocking (`eslint .` exits 0 на warnings); (b) STEP `glossary-ids` внутри батча `guards-warn`, запускающий `pnpm lint:glossary-ids` (свой `eslint.glossary-ids.config.mjs`, включающий только это правило на `error`) как promotable pass/fail check surface. Severity живёт в exit-кодах, а не в имени job'а (#1253): `continue-on-error: true` стоит на STEP'е — никогда на job'е, иначе check-run завершится FAILURE на WARN-находке и заблокирует merge gate, — а финальный `if: always()` WARN-report step перечисляет все сработавшие guard'ы в job summary и выходит с 0, поэтому run с WARN-находками завершается SUCCESS, оставаясь видимым. Promote до BLOCK — перенести step в батч `guards-block` (он в `ci` needs-list) и убрать его строку из WARN-report, без изменения правила — когда появится настоящая glossary-consumer поверхность и откроется §2.6 sweep.
...
**Milestone vs spec.** GitHub Milestone — это **один поставляемый релиз одного трека**, с русским именем `«<Трек> R<n> — <результат>»` (например `«Академия R1 — Архив записей»`), несущий 1–4 feature-level Issues; `R<n>` нумеруется внутри трека, и два трека никогда не делят milestone. Milestone — это **не** spec-папка и **не** долгоживущая тема: milestone независим от спек. Спека, которую реализует конкретная работа, привязывается к ней **лейблом** `feature:NNN-<slug>`, slug которого и есть имя spec-папки (`apps/docs/content/specs/features/NNN-<slug>/`). Поэтому один релиз может нести Issues нескольких спек, а Issues одной спеки — разъехаться по релизам.

**Setup для каждого feature:**

1. **Назначить релизный Milestone.** Положить feature в milestone того релиза, в котором он едет, либо открыть следующий релиз его трека (имя релиза `«<Трек> R<n> — <результат>»`, не `NNN-<slug>`); всё, что дальше двух ближайших релизов, идёт в backlog-milestone трека («Академия · Позже» / «Витрина · Позже»):
...
| 3    | Подключить `docs/adr/*.md` в Fumadocs через `apps/docs/source.config.ts` mapper (Variant B — без симлинков и копий)                                                                                                                                             | ADR visible в portal на `/adr/<slug>` route              | step 2    |
| 4    | Scaffold `packages/glossary/` + generate.ts                                                                                                                                                                                                                     | пустой ids.ts                                            | —         |
| 5    | Создать seed glossary (~30 DS Platform доменных терминов: doctor, nmo_credit, accreditation, course, lesson, certificate, ledger, con, pul, au, etc.) — seed list определяется в отдельном prep-step DSO-31 на основе PRD §13-15 + bounded contexts ADR-0006 §5 | glossary populated                                       | step 4    |
| 6    | Add `tools/lint/` scripts                                                                                                                                                                                                                                       | events-lint, glossary-mdx-lint, module-readme-lint stubs | —         |
| 7    | Add CI workflow.yml                                                                                                                                                                                                                                             | green build на empty app                                 | steps 2-6 |
