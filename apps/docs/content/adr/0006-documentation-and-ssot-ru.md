...
**Наследует:** ADR-0001 (IdP = Zitadel, закрыт по §8 / DSP-209; Cerbos RBAC живёт в ADR-0003 §5), ADR-0002 (NestJS+Zod+REST+openapi-typescript), ADR-0003 (Postgres17+Drizzle+drizzle-kit), ADR-0004 (Next.js 15 + 4 apps + Refine + Payload v3), ADR-0005 (RN+Expo+WatermelonDB+GlitchTip)
...
- Как Product Lead (нетехнарь) пишет и обновляет PRD/Vision без параллельного prose-store.
...
- Низкий порог входа для Product Lead: обычные текстовые файлы, без build-шага и без поднятия dev-стенда.
- Один источник для всех авторов: Product Lead, Tech Lead и AI правят одни и те же файлы.
...
| DB схема                                     | Drizzle TS schemas в `packages/db/src/schema/`                                                                | drizzle-kit generate → SQL migrations + introspect → ERD .svg (ADR-0003 §4)                               |
| Доменные ID (immutable)                      | `apps/docs/content/product/glossary/*.md` (file-per-term)                                                     | `pnpm generate:glossary` → `packages/glossary/src/ids.ts` TS const → ESLint enforce import                |
...
| Tech specs (architectural brainstorm output) | `docs/content/specs/tech/YYYY-MM-DD-*.md`                                                                     | DSO-25..29 pattern, правится в IDE                                                                        |
...
### 3. Модель авторства: **Markdown в Git, правки в IDE и через PR**

Отдельного редакторского приложения нет. Каждый документ — файл `.md` / `.mdx` / `.yaml` в `apps/docs/content/`, редактируется там же, где остальной репозиторий, и приезжает через pull request:

- Tech Lead и AI-агенты пишут в IDE; правки уходят обычными коммитами и ревьюятся в PR.
- Product Lead правит prose-страницы (PRD, vision, business-rules, glossary) в тех же файлах — локально либо через веб-редактор GitHub — и правка проходит тот же PR-flow.
- Fumadocs рендерит результат на `docs.doctor.school`: Git — поверхность записи, отрендеренный портал — поверхность чтения.

**Почему Git как поверхность авторства:**

- Контент = чистый Markdown / MDX / YAML / JSON — AI читает и пишет напрямую, без proprietary deserialization.
- Один review-gate для prose и кода: правки доков проходят тот же PR + CI-lint, поэтому frontmatter-, glossary- и drift-проверки одинаково применяются к любому автору (§6).
- Структуру гарантирует CI-lint, а не UI редактора, — гарантия держится независимо от того, кто или что написал файл, включая AI-агента, пишущего без присмотра.
- Нет дополнительного runtime-приложения, которое нужно деплоить, закрывать OIDC, обновлять и держать совместимым с render-pipeline.

**Self-host honest framing.** Doc-repo живёт на GitHub.com (Microsoft US-инфраструктура). Доки prose/specs/ADR/glossary **не содержат ПДн** → 152-ФЗ не нарушается. ПДн платформы живут в RF-Postgres (ADR-0003) и Timeweb Object Storage (ADR-0002). Doc-repo на GitHub.com — приемлемый trade-off, не «полностью self-host». Если требуется full air-gap (например при потере доступа к GitHub.com) — fallback к self-hosted Gitea/GitLab, контент переезжает как есть. Trigger: блокировка GitHub.com из RF, или политическое решение перенести source code в RF.

**Content format.** Prose collections (PRD chapters, Vision, business-rules) и glossary пишутся как stock MDX с YAML-frontmatter — ровно тот формат, который потребляет Fumadocs, без промежуточного слоя сериализации. У каждой коллекции есть файл-шаблон, чтобы новая страница начиналась с нужной формы frontmatter.

**Альтернативы и почему не выбраны:**

- **Отдельный block-editor CMS над репозиторием (Git-backed):** даёт Notion-like UX одному prose-автору ценой ещё одного Next.js-приложения, OIDC-закрытого хоста, слоя схем в поддержке и editor-диалекта markdown, который надо мирить с MDX-render-pipeline. При текущем объёме доков не окупается.
...
**Risk acknowledged:** Product Lead пишет Markdown руками, без block-редактора. Mitigation: шаблоны на каждую коллекцию, ревью в PR и CI-lint, который сообщает о некорректной странице человеческим языком.
...
**Нумерация EARS — плоская по умолчанию.** EARS-требования в `NNN-requirements.md` нумеруются `EARS-1`, `EARS-2`, `EARS-3`, …, без под-компоненты `.M`, независимо от общего числа. Имя Vitest-теста соответствует: `it('EARS-N: when <trigger>, system shall <behavior>', () => { … })`. Вложенный `EARS-N.M` допустим **только** тогда, когда один обработчик действительно несёт несколько shall-выражений (например, OIDC-callback, который и upsert'ит профиль врача, и эмитит событие `DoctorRegistered` — `EARS-3.1` create/upsert + `EARS-3.2` emit). Триггер для вложенности — «одно EARS-предложение становится трудно сформулировать без 'and' или 'while'»; если плоский номер вытягивает, используем плоский. Тест, покрывающий **два соседних требования сразу**, использует составную форму `EARS-N/M:` (например `EARS-25/16:`), а любой EARS-заголовок может нести аннотацию issue `EARS-N (#issue):`.

**EARS-id несут только тесты уровня требования.** Юнит-тест против детали реализации — и регрессия с тегом `#issue:` — законно несёт **никакого** EARS-id: по стандарту двунаправленной трассируемости требований навешивать id на каждый тест — это анти-паттерн «высокое покрытие кода при низком покрытии требований». Контракт с двух сторон обеспечивают два WARN-guard'а (ADR-0007 §2.6): **`ears-tests`** (`ears-test-lint.ts`) — направление **coverage + orphan**: у каждого id требования есть заголовок теста, который его цитирует, и ни один заголовок не цитирует требование, которого нет в спеке (читает id из заголовков `it`/`test`/`describe` по `*.test`/`*.spec`/`*.e2e-spec`, в плоской, вложенной и составной формах, и из билингва-сплита `NNN-requirements-en.md`); **`ears-naming`** (`ears-naming-lint.ts`) — направление **формата**: заголовок, который _пытается_ быть EARS, обязан написать префикс канонически (строчный `ears-3:`, без дефиса `EARS3:` или без двоеточия — флагаются; обычный не-EARS заголовок не трогается).
...
- Master = `apps/docs/content/product/glossary/*.md` — file-per-term collection. Каждый файл несёт frontmatter (`title` / `description` / `lang`) и указывает **канонический id в теле** маркером `**Canonical id:** \`snake_id\``(рядом с`**Bounded context:** <ctx>`), после чего идёт markdown-определение. Канонический id — это машинно-проверяемый ключ (парсится glossary-guard'ами), а не поле frontmatter.
- Generated artifact = `packages/glossary/src/ids.ts` (TS const enum) + sync в Payload Glossary Collection.
- 3 CI validation layers + 1 CI roundtrip-check:

1.  **MDX glossary-lint** — кастомный AST-парсер сканирует glossary-директивы `[[g:term-id]]` в `apps/docs/content/**/*.{md,mdx}`; неразрешённый id без same-line opt-out `new-term: <id>` → fail.
2.  **ESLint `local/glossary-canonical-ids`** — TS-литералы, совпадающие с GlossaryId, должны импортироваться из `@ds/glossary/ids`, не быть инлайн-string'ом.
3.  **Payload Lexical glossary-ref check** — каждый `<GlossaryRef id="...">` в Payload Lexical AST экспорта существует в glossary.
...
| **Glossary lint (3 CI checks)** | custom MDX-lint + ESLint custom rule + Payload AST scan | См. §6 выше                                                                                                                                            |
...
| **Module README**               | `tools/lint/module-readme-lint.ts`                      | Каждая top-level NestJS-директория модуля (прямой потомок `apps/<app>/src/` с файлом `*.module.ts`) имеет README (warn-only v1; exports ↔ README в v2) |
| **Docs build**                  | `apps/docs` next build                                  | Fumadocs билдится без ошибок                                                                                                                           |

**Расположение module-README.** _Модуль_ — это top-level NestJS-директория модуля: прямой потомок `apps/<app>/src/`, содержащий хотя бы один файл `*.module.ts` (например `apps/api/src/auth/`); его README лежит в корне этой директории (`apps/<app>/src/<module>/README.md`). NestJS-модули вложены прямо под `src/` — промежуточной директории-обёртки `modules/` нет. App-root composition-модуль (`apps/<app>/src/app.module.ts`, без поддиректории) вне scope, а вложенный sub-модуль покрывается README родительского модуля. `module-readme-lint.ts` в v1 проверяет только наличие README (warn-only); сверка exports ↔ README — это ужесточение v2.
...
- **Доки в Git, без отдельного prose-store** — каждая правка это коммит, отревьюенный в PR и видимый AI следующей сессией; prose и код держат один и тот же gate.
...
- **Self-host runtime stack** (Fumadocs portal, lint-tools) — все compute в RF-периметре. Документация в Git на GitHub.com — приемлемый trade-off (нет ПДн в doc-repo), 152-ФЗ не нарушается. Trigger to revisit: блокировка GitHub.com или политическое решение перенести source code в RF (Gitea/GitLab self-host).
- **Контент-portability**: контент = stock `.md`/`.mdx`/`.yaml` — portal swappable без потери данных.
...
- **Product Lead пишет Markdown руками** — без block-редактора; frontmatter и синтаксис директив это новая среда; первый месяц + туториал + шаблоны на каждую коллекцию.
- **Glossary 3-layer validation** требует написать ~3 custom lint-скрипта в `tools/lint/` (~300 строк TS). Не trivial, но прямой pattern.
...
- **Fumadocs youth** — render-pipeline может сломаться на major upgrade Next.js. Mitigation: pin major Next.js, проходим upgrade через canary branch.
- **Product Lead продолжает писать в Notion** — социальный risk. Mitigation: явно сказать «Notion больше не Master для DS Platform docs»; деактивировать соответствующие Notion-страницы (или сделать read-only mirror через CI).
- **Автор пишет в `apps/docs/content/` без нужной формы** — например добавляет `.md` файл без frontmatter или с битым маркером канонического id; UI редактора, который заблокировал бы save, нет, ловит только CI. Mitigation: frontmatter/MDX-lint в CI — fail PR, если файл не соответствует schema своей коллекции.
...
- ADR-0005 — Module README pattern переиспользуется в `apps/mobile/src/<module>/`

**Делегировано в другие задачи:**

- **DSO-31 (Repo strategy / Engineering readiness):** monorepo tooling финализация (Turborepo); CI workflow.yml; Fumadocs setup; AGENTS.md/CLAUDE.md draft; первый glossary YAML scaffold; lint-tools пакет; sync-glossary-to-payload script; deployment домен `docs.doctor.school`.
...
- **DSO-31** — структура `apps/docs/`, `packages/glossary/`, `tools/lint/`.
