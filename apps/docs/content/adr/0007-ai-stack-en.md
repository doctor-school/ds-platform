...
- **Supported local harnesses: Claude Code and Codex.** Both implement the same project discipline for interactive development and review. Claude-specific bindings live in `CLAUDE.md`; Codex role profiles live in `.codex/agents/` and inherit supported parent model settings.
- **Optional async: Codex cloud.** Activated separately by the Tech Lead when an async task is needed; local Codex use does not depend on a cloud integration, GitHub App or label trigger.
- **Deferred: Cursor.** Trigger: hiring of a second engineer with inline-AI workflow preference.

All harnesses follow the same orchestrated iteration cycle (§2.4) and read [portable agent discipline](../agent-discipline.md) at entry and after compaction. AGENTS.md is universal; shared instructions and skills map capabilities to actual tools, permissions, model settings and telemetry. Codex does not implicitly load CLAUDE.md or Claude rules. Missing required capabilities remain explicit prerequisites; configured hooks are not proof of trusted or observed enforcement. Owner authorization persists within its approved scope; Stage A/B, destructive-infra and release gates retain their explicit evidence requirements.

### 2.3 SDD + TDD as hard rules

- **SDD:** no production code without a feature spec in `apps/docs/content/specs/features/NNN-<slug>/` (3 files: requirements/design/scenarios — format from ADR-0006 §4). If there is no spec, route through the project `author-feature-spec` skill; a user-facing feature without a PRD starts with `do-product-discovery` (ADR-0014).
...
- **Codex:** AGENTS.md §3.5 and portable agent discipline require `pnpm bootstrap` when the active harness has not supplied a snapshot. Read command output explicitly; do not assume Claude SessionStart injection.
- **Manual:** `pnpm bootstrap` alias.

The rollup is derived context, not board ground truth: reconcile open Issues/Projects and preserve fetch failures as unavailable evidence. Sketch and edge cases — design spec §4.

### 2.6 AI-specific CI drift guards (on top of ADR-0006 §7)

| Guard                           | What it catches                                                                                                                                                                                                                                                                                                                                                                           | Severity Phase 0      |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| **spec-link required**          | PR labeled `feature:NNN-<slug>` not linked to its spec: no `Closes #N`, a linked Issue without a (track-release) milestone, or the label's spec folder (`features/NNN-<slug>/`) missing. Non-feature PR (bug/chore) — skipped.                                                                                                                                                            | BLOCK                 |
...
| **prod-surface readiness**      | unclassified prod-reachable route (App-Router page absent from `tools/lint/prod-surface-manifest.yaml`); a `deferred` route without an open tracking Issue; scaffold-tell copy (ADR/EARS refs, «Каркас приложения») or a session-dump body on a `product-ready` route                                                                                                                     | BLOCK (day-0)         |
| **handoff task-kind ↔ surface** | `handoff:verify` (#778): a handoff declaring an IMPLEMENTATION / feature-iteration task-kind that routes a `feature:*`-labelled, user-facing-surface Issue straight to code while its owning feature-spec has no `NNN-product.md` PRD (ADR-0014) — the #768/#776 SDD bypass. Emits a WARN row + stderr nudge (route via `do-product-discovery` first), never bumps `stale` → exit stays 0 | WARN (Phase 0)        |

Implementation in `tools/lint/*.ts` (one script per guard). These guards are CI signals visible directly to the human reviewer in the PR UI: WARN-only guards appear as non-blocking checks; BLOCK guards prevent merge. Their role is "nudge the human reviewer" — they are inputs to human review, not inputs to an automated reviewer. The table above is the founding set; the guard inventory grows feature-by-feature, and the **authoritative list + current severity of every guard is `.github/workflows/ci.yml` + `.github/workflows/pr-body-guards.yml`** — the PR-body-parsing family (`spec-link`, `registry-research`, `spec-status-fresh`, `prior-decisions`) lives in the dedicated workflow so a PR body edit re-runs it without the full matrix (#651).

**Severity contract — one unambiguous rule.** Severity lives in exit codes; the merge gate blocks on any red check-run. There is no name-based exemption anywhere in the tooling.

- **WARN — visible, never merge-blocking.** A WARN guard runs as a `continue-on-error: true` STEP (so one flagged guard never stops the others), and its batch's closing step REPORTS the flagged guards to the job summary and exits 0. Those two exit codes are the whole mechanism: a run carrying WARN findings CONCLUDES SUCCESS, so no merge gate ever has to forgive it. Visibility is the per-step red annotation + the step log + that summary. WARN is also absent from the `ci` meta-job needs-list. A mechanism that re-reds a check-run to make WARN "count" is a WARN guard promoted by the back door.
- **BLOCK — blocks.** A BLOCK guard runs as a plain step whose failure fails its batch. `ci`-visible BLOCK jobs sit in the `ci` meta-job needs-list; a PR-body BLOCK guard (`spec-link`) cannot (cross-workflow `needs` is impossible) and blocks through its red head-SHA check-run, which the merge gate reads directly. A batch may host both classes: its red then belongs to the BLOCK members.
- **Tool-encoded severity (the `primitives-first` posture).** A guard may carry BOTH severities inside one script: its WARN classes print findings and exit 0, only its BLOCK class exits non-zero. The exit code is then the whole severity signal, so such a step is a plain step of a BLOCK batch — a `continue-on-error` wrapper would swallow its BLOCK class.
- **A red WARN batch means the batch did not run — and blocks.** Once WARN findings conclude SUCCESS, the only ways the WARN batch can fail are checkout/install failure, a dead runner, a malformed step, or a new WARN guard added without `continue-on-error`. "The guards never ran" is indistinguishable from "everything is clean" to anyone reading the board, so both merge classifiers (`tools/gh/merge-gate.mjs`, `tools/gh/wait-ci-green.mjs`) keep one total rule: any non-success terminal check-run blocks, whatever it is called. Exempting the WARN batch by name would add nothing in the benign case and fail OPEN in exactly this one.

**New-guard posture.** A new guard lands as WARN v1, unless a documented security mandate requires hard failure from day one (the `endpoint-authz` class, per ADR-0001 design §2.5). The `prod-surface readiness` guard carries such a documented day-0-BLOCK mandate: Issue #771 (wave-1 prod-smoke retro — enforcement of «no scaffold face in prod», the F-22 class).
...
Promotion mechanics: move the guard's STEP into the BLOCK batch, drop `continue-on-error` and the guard's row from the WARN-report step, update both batches' header comments. Dropping `continue-on-error` IS the promotion — from that moment the guard's failure fails its batch's check-run, which the merge gate blocks on. **Demotion:** one confirmed false positive on a BLOCK guard demotes it back to WARN in the same beat, with an Issue tracking the guard fix. **Cadence:** the whole WARN set is re-evaluated at every "drain the matured debt backlog" checkpoint (AGENTS.md §3.5); each sweep records a dated per-guard verdict (promoted / left WARN + reason) in the Issue or PR that carries it.
...
Design policy for project-owned runtime LLM clients (future Content Pipeline, etc.); interactive harness caching is controlled by the active client:

- `cache_control: ephemeral` on 3 stable tier blocks (Anthropic limit = 4 breakpoints; 3 of 4 are used): (1) AGENTS.md + portable agent discipline + applicable harness overlay, (2) active spec 3 files concat, (3) ADRs sorted concat. One breakpoint remains free for future expansion (module READMEs, persona configs, etc.).
...
(ii) >50 PRs of review-loop data exist (interactive `request-mode-a-review` skill outputs logged manually, OR an automated reviewer-bot is reconsidered and built).
...
| **Self-hosted GHA runner**                   | Not AI-specific; owned at the repo level (ADR-0008 §2.8). CI runs GitHub-hosted; the BBM shared pool was trialled and rejected on queue starvation (#1249). Re-trigger: the pending post-revert minute-consumption review finds hosted runners not viable on cost |
...
- **Local Claude Code and Codex share one execution contract** — capabilities and evidence are mapped per harness; Claude retains SessionStart injection through `.claude/settings.json`.
...
- **Codex cloud stays optional** — its access and setup are verified separately; local development and independent Codex review do not wait for cloud activation.
...
| **Claude Code only (no Codex)**                                                             | Vendor lock-in without diversification; Mode (b) parallel-Codex review path loses its purpose. Local Codex provides a supported second harness; cloud async remains optional.                                                                                                                                      |
...
| OQ-AI6  | Optional Codex cloud integration specifics                                                                                                                                                         | When Tech Lead activates cloud async (§2.11); not a local Codex prerequisite                            |
