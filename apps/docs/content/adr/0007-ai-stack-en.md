...
**Severity contract — one unambiguous rule.** Severity is a property of the CHECK-RUN, because a check-run carries exactly one bit of verdict:

**Severity lives in exit codes; the merge gate blocks on any red check-run.** There is no name-based exemption anywhere in the tooling.
