...
**Контракт severity — одно однозначное правило.** Severity — свойство CHECK-RUN'а, потому что check-run несёт ровно один бит вердикта:

**Severity живёт в exit-кодах; merge gate блокирует на любом красном check-run.** Никаких исключений по имени в тулинге нет.
