...
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| **spec-link required**          | PR с label `feature:NNN-<slug>`, не связанный со своей спекой: нет `Closes #N`, у связанного Issue нет (релизного) milestone, либо отсутствует spec-папка лейбла (`features/NNN-<slug>/`). Non-feature PR (bug/chore) — skipped.                                                                                                                                       | BLOCK                   |
...
| **handoff task-kind ↔ surface** | `handoff:verify` (#778): handoff, объявляющий task-kind IMPLEMENTATION / feature-iteration и отправляющий `feature:*`-labelled user-facing Issue сразу в код, когда у его feature-спеки нет `NNN-product.md` PRD (ADR-0014) — обход SDD из #768/#776. Даёт WARN-строку + stderr-подсказку (сначала через `do-product-discovery`), не трогает `stale` → exit остаётся 0 | WARN (Phase 0)          |

Реализация в `tools/lint/*.ts` (один скрипт на guard). Эти guard'ы — CI-сигналы, видимые прямо человеку-ревьюверу в PR UI: WARN-guard'ы — non-blocking checks, BLOCK-guard'ы — блокируют merge. Их роль — «подсказать человеку-ревьюверу»; они вход для human review, а не для автоматического ревьювера. Таблица выше — стартовый набор; инвентарь guard'ов растёт от фичи к фиче, и **авторитетный список + текущая severity каждого guard'а — `.github/workflows/ci.yml` + `.github/workflows/pr-body-guards.yml`** — семейство body-parsing guard'ов (`spec-link`, `registry-research`, `spec-status-fresh`, `prior-decisions`) живёт в выделенном workflow, чтобы правка тела PR перезапускала его без полной матрицы (#651).

**Контракт severity — одно однозначное правило.** Severity живёт в exit-кодах; merge gate блокирует на любом красном check-run. Никаких исключений по имени в тулинге нет.

- **WARN — видим, никогда не блокирует merge.** WARN-guard выполняется как STEP с `continue-on-error: true` (чтобы один сработавший guard не останавливал остальные), а закрывающий шаг его batch'а ПЕЧАТАЕТ список сработавших guard'ов в job summary и выходит с 0. Эти два exit-кода и есть весь механизм: ран с WARN-findings ЗАВЕРШАЕТСЯ УСПЕХОМ, поэтому ни одному merge gate не приходится ничего прощать. Видимость — красная аннотация на шаге + лог шага + этот summary. WARN также отсутствует в needs-list meta-job'а `ci`. Механизм, который перекрашивает check-run в red, чтобы WARN «считался», — это promotion WARN-guard'а с чёрного хода.
- **BLOCK — блокирует.** BLOCK-guard выполняется как обычный step, падение которого роняет его batch. BLOCK-job'ы, видимые `ci`, стоят в needs-list meta-job'а `ci`; body-parsing BLOCK-guard (`spec-link`) там стоять не может (cross-workflow `needs` невозможен) и блокирует через красный check-run по head SHA, который merge gate читает напрямую. Batch может нести оба класса: тогда его red принадлежит BLOCK-участникам.
- **Severity, закодированная в инструменте (posture `primitives-first`).** Guard может нести ОБЕ severity внутри одного скрипта: его WARN-классы печатают findings и выходят с 0, ненулевой код даёт только BLOCK-класс. Тогда весь сигнал severity — это exit-код, поэтому такой step стоит обычным шагом BLOCK-batch'а: обёртка `continue-on-error` проглотила бы его BLOCK-класс.
- **Красный WARN-batch означает, что batch не отработал, — и блокирует.** Раз WARN-findings завершаются успехом, WARN-batch может упасть только из-за сбоя checkout/install, умершего раннера, сломанного шага или нового WARN-guard'а, добавленного без `continue-on-error`. «Guard'ы вообще не запускались» на доске неотличимо от «всё чисто», поэтому оба merge-классификатора (`tools/gh/merge-gate.mjs`, `tools/gh/wait-ci-green.mjs`) держат одно тотальное правило: любой не-success терминальный check-run блокирует, как бы он ни назывался. Исключение WARN-batch'а по имени ничего не дало бы в безобидном случае и сработало бы fail-OPEN ровно в этом.
