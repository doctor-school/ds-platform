...
| CI runner              | BBM self-hosted pool `bbm-ci`, per-job resource profiles (`light`/`browser`/`heavy`/`docker`/`notify`); `ubuntu-latest` only for the two credential-blocked bot jobs (#1224) | §2.8                        |
