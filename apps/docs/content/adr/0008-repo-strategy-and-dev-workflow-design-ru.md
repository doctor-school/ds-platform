...
| CI runner              | Self-hosted пул BBM `bbm-ci` для 36 eligible-jobs `ci.yml`; GitHub-hosted `ubuntu-latest` keep-set (docker/browser/egress-bound jobs, guards, release/bot workflows) | §2.8                        |
...
- **Repo:** pnpm workspaces + Turborepo + changesets + GitHub Actions CI на self-hosted пуле BBM + hosted keep-set (ADR-0008 §2.8)
