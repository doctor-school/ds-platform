...
- **Split runner topology.** The `eligible` `ci.yml` jobs (36 — per-job verdict table in #1224) run on the shared BBM ephemeral self-hosted pool `bbm-ci`: a repo-scoped JIT runner + container per job, selector `[self-hosted, linux, x64, bbm-ci, node22, heavy, ci]`; onboarding/runtime contract `sidorovanthon/bbm/infra/ci-runners/REPOSITORY-ONBOARDING.md` — runner infra, host ops, egress policy and monitoring are owned by `sidorovanthon/bbm`, not here. GitHub-hosted `ubuntu-latest` keeps the pool-incompatible set: `api-e2e` (docker-run Postgres), `markdown-links` (unbounded link-crawl egress), `playwright-axe` / `playwright-axe-portal` (browser + apt deps), the `ci` meta-job, plus `pr-body-guards.yml`, `release.yml` and the bot workflows (write tokens / webhook secrets stay off the shared pool). The hosted remainder (~1600 min/month, measured 2026-08) fits the GitHub Free private-repo quota of 2000 min/month.
...
- **Pool activation gates (tracked in #1224):** the pool serves this repo only once the least-privilege BBM GitHub App is installed here (numeric installation id + explicit runner-group id recorded in the bbm manager policy), the repo is private (pool v1 contract), and the bbm manager is deployed with the `doctor-school/ds-platform` policy. A CI job that needs RF-private network access goes to neither runner class — production deploy runs off-CI (ADR-0012).
...
- **Free CI minutes — a bottleneck once the repo is private**. The GitHub Free 2000 min/month quota covers only the keep-hosted remainder (~1600 min/month); the 36-job eligible set rides the BBM pool (§2.8). Trade-off accepted with it: the pool's single global slot serializes the fan-out (~4 min parallel wall-time becomes ~35–45 min serial per CI run); the mitigation path is bbm-side slot scaling, not a hosted fallback.
