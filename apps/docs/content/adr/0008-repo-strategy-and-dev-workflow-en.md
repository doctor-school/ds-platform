...
- **Release-train escape hatch.** The Changesets bot branch `changeset-release/main` carries no CI check-runs, so `ci` is unsatisfiable there forever. Its documented merge — raw `gh pr merge --squash --delete-branch` run by the owner's admin token — proceeds through the `pull_request`-scoped bypass actor. This is the single, documented manual override; the gate on it is the file-scope + main-CI-green check in `repo-conventions.md` → _Version-Packages release PR_.
