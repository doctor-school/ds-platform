...
- **Pool runner topology.** CI jobs run on the shared BBM ephemeral self-hosted pool `bbm-ci` — a repo-scoped JIT runner + container per job — under per-job resource profiles: `light` (up to four parallel lanes), `browser` (Chromium deps baked into the `node22` image), `heavy` (exclusive; monorepo builds/tests), `docker` (exclusive; a per-job rootless Docker daemon — carries the Postgres-backed `api-e2e` with its `docker build/run` steps verbatim), `notify` (light-sized; secret-bearing notification jobs in a policy block that admits only trusted non-PR events — carries the release `digest`). Selector `[self-hosted, linux, x64, bbm-ci, node22, <profile>, ci]`; the per-job profile table lives in #1224. Onboarding/runtime contract `sidorovanthon/bbm/infra/ci-runners/REPOSITORY-ONBOARDING.md` — runner infra, host ops, egress policy and monitoring are owned by `sidorovanthon/bbm`, not here. GitHub-hosted `ubuntu-latest` keeps only the two credential-rule-blocked bot jobs — `dependabot-auto-merge` and `product-note-mattermost` (pool policy forbids write tokens / webhook secrets on PR events) — pending a bbm-side policy nuance or a re-trigger rework tracked in #1224, plus the three `if: false` placeholders; that remainder is tens of minutes/month against the GitHub Free private-repo quota of 2000.
...
- **Pool activation gates (tracked in #1224):** the pool serves this repo only once a least-privilege BBM GitHub App the `doctor-school` org can install is available (the `bbm-academy-org` private App cannot be installed cross-owner — it goes public or a dedicated App is created) and installed on this repo only, with the numeric installation id + explicit runner-group id recorded in the bbm manager policy; the repo is private (pool contract); and the bbm manager is deployed with the four per-profile `doctor-school/ds-platform` policy blocks. A CI job that needs RF-private network access goes to neither runner class — production deploy runs off-CI (ADR-0012).
...
- **Minimum moving parts at the start** — no Vault/feature-flags/cache-server in Phase 0. Each is added via an explicit trigger documented either here or in the engineering-readiness spec; self-hosted CI capacity is consumed as a service from the BBM pool (§2.8), not operated from this repo.
...
- **Shared-pool wall-time vs hosted fan-out**. On GitHub-hosted runners the 40+ jobs ran fully parallel (~4 min wall); the pool schedules them through 4 capacity units with exclusive `heavy`/`docker` classes, so a full run is expected around ~12–20 min — the contract mandates measuring at acceptance (two clean runs), not extrapolating. The quota problem this migration solves: ~5000 billed min/month against the 2000 min/month GitHub Free private-repo quota; post-migration the hosted remainder is tens of minutes. Capacity growth (more units, host resize) is a bbm-side decision, not a hosted fallback.
...
| **Self-hosted Forgejo Actions / Drone / Woodpecker**         | Replacing the CI _system_ mismatches the GitHub-built loop (Issues, `gh` tooling, PR auto-close). Self-hosted capacity is taken at the _runner_ layer instead — the BBM pool (§2.8). Rejected.                                                                  |
...
| OQ-R12 | Self-hosted GHA runner — concrete setup                                                                                                                                                                                  | **CLOSED** — shared BBM ephemeral pool per §2.8; host/manager/egress owned by `sidorovanthon/bbm` (its spec is the SSOT), no dedicated runner VPS and no separate ADR here |
...
- **Issue #1224 (BBM runner-pool onboarding):** selector migration, activation gates and acceptance evidence per the bbm onboarding contract; the pool architecture spec lives in `sidorovanthon/bbm`, so no runner ADR exists in this repo.
