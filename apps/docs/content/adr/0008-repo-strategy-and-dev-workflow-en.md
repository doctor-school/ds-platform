...
- **GitHub repository:** `doctor-school/ds-platform`, **private** (org plan: GitHub Free). Visibility is an owner-owned lever with a live trade-off, not a closed question: private includes 2000 Actions min/month (then $0.008/min for 2-core `ubuntu`) and leaves the branch-protection API paywalled (§2.6 reactivation triggers); **public** unlocks both for free — unmetered hosted minutes and a reachable protection API — at the cost of the source being readable. Public was in fact the state held until 2026-08-12, taken precisely as the mitigation for those two paywalls, and it remains available whenever billed minutes or the missing protection API become the binding constraint. Staying private instead requires an enabler on the cost side — a CI-minutes diet that fits the free quota (§2.8 batching), or a GitHub Team upgrade. Visibility does **not** affect ownership: the code is `UNLICENSED` (proprietary, all rights reserved) at any visibility; an opened state is _source-available_, not open-source (ADR-0013 §5).
...
- **CI runner cost review (follows #1249):** after 2–4 weeks on GitHub-hosted runners, pull actual Actions minute consumption from org billing and weigh it against a managed runner service, the amortised cost of a dedicated box, and a flip back to public (§2.1); record the verdict as an ADR-0008 revision. The self-hosted pool trial (#1224) is closed — reverted under #1249.
