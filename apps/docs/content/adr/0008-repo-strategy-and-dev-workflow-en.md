...
- **Branch protection rule on `main` — target state, activation tracked separately.** The `doctor-school/ds-platform` repo is **public** (the `doctor-school` organisation stays on the GitHub Free plan), so the branch-protection API and Rulesets are available — the reactivation trigger below has fired. The rule list is still the **target contract** and is not yet enforced server-side: applying it verbatim would deadlock the release train, because the Changesets bot branch `changeset-release/main` carries no CI check-runs, and a naive `required_status_checks` on `main` would leave the Version-Packages PR permanently unmergeable. Choosing the rule shape that avoids that (a ruleset with a bot bypass, or protection without required status checks) and applying it is tracked as its own engineering task — Issue #1403. Until it lands, the interim process-level gate below is the operative mechanism.
...
- **Interim (Phase 0) merge gate — process-level, not server-side.** Same intent, different mechanism:
  ...
  | 4. Required status check `ci` — passing | `pnpm pr:land <N>` is the standard merge command: its deterministic gate reads the head-SHA-pinned check-runs and refuses the squash-merge unless they are terminal-green (a Mode-a APPROVE pinned to the same head is checked alongside). `gh pr merge --auto` is NOT usable — with no server-side protection GitHub rejects it on `main` as "Pull request is in clean status". |
  | 5. Branches up-to-date before merging   | Convention: `git pull --rebase origin main` before push; the head-SHA pinning in the `pr:land` gate makes a stale-head merge fail rather than pass silently.                                                                                                                                                                                                                     |
...
- **Reactivation trigger — fired.** The trigger was "the API becomes reachable": the org upgrades to GitHub Team/Enterprise, **or** the repo is made public (branch protection is free on public repos), or the repo migrates to a forge where the equivalent feature is free (a separate ADR in that case). The middle condition is now true — the repo is public. What remains is not the trigger but the rule shape: the 10-item list cannot be applied verbatim while `changeset-release/main` has no check-runs, so Issue #1403 owns picking the shape, applying it, reconciling `branch-protection.json` with what is actually enforced, and re-verifying both a normal PR and a Version-Packages PR afterwards.
...
| OQ-R14 | Reactivation discipline owner — the §2.6 trigger has fired (the repo is public); who is responsible for applying the branch-protection contract?                                                                         | Owner: Tech Lead. Tracked as GitHub Issue #1403, which also owns the rule shape that keeps the Version-Packages bot branch mergeable.                                           |
