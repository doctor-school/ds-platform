...
- **Branch protection rule на `main` — target state, активация трекается отдельно.** Репо `doctor-school/ds-platform` — **public** (организация `doctor-school` остаётся на GitHub Free), поэтому branch-protection API и Rulesets доступны — reactivation trigger ниже сработал. Правила ниже всё ещё **target contract** и на сервере пока не enforce'ятся: применить их дословно значит заблокировать release train, потому что bot-ветка Changesets `changeset-release/main` не несёт ни одного CI check-run, и наивный `required_status_checks` на `main` сделал бы Version-Packages PR навсегда неmergeable. Выбор формы правила, которая этого избегает (ruleset с bypass для бота либо protection без required status checks), и её применение вынесены в отдельную инженерную задачу — Issue #1403. Пока она не закрыта, действует interim process-level gate ниже.
...
- **Interim (Phase 0) merge gate — process-level, не server-side.** Та же intent, другой механизм:
  ...
  | 4. Required status check `ci` — passing | `pnpm pr:land <N>` — стандартная merge-команда: её детерминированный gate читает check-run'ы, привязанные к head-SHA, и отказывает в squash-merge, пока они не терминально-зелёные (там же проверяется Mode-a APPROVE, привязанный к тому же head). `gh pr merge --auto` НЕ применим — без серверной protection GitHub отклоняет его на `main` как "Pull request is in clean status". |
  | 5. Branches up-to-date before merging   | Convention: `git pull --rebase origin main` перед push; привязка к head-SHA в gate `pr:land` делает merge со stale-head падающим, а не молча проходящим.                                                                                                                                                                                                                              |
...
- **Reactivation trigger — сработал.** Триггером было «API становится доступным»: организация апгрейдится на GitHub Team/Enterprise, **или** репо переводится в public (на public repos branch protection бесплатна), или репо переезжает на forge, где эквивалентный feature бесплатен (тогда — отдельный ADR). Второе условие выполнено: репо public. Остался не триггер, а форма правила: список из 10 пунктов нельзя применить дословно, пока у `changeset-release/main` нет check-run'ов, поэтому Issue #1403 владеет выбором формы, её применением, приведением `branch-protection.json` в соответствие с реально enforce'ящимся состоянием и перепроверкой после включения — обычным PR и Version-Packages PR.
...
| OQ-R14 | Reactivation discipline owner — trigger §2.6 сработал (репо public); кто отвечает за применение branch-protection контракта?                                                                                         | Owner: Tech Lead. Трекается как GitHub Issue #1403, которая владеет и формой правила, сохраняющей mergeable bot-ветку Version-Packages.                                        |
