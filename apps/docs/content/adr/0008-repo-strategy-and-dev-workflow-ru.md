...
- **Разделённая runner-топология.** `eligible`-jobs из `ci.yml` (36 — по-job'овая таблица вердиктов в #1224) выполняются на общем эфемерном self-hosted пуле BBM `bbm-ci`: repo-scoped JIT runner + контейнер на каждый job, селектор `[self-hosted, linux, x64, bbm-ci, node22, heavy, ci]`; контракт подключения/рантайма — `sidorovanthon/bbm/infra/ci-runners/REPOSITORY-ONBOARDING.md`; инфраструктура раннеров, host ops, egress-политика и мониторинг принадлежат `sidorovanthon/bbm`, не этому репо. GitHub-hosted `ubuntu-latest` сохраняет несовместимый с пулом набор: `api-e2e` (docker-run Postgres), `markdown-links` (неограничиваемый egress link-crawl'а), `playwright-axe` / `playwright-axe-portal` (браузер + apt-зависимости), мета-job `ci`, плюс `pr-body-guards.yml`, `release.yml` и бот-workflows (write-токены / webhook-секреты не попадают на общий пул). Hosted-остаток (~1600 min/мес, замер 2026-08) укладывается в квоту GitHub Free для private repo — 2000 min/мес.
...
- **Активационные гейты пула (трекаются в #1224):** пул обслуживает этот репозиторий только после того, как least-privilege GitHub App BBM установлена сюда (numeric installation id + explicit runner-group id записаны в policy manager'а bbm), репозиторий переведён в private (v1-контракт пула) и manager bbm развёрнут с policy `doctor-school/ds-platform`. CI-job, которому нужен доступ в RF-private network, не попадает ни в один runner-класс — production deploy выполняется вне CI (ADR-0012).
...
- **Free CI minutes — узкое горло после перевода репо в private**. Квота GitHub Free 2000 min/мес покрывает только keep-hosted остаток (~1600 min/мес); 36 eligible-jobs едут на пуле BBM (§2.8). Принятый вместе с этим trade-off: единственный глобальный slot пула сериализует fan-out (~4 мин параллельного wall-time превращаются в ~35–45 мин последовательных на прогон CI); путь митигации — масштабирование слотов на стороне bbm, а не hosted-fallback.
