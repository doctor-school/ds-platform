...
- **Escape hatch для release train.** Bot-ветка Changesets `changeset-release/main` не несёт ни одного CI check-run, поэтому `ci` там неудовлетворим никогда. Её документированный merge — сырой `gh pr merge --squash --delete-branch` под admin-токеном владельца — проходит через bypass actor в режиме `pull_request`. Это единственный документированный ручной override; гейтом на нём остаётся проверка состава файлов + зелёного main-CI из `repo-conventions.md` → _Version-Packages release PR_.
