...
- **Runner-топология пула.** CI-jobs выполняются на общем эфемерном self-hosted пуле BBM `bbm-ci` — repo-scoped JIT runner + контейнер на каждый job — с ресурсным профилем на каждый job: `light` (до четырёх параллельных линий), `browser` (Chromium-зависимости запечены в образ `node22`), `heavy` (эксклюзивный; сборки/тесты монорепо), `docker` (эксклюзивный; отдельный rootless Docker-daemon на job — несёт `api-e2e` с его Postgres и шагами `docker build/run` без изменений), `notify` (размер light; notification-jobs с секретом в policy-блоке, принимающем только доверенные не-PR события — несёт релизный `digest`). Селектор `[self-hosted, linux, x64, bbm-ci, node22, <profile>, ci]`; по-job'овая таблица профилей — в #1224. Контракт подключения/рантайма — `sidorovanthon/bbm/infra/ci-runners/REPOSITORY-ONBOARDING.md`; инфраструктура раннеров, host ops, egress-политика и мониторинг принадлежат `sidorovanthon/bbm`, не этому репо. GitHub-hosted `ubuntu-latest` сохраняет только два бот-job'а, заблокированных credential-правилом пула — `dependabot-auto-merge` и `product-note-mattermost` (policy запрещает write-токены / webhook-секреты на PR-событиях) — до policy-нюанса на стороне bbm либо переустройства триггеров (трекается в #1224), плюс три `if: false` заглушки; этот остаток — десятки минут в месяц против квоты GitHub Free 2000 min/мес для private repo.
...
- **Активационные гейты пула (трекаются в #1224):** пул обслуживает этот репозиторий только после того, как появится least-privilege GitHub App BBM, которую организация `doctor-school` может установить (private App организации `bbm-academy-org` не ставится cross-owner — она становится public либо создаётся отдельная App), установлена она только на этот репозиторий, numeric installation id + explicit runner-group id записаны в policy manager'а bbm; репозиторий переведён в private (контракт пула); manager bbm развёрнут с четырьмя per-profile policy-блоками `doctor-school/ds-platform`. CI-job, которому нужен доступ в RF-private network, не попадает ни в один runner-класс — production deploy выполняется вне CI (ADR-0012).
...
- **Минимум moving parts на старте** — нет Vault/feature-flags/cache-server в Phase 0. Каждый из них добавляется по explicit trigger, документированному либо здесь, либо в engineering-readiness spec; self-hosted CI-мощность потребляется как сервис из пула BBM (§2.8), а не оперируется из этого репо.
...
- **Wall-time общего пула против hosted fan-out**. На GitHub-hosted раннерах 40+ jobs шли полностью параллельно (~4 мин wall); пул планирует их через 4 capacity-units с эксклюзивными классами `heavy`/`docker`, так что полный прогон ожидается в районе ~12–20 мин — контракт требует мерить на acceptance (два clean-run), а не экстраполировать. Проблема квоты, которую решает миграция: ~5000 оплачиваемых min/мес против квоты GitHub Free 2000 min/мес для private repo; после миграции hosted-остаток — десятки минут. Рост мощности (больше units, resize хоста) — решение стороны bbm, а не hosted-fallback.
...
| **Self-hosted Forgejo Actions / Drone / Woodpecker**         | Замена CI-_системы_ не совместима с уже построенной на GitHub петлёй (Issues, `gh`-tooling, PR auto-close). Self-hosted мощность берётся на уровне _runner_'а — пул BBM (§2.8). Rejected.                                                                                             |
...
| OQ-R12 | Self-hosted GHA runner — конкретный setup                                                                                                                                                                            | **CLOSED** — общий эфемерный пул BBM per §2.8; host/manager/egress принадлежат `sidorovanthon/bbm` (его спека — SSOT), без выделенного runner-VPS и без отдельного ADR здесь |
...
- **Issue #1224 (onboarding на runner-пул BBM):** selector-миграция, активационные гейты и acceptance-evidence по onboarding-контракту bbm; архитектурная спека пула живёт в `sidorovanthon/bbm`, поэтому runner-ADR в этом репо не существует.
