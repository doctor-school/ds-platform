...
Все PD-lifecycle таблицы (`consent_*`, `data_export_requests`, `erasure_requests`, `subject_keys`, `audit_retention_keys`) живут в `packages/db/src/schema/pd/`. По DSO-63 #10/I, schemas живут в `packages/db/`, а не в `apps/api`; ADR-0003 §4 (ORM + Migrations) обновляется inline, чтобы зафиксировать этот layout.
