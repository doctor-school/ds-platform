...
**(c) Reviewer-agent — в Phase 0 не реализован.**

Автоматического GitHub-Actions reviewer-bot, упоминавшегося в ранних драфтах, в Phase 0 нет; ADR-0007 §2.10 заменил его на interactive review modes (Mode (a) subagent dispatch, Mode (b) параллельный Codex CLI, Mode (c) human). Interactive modes — локальный developer tooling, не backend AI flow со side-effect tools, поэтому dual-LLM mandate на них не распространяется. Если будущая ADR вернёт автоматический reviewer с write-tools (auto-approve, auto-merge, auto-fix-commit), он ОБЯЗАН быть спроектирован как dual-LLM с самого начала (Q-LLM читает PR/comments → P-LLM с merge-tool работает на trusted refs) — §5.4 ниже служит reference flow для этого случая.
...
**Reference flow only — автоматического reviewer'а в Phase 0 нет** (ADR-0007 §2.10 использует interactive review modes вместо него). **Untrusted source:** PR body + comments (vector 8). Этот flow описывает дизайн, которому ОБЯЗАН следовать любой будущий автоматический reviewer с write-tools.
...
- **Trigger to implement:** любая будущая ADR, возвращающая автоматический reviewer с write-tools, делает этот flow normative.
