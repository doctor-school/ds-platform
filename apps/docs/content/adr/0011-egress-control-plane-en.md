...
|   4 | **Reviewer agent prompts** (cross-vendor LLM review)  | Code review by an independent LLM vendor (ADR-0007 §2.8).            | Diff text + spec body. **Never:** runtime DB content, secrets, prod-traffic captures.                                    | Reviewer-agent sanitizer: strips `.env`, `secrets/*`, `fixtures/prod-*`. Prompt-eval corpus separated from reviewer-prompt. | Per-review log: vendor, model, tokens, sanitization_status.                                                                           | AI lead              |
|   5 | **Cost ledger** (PR target in repo)                   | AI cost tracking (ADR-0007 §2.9).                                    | Metadata only: model, tokens, cost_estimate, run_id. **Strip prompt body** before write.                                 | Cost-ledger writer filters prompt content (sha256-hash references instead of raw text).                                     | Append-only audit row per ledger update.                                                                                              | AI lead              |
...
- Per-channel feature flag in `.github/agents-config.json` (ADR-0007 §2.11) for channels #1, #4, #5 (AI-related). For channels #2, #6, #7, #8, #9 — environment-level disable via config.
...
- **ai-stack-design §6** — must reference §2.2 channels #1 + #4.
...
### Amendment A1 — Channels #4 (Reviewer agent prompts) and #5 (Cost ledger) vestigial post ADR-0007 Amendment A1 (2026-05-19)

**Context:** ADR-0007 Amendment A1 (2026-05-19) dropped the automated GitHub-Actions reviewer-bot and the cost-ledger CI automation. ADR-0011 §2.2 table lists channel #4 (reviewer-agent prompts to external LLM vendor) and channel #5 (cost ledger PR target) — both producers are now removed in Phase 0.

**Effect:**

- **Channels #4 and #5 — vestigial in Phase 0** (no producer). Kept inline in §2.2 as reference design — if a future ADR reinstates an automated reviewer or a cost-ledger writer, the egress contract is pre-defined here without a new amendment.
- **Channels #1 (LLM API providers) and #2 (GitHub) remain load-bearing** — runtime AI flows (chat assistant, content pipeline, NMO-assistant per ADR-0010 §2.3) use channel #1; all source code + docs + ADR/spec text still goes through channel #2.
- **§2.4 sanitizer / quarterly egress audit — scope reduced** to channels #1 and #2 in Phase 0; #4 / #5 audits skip cleanly (no rows to audit).
- **Owner column "Tech Lead / All devs" on channel #2** — typo corrected to "Tech Lead / All devs" in the name-sweep pass (2026-05-19).

**Cross-refs:** ADR-0007 §Amendment A1, ADR-0008 §Amendment A2, ADR-0010 §Amendment A1, AI-stack design spec §6/§7/§10 SUPERSEDED callouts.
