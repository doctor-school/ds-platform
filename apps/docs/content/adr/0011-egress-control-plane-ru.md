...
|   4 | **Reviewer agent prompts** (cross-vendor LLM review)  | Code review by independent LLM vendor (ADR-0007 §2.8).               | Diff text + spec body. **Never:** runtime DB content, secrets, prod-traffic captures.                                               | Reviewer-agent sanitizer: strips `.env`, `secrets/*`, `fixtures/prod-*`. Prompt-eval corpus отделён от reviewer-prompt. | Per-review log: vendor, model, tokens, sanitization_status.                                                                       | AI lead              |
|   5 | **Cost ledger** (PR-target в репо)                    | Кост-трекинг AI-расходов (ADR-0007 §2.9).                            | Metadata only: model, tokens, cost_estimate, run_id. **Strip prompt body** перед записью.                                           | Cost-ledger writer фильтрует prompt content (sha256-hash references вместо raw text).                                   | Append-only audit row на каждый ledger update.                                                                                    | AI lead              |
...
- Per-channel feature flag в `.github/agents-config.json` (ADR-0007 §2.11) для каналов #1, #4, #5 (AI-связанные). Для каналов #2, #6, #7, #8, #9 — environment-level disable через config.
...
- **ai-stack-design §6** — должен ссылаться на §2.2 channel #1 + #4.
...
### Amendment A1 — Каналы #4 (Reviewer agent prompts) и #5 (Cost ledger) vestigial после ADR-0007 Amendment A1 (2026-05-19)

**Контекст:** ADR-0007 Amendment A1 (2026-05-19) дропнул автоматический GitHub-Actions reviewer-bot и cost-ledger CI automation. ADR-0011 §2.2 таблица перечисляет канал #4 (reviewer-agent prompts во внешний LLM vendor) и канал #5 (cost ledger PR target) — оба producer'а удалены в Phase 0.

**Effect:**

- **Каналы #4 и #5 — vestigial в Phase 0** (нет producer'а). Оставлены inline в §2.2 как reference design — если будущая ADR вернёт автоматический reviewer или cost-ledger writer, egress contract уже определён здесь без нового amendment'а.
- **Каналы #1 (LLM API providers) и #2 (GitHub) остаются load-bearing** — runtime AI flows (chat assistant, content pipeline, NMO-assistant по ADR-0010 §2.3) используют канал #1; весь source code + docs + ADR/spec text по-прежнему идёт через канал #2.
- **§2.4 sanitizer / quarterly egress audit — scope сокращён** до каналов #1 и #2 в Phase 0; #4 / #5 audits пропускаются cleanly (нет rows для audit).
- **Owner column "Tech Lead / All devs" в канале #2** — typo исправлен на "Tech Lead / All devs" в name-sweep проходе (2026-05-19).
