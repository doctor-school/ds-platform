...
**Decision.** A capability used by more than one frontend has **one canonical core implementation**. Before new frontend work, the team inventories sibling apps and shared packages for the same capability and records its current owners and extraction targets. A shipped app-local precedent is extracted into the appropriate shared package/module before a second app consumes it; frontends never import from each other and never maintain copy/fork implementations. Shared ownership covers presentation blocks, domain/query services, portable schemas and URL-state codecs, state machines, action/entry policy and realtime/live lifecycle. Thin host projections are allowed only to apply storefront-specific defaults, authorization/targeting, response envelopes, routes, copy and composition; they must delegate to the canonical core rather than reproduce it. Any deliberate behavioural divergence requires an ADR-backed product or technical reason recorded before implementation.

**Consequences.** Doctor's events surface consumes Academy's event-listing core, calendar grids, `WebinarCard`, room live-state and registration/entry lifecycle as shared capabilities. Academy and Doctor may expose different host projections over that core, but neither may own a second query engine, URL-state codec, room or lifecycle. A shared contract may be widened additively for doctor-specific formats only at the canonical owner and remains available to every consumer. Review rejects a visually identical component backed by different app-local behaviour just as it rejects copied JSX. The auth flows are a canonical-core family on the same terms: the auth shell and the login, password-recovery and email-confirmation compositions are owned by `@ds/design-system` blocks, and each storefront mounts a thin projection supplying only its brand assets, localized copy and app policy.
