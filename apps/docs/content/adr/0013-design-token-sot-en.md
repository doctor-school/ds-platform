...
**Approved registry whitelist (committable):** ① official **shadcn/ui** blocks + primitives (incl. `input-otp`), Radix variant · ② **Intent UI / JollyUI** (React-Aria) · ③ **Kibo UI**. Origin UI was removed from the whitelist on 2026-08-27: upstream `origin-space/originui` was absorbed into the `cosscom/coss` monorepo, whose default licence is AGPL-3.0 with `apps/origin/` and `apps/ui/` carved back to their original MIT. The collection is therefore adoptable only through a per-directory provenance check on every copy, and it stopped moving at the absorption — it fails the acceptance bar below on licence clarity and maintenance freshness, not on a blanket relicense (pattern-only reference remains allowed). Acceptance bar for an adopted block: permissive license · correct RSC boundaries · accessibility · no superfluous dependencies · maintenance freshness.
...
Token discipline alone is not enough: a token-clean `className` on a shared primitive still forks that primitive per surface. So **hosts and shared blocks consume DS primitives through props (`variant` / `size` / `tone`) and positional utilities only** — geometry, typography, colour and borders live in the primitive, and a look the primitive does not offer is a new canvas-backed variant (an owner design decision), never a local class stack. Shared blocks take **data props, never a `ReactNode` slot for an interactive element**, so a host cannot smuggle a hand-assembled button through a slot. Enforced by `local/no-primitive-style-override` = **BLOCK** over `apps/**` and the shared storefront packages: it reads the `className` of any JSX element resolving to a `@ds/design-system` import (string, template, `cn()`, conditional and array/object forms; every responsive/state prefix stripped before the check), allows only positional utilities (placement, display, flex/grid participation, order, alignment, overflow, layout width, `sr-only`, `pointer-events-*`), and flags a raw `<a>` / `<button>` / `<input>` / `<select>` / `<textarea>` / `<summary>` carrying non-positional styling as «use the primitive». An escape hatch exists but is never silent: a per-occurrence disable directive must carry ` -- <reason>`, and a bare disable is itself an error.
...
**Decision.** A capability used by more than one frontend has **one canonical core implementation** — a feature package under `packages/<capability>` that exports the complete behaviour: page-level components, hooks, URL-state codecs, post-action routing, error dictionaries, default copy and the tests that came with the first implementation. Before new frontend work, the team inventories sibling apps and shared packages for the same capability and records its current owners and extraction targets. A shipped app-local precedent is extracted into that package before a second app consumes it; frontends never import from each other and never maintain copy/fork implementations. Shared ownership covers presentation blocks, domain/query services, portable schemas and URL-state codecs, state machines, action/entry policy and realtime/live lifecycle. **A thin host projection is exactly two things: a route file that mounts the package component, and a host-config object carrying storefront defaults, authorization/targeting, response envelope, routes and copy.** Nothing else about that capability may exist under `apps/<host>/`: a helper under `apps/<host>/{lib,components}` whose sibling exists on the other host is a violation of this rule, not «app-local» code. Any deliberate behavioural divergence that is not expressible as a host-config field requires an ADR-backed product or technical reason recorded before implementation.

**Consequences.** Doctor's events surface consumes Academy's event-listing core, calendar grids, `WebinarCard`, room live-state and registration/entry lifecycle as shared capabilities. Academy and Doctor may expose different host projections over that core, but neither may own a second query engine, URL-state codec, room or lifecycle. A shared contract may be widened additively for doctor-specific formats only at the canonical owner and remains available to every consumer. Review rejects a visually identical component backed by different app-local behaviour just as it rejects copied JSX. The auth flows are a canonical-core family on the same terms: the auth shell and the login, password-recovery and email-confirmation **compositions** — together with their behaviour (held-password replay, registration resume/handoff, return-to-origin, error mapping, bot protection) — are owned by `packages/auth-flow`, while `@ds/design-system` keeps the **cards**, i.e. presentation only. Each storefront mounts the package supplying a host-config object and nothing else.
...
**Open follow-up.** A capability both storefronts render lives in one feature package; existing duplication is retired by the extraction waves of epic #2020 (tech spec [`specs/tech/2026-09-07-one-code-two-storefronts-plan-en.md`](../specs/tech/2026-09-07-one-code-two-storefronts-plan-en.md) §4). The registry [`specs/product/two-site-ia/capability-ownership.md`](../specs/product/two-site-ia/capability-ownership.md) is the checked-in answer key: its «Target package / wave» column names the owning package and the wave that extracts it, and its «Host-file allowlist» enumerates every file permitted under `apps/{portal,doctor}/{lib,components}` with its reason. A CI allowlist guard (#2002) enforces that file boundary on the tree.
