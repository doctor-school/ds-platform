...
Одна строка на человека, на обеих витринах (OWD-1). Контакты (`email`, `phone`) никогда не входят в выгрузку для инвестора (OWD-2). Основная специальность — **не** колонка в `users`, а связующая таблица ниже.

#### Человек ↔ специальность Минздрава — `doctor_specialties`
...
| `role`                                  | enum      | `doctor_specialty_role` — сегодня `primary`, ось для будущих ролей                   |
| `record_status` / `deleted_at`          | enum / ts | retained-row lifecycle; CHECK держит инвариант «`retired` ⇔ проставлен `deleted_at`» |
| `version` / `created_at` / `updated_at` | int / ts  | стандартные поля жизненного цикла                                                    |

У врача **не более одной активной основной специальности** — это обеспечивает частичный уникальный индекс по `doctor_id` по активным строкам с ролью `primary`, то есть база данных, а не код приложения. Смена специальности — это «снять и вставить» в одной транзакции: прежний выбор сохраняется, а не затирается, поэтому история заявленных специальностей и их дат переживает любое изменение. Именно отдельная таблица связи (вместо колонки `primary_specialty_id`) делает возможными оба свойства и оставляет место для неосновных ролей, заложенных в enum `role`.
...
`id` · `code` · `title` · `is_active`. **Закрытый** справочник — сид по номенклатуре Минздрава, действующей на момент выпуска (с 01.09.2026 — Приказ № 435н), плюс «Другое», пересев при смене приказа; его размер — фактическое число строк, а не литерал; редакционно не расширяется. В профиле врача, на документах и для НМО (OWD-11, REQ-101).
...
| — (новые)                                                   | **новые**                     | `role_assignments`, `memberships`, `account_expert_links`, `specialties_minzdrav`, `doctor_specialties` (§2.1, поставлена миграцией 0027), `direction_adjacency`, `direction_specialties`, таблицы результатов (школа/курс/модуль/урок/подкаст), `ledger_accounts`, `postings`, `document_records` |
