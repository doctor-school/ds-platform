...
`users` remains the single account entity across both storefronts (OWD-1, ADR-0001 §6): one person, one Zitadel subject, one row. A doctor who is also an expert, also a partner employee, also a podcast guest is **one** `users` row.
...
An **organisation** is a first-class entity covering every institutional counterparty: partner/investor (pharma, medical-equipment manufacturers), clinical base (clinic + the institute operating on its base — OWD-4), licensee / educational organisation, employer of an expert. Organisation **kind** is a set of attributes, not a table per kind: one counterparty may combine roles (glossary, V-RV-10), so the kinds are a multi-valued attribute of a single organisation row.

**Membership** is a separate relation `person ↔ organisation` carrying its own role (partner representative, medical representative, organisation admin, …), a validity interval and a revocation state (OWD-8). Multiple simultaneous memberships are allowed. Authority is evaluated **in the context of a specific organisation and a specific project**: a representative sees doctor personalia only inside a permitted project of their own organisation (OWD-2), and when an employee leaves, the **membership** is revoked, never the account.
...
- **Ledger accounts** — one per subject that can hold a balance: per person (attention points, OWD-10/REQ-3), per project unit fund, per organisation (partner/investor), plus the system accounts a double-entry posting needs.
- **Postings** — immutable, double-entry-style entries: each posting moves a quantity from one account to another, carries a unit (attention points / money), a project reference (OWD-9), an occurred-at, and the id of the rule that produced it. Corrections are compensating postings; nothing is updated in place.
...
**REQ-134 (investor terminology fork) is left open deliberately** and the model is built so it costs nothing to settle: the ledger subject is an account attached to a person or an organisation, and the smart-contract role is a **role assignment plus the postings that reference it**. A partner can therefore be the smart-contract investor, an Academy audience member, or both, with **no schema change** — only different role assignments and different postings.
...
Consents are **per purpose, explicit and revocable** (REQ-34) and reuse the existing `consent_records` family (ADR-0009) — one record per purpose and version, with revocation recorded as a new record rather than a deletion. A partner receives data only for the doctors who consented for that purpose (OWD-2, CON-2). What happens to lists already handed to a partner when a consent is revoked is an open product question (§ below), not a model gap.
...
**REQ-105 (migration of the ~10k GetCourse doctors and the congress registration base) is named here as a follow-up, not designed here.** It needs its own artifact covering legal basis, consent re-confirmation and field mapping, and it is a prerequisite of the first partner report — but it is a migration decision, not a core-model decision.
...
**C. Membership as a property of the account (an `organisation_id` column on `users`).** Rejected by OWD-8: it cannot express multiple simultaneous memberships, and revoking access on an employee's departure would mean touching the account. A revocable link is the whole point.
...
Product decisions this ADR deliberately does **not** settle. Each stays with the Product Lead and blocks only the slice that needs it.

1. `TODO(Product Lead)` **REQ-134** — investor terminology fork: is the smart-contract role called "first investor" with the Academy audience called "investor", or another pair? The model is fork-proof (§6); only the labels are open.
2. `TODO(Product Lead)` **Q-33** — who is the subject of an investment/contract (person or organisation) when a person holds several memberships under the exclusivity of REQ-90.
3. `TODO(Product Lead)` **Q-34** — is the medical representative a subject with personal attribution in the ledger, or only an attribution attribute on a posting.
4. `TODO(Product Lead)` **Q-35** — on revocation of a consent, what happens to personalia lists already handed to a partner (OWD-2).
5. `TODO(Product Lead)` **REQ-105** — legal basis and consent re-confirmation for migrating the ~10k GetCourse doctors and the congress registration base; the migration design is a separate artifact.
6. `TODO(Product Lead)` **REQ-36/48/49** — accrual rules, point prices and bonus policy (two-way doors, referenced by id in §6 and out of scope here).
7. `TODO(Product Lead)` **Q-31 residue** — for each kind of training: who is the provider/licensee and who issues the document; this shapes the document-record `issuer` attribute (§7).
8. `TODO(Product Lead)` — whether account ↔ expert confirmation is a **mandatory** precondition of the first payout to an author (accruals accumulate on the expert but cannot be withdrawn until linked). This is a policy rule, not a model change.

---

## Consequences

**Positive.** The one-way doors of the discovery package are closed in a single artifact, so every relaunch feature slice inherits the same entity shapes instead of inventing them. Features 012 and 014 keep shipping unchanged — the model absorbs them rather than superseding them. Roles-as-assignments and membership-as-a-link remove the two most expensive retrofits the register warned about. The ledger makes both the points balance and the partner-facing money chain derivable, and it survives the REQ-134 fork untouched.
