...
The same pick settles row activation: a **single-action list has no «Действия» column — the whole row opens the record**, and an actions column exists only when a row genuinely has ≥2 actions. That is a prop on the block, never a per-app hack: the record cell carries a real link/button so assistive tech gets the semantics and the keyboard a visible focus ring, and its stretched overlay makes the whole row a click target with `cursor-pointer` and a hover cue (ADR-0013 §7). On 2026-08-28 the Product Lead chose **pressed option 1** ([Issue #1578 comment 5450301300](https://github.com/doctor-school/ds-platform/issues/1578#issuecomment-5450301300)): the row/card continues one step past hover to `tint-pressed` (blue.200 light / blue.700 dark), with no extra boundary, layout movement, or focus-ring reuse. Dark pressed context uses `foreground` for AA.

**Row activation deviates from the Link hover contract — no underline on the record title.** The [Link / navigation](#link--navigation) rule is colour + **hover-underline** + focus ring; a data-table row does not follow it. On 2026-08-27, reviewing the rendered block, the Product Lead ruled: «Заголовок в таблице не нужно подчёркивать при наведении - покраски строки и поинтера достаточно, иначе только лишний визуальный шум появляется». The affordance for a clickable row is the **row tint plus `cursor-pointer`**, applied to the whole row rather than to the title alone — an underline on the title repeats an affordance the row already carries and reads as noise across twenty rows. The record title therefore carries **no `hover:underline` in any state**, while the keyboard contract is untouched: the title is still a real link/button and still shows its `focus-visible` ring. The deviation is **scoped to rows inside `DataTable`** — every other link, in-body or standalone, keeps the Link contract unchanged. Pinned by a block test, so a later "consistency" refactor cannot quietly restore the underline.

**Token / primitive mapping.** `Table` (verbatim shadcn markup, re-skinned) + the owned `DataTable` block in `@ds/design-system`, replacing the hand-composed `apps/admin/components/admin-list-shell.tsx` table. Structural chrome uses the flat-brand tokens — `border` / `hairline` at the 2px structural weight, `card` for the surface, `accent` for the header row, `tint` for row hover and `tint-pressed` for live press, `radius-base` (0) throughout; text uses `foreground`, `muted-foreground` for the record's second line and `faint` for metadata; status chips reuse the `success-tint` / `warning-tint` / `destructive-tint` + `*-text` pairs, never a raw hex; numeric cells take tabular numerals. Concrete classes live only in `packages/design-system/README.md`.

**Rendered contract.** Showcase → Blocks → DataTable: the same RU row set in populated, loading, error, both empty states and the actions variant, at both breakpoints and both themes, plus a long-value row proving the declared overflow behaviour and a keyboard pass over row activation. Showcase → Adopted records the completed pressed-state pick as one adopted option (desktop/mobile × light/dark), not three stale candidates.

**Decision & enforcement.** [ADR-0013 §7](../adr/0013-design-token-sot-en.md) layers 1–4; guards `primitives-first` (an app-local `<table>` in `apps/*` is a violation once the block exists), `interaction-states` (row hover + row-activation focus-visible), `aa-contrast`, `playwright-axe` (table semantics: `<th scope="col">`, caption/accessible name, no layout tables). Block tests pin the column contract: declared widths applied (a grid declared entirely in absolute units also gets a table `min-width` equal to their sum and single-line headers, so a table wider than its frame scrolls inside the keyboard-focusable scroll region instead of squeezing columns; the percent contract is unchanged), numeric alignment, the full value reachable on a truncated cell, row activation as a real link/button, the actions column only when actions are supplied, and the state routing.
...
**Token / primitive mapping.** `Tabs`/`TabsTrigger` (`TabsList` `overflow-x-auto`; trigger focus = `focus-visible:shadow-focus-inset`) → `packages/design-system/README.md` → _Clickable state matrix_.
