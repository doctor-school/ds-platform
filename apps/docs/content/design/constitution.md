...
**Unit & states.** One operator list surface: header + primary action → filter bar → column header row → data rows → pagination footer. Its state set is `loading` (skeleton rows, header already drawn) / `error` / `empty — no records at all` / `empty — no results for the current filters` / `populated`, and per row `default` / `hover` / `active` / `focus-visible` / `row action`. Each column declares its own contract — width behaviour, alignment (text left, numeric right), and overflow behaviour (truncate-with-full-title · clamp to two lines) — so a long RU title is a designed outcome, not an accident of the content.
