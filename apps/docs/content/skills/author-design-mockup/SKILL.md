...
5. **Screen-composition Stage A — the owner's LAYOUT pick.** Deliver the rendered mockup as a **visual the owner picks from** (recipe: [`report-task-outcome`](../report-task-outcome/SKILL.md)) — the arrangement/layout, 2–3 options where a composition choice is genuinely open; **never a text questionnaire**. This is a **distinct** decision from the element-class Stage A that `research-ui-element` settles per class: here the owner approves *how the surface is composed*, not *what a button is*. **Record the pick as an artifact** in `NNN-product.md`. A handoff-asserted "approved" is unverified — re-confirm with the live owner.
6. **Hand the approved mockup to delivery.** Record its reference in `NNN-product.md`. Delivery (`do-feature-iteration` + `build-ui-from-design-system`) builds THAT layout from `@ds/design-system`; the element-class cycle handles any per-element work; **Stage B** (live-verify on the running stand) runs at merge.
...
- **Presenting Stage A as text options** instead of a rendered layout — the owner picks from what they can see.
