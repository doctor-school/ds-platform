...
2. **Write `NNN-requirements.md`** (filename prefixed with the spec number per ADR-0006 §4):
   ...
   - Frontmatter: `tracker:` (GitHub Milestone URL placeholder if the milestone isn't created yet), `status: Draft`, **`surface:`** (required — see below). **Quote any frontmatter scalar containing `: `** (e.g. a `prior_decisions` entry citing `access: authenticated, required_roles: …`) — an unquoted colon-space parses as a nested YAML mapping and breaks the docs build only in CI (PR #596 red).
   ...
   - **Cross-host projection boundary line (one code, two storefronts).** A storefront spec (`track:doctor` / `track:academy`) for a capability the OTHER host already runs live does **not** re-specify it as EARS. It writes exactly ONE boundary line per such capability under Scope: «projection of `packages/<x>`; host-config diff: <fields>» — naming the feature package that owns the behaviour and the host-config fields (copy, route, brand, authz envelope) that differ. Everything not named in that diff is, by definition, identical to the other host and is not a spec question. A second EARS list for behaviour that already ships is not "the same contract restated" — it is a fork the day it is written. **Recorded counter-example: 021 (`021-doctor-registration`)**, whose boundary table said the doctor host ships «a second rendition of the same contract»; the post-confirmation sign-in behaviour was silently dropped and the doctor stayed a guest until #1996 re-derived it. The registry `apps/docs/content/specs/product/two-site-ia/capability-ownership.md` (column «Target package / wave», section «Host-file allowlist») is the answer key for which package owns what; a host file that is neither the route file nor a host-config object needs an allowlist row, not a spec paragraph.
...
- A storefront spec that re-specifies as EARS a capability already live on the other host, instead of the single boundary line «projection of `packages/<x>`; host-config diff: …» — return error (021 is the recorded counter-example).
