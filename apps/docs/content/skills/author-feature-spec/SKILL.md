...
Authoring a new feature-spec (the SDD triplet) — the `spec-authoring` kind. A **user-facing product feature with no PRD** enters upstream at `product-discovery` (`do-product-discovery`), which hands off _into_ this skill once the brief + PRD + approved mockup exist (ADR-0014). A pure ADR / design-spec authoring task uses `brainstorming` under §3.4 directly — this skill is for the feature-spec triplet.
...
- **Brainstorming without ADRs loaded** (F-16) — running `brainstorming` before `read-relevant-adrs`, re-deriving an answer already fixed in an ADR.
- **Chaining into a plan-writing skill** after the brainstorm — the triplet is the plan; the chain is disallowed (AGENTS.md §3.4).
