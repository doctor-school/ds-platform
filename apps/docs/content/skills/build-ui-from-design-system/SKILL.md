...
- **Stage B — before live-verify / merge (around step 8).** Build the picked option, apply our tokens + brand, then **render it and capture a screenshot** (Playwright over a throwaway preview route or the live stand) and **DELIVER it to the product owner** — copy to a user-visible folder outside git and open it (the visual-delivery recipe in [`report-task-outcome`](../report-task-outcome/SKILL.md)). Never ask «подтверждаешь?» while the screenshot is visible only inside your own CLI (`Read` images do not render to the user). Iterate until "ok". Only then run the live-verify journeys and proceed to review/merge. Storybook is deferred (spec §3.2), so the preview artifact is a Playwright screenshot, not a story.
...
- **Presenting a look/colour pick as TEXT options instead of a rendered, delivered visual** (Stage A), or asking for a Stage-B sign-off on a screenshot visible only in the agent's CLI — the owner picks/approves from what they can see, never from a text questionnaire. Skipping the brand-SoT read (`primitive.json` Pantone map) before generating options is the upstream cause of wasted option rounds.
