...
5. **Sweep cross-references** — grep the rest of the repo for citations of the changed text (`grep -r "<old-rule-fragment>" apps/docs/content/` and around `AGENTS.md`, `CLAUDE.md`, skills, CI workflows, lint TS files). Update each citation to point at the new rule formulation; if the citation referenced content you deleted, delete the citation or rewrite the surrounding prose to not need it.
