...
1. **Read the debt item** and classify its nature:
   ...
   - **New ADR** — decision is large enough to warrant its own immutable record → route to `brainstorming` (sole allowed exception per AGENTS.md §3.4) for the spec, then to `do-adr-revision` for the implementation if it revises an existing ADR, or to a fresh ADR-NNNN authoring task.
