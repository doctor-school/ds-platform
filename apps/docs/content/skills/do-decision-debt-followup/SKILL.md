...
1. **Read the debt item** and classify its nature:
   - **ADR-amendment** — silent deviation from an existing ADR → route to `do-adr-amendment`.
   - **New ADR** — decision is large enough to warrant its own immutable record → route to `superpowers:brainstorming` (sole allowed exception per AGENTS.md §3.4) for the spec, then to `do-adr-amendment` for the implementation if it amends an existing ADR, or to a fresh ADR-NNNN authoring task.
   - **Feature-spec correction** — a `requirements.md` / `design.md` is incomplete or wrong → inline correction PR (no orchestration skill needed for small corrections; treat as a doc PR similar to `do-hotfix-pr` without the test).
...
- [../do-adr-amendment/SKILL.md](../do-adr-amendment/SKILL.md)
