...
- **Open extraction epic precedence** — while an extraction epic (today #2020, milestone «Платформа — один код на два фронта») is open, a `track:doctor` / `track:academy` product Issue whose capability the other host already ships (a registry row with a «Target package / wave» value) is NOT takeable: wire `blocked_by` on the wave Issue with the rationale and leave it out of the wave. The epic's sub-issues rank first in `## Takeable` regardless of milestone date.
