...
**Preferred single entry point for the full closeout tail — `pnpm pr:land <N>` (#1026).** When the whole post-merge tail should run too (the AGENTS.md §6 "PR lifecycle runs to completion" steps), run `pnpm pr:land <N>` instead of a bare `merge:when-green`: it chains merge gate → `gh pr merge <N> --squash --delete-branch` → board Status = Done for each linked `Closes #N` Issue (`set-board-status.mjs`) → `worktree:teardown <N>` iff `.claude/worktrees/<N>` exists (candidates: linked Issue numbers + the branch's `<prefix>/<N>-<slug>` number) → re-sweep (`gh pr list` + `git ls-remote --heads origin`, one compact summary line). Each stage is its own statement — never a pipe — and the first non-zero stage aborts the tail with the stage named + a one-line remedy and a non-zero exit. Gate semantics are unchanged: stage 1 **is** `merge:gate` (same barrier as `merge:when-green`, #928), `--mode-a-exempt "<reason>"` is forwarded to the gate and loudly printed, and it refuses a worktree cwd (exit `4`) — a lead-side, MAIN-tree command. A stage that fails mid-tail (e.g. board mutation after a landed merge) is finished by hand per its printed remedy — the merge itself is never retried past a non-green gate.
