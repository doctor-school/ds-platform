...
**Step 1 — confirm CI green via the deterministic merge gate (Phase-0 manual gate).** The repo is public, so the branch-protection API is reachable — but the rule is not applied yet (its shape must not deadlock the Changesets bot branch; owned by Issue #1403), so in Phase 0 there is still **no server-side required-checks gate**: `--auto` does **not** hold the merge for CI — it merges the instant an approval exists, even while checks are still pending (this is what merged L1 #278 ahead of a pending CI). So the real gate is manual, and its **single sanctioned mechanical form** is the merge gate (#836):
