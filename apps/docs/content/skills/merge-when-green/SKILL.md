...
**The gate also enforces the Mode-a verdict, not just CI (#992).** Before the CI poll it fetches the PR's reviews and requires a **head-SHA-pinned Mode (a) APPROVE**: the latest review whose body opens `## Mode (a) Review` and carries a `VERDICT:` line must be `APPROVE`, **and** its native `commit_id` (the head the reviewer submitted against) must equal the current head SHA. A missing verdict, a latest `REQUEST_CHANGES`, or an APPROVE recorded against a superseded head (a rework invalidates the verdict) is RED (exit `1`) with a re-dispatch instruction — the "positive verdict" input at the top of this skill is now checked mechanically, not on trust. For the sanctioned no-Mode-a classes (AGENTS.md §3.8: pure docs / test-only / generated-regen; the Version-Packages bot PR) pass the explicit escape flag `--mode-a-exempt "<reason>"` — it skips the verdict check and prints the reason loudly as the audit record (no silent auto-detection); `pnpm merge:when-green <N> --mode-a-exempt "<reason>"` forwards it.
