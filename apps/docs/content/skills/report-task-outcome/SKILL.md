...
1. **✅ #N — title · status** (merged / closed / blocked). **Name the exact landing state — never bare «отгружено».** Production is live and separate (AGENTS.md §1): say «смёржено в `main` (в прод НЕ выкачено)» vs «выкачено в прод», so the owner never has to ask "куда — в прод?" (2026-07-12, that ambiguity forced a clarifying round-trip). Deploy is separate from merge: `run-prod-deploy` owns standing authorization, readiness checks and escalation classes; do not re-request already-granted authorization.
