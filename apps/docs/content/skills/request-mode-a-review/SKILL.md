...
6. **Review necessity before technical compliance:**
   - **Scope and sufficiency.** Does the diff achieve the owner’s requested result? Identify unjustified scope, repeated work, speculative abstractions and unnecessary process. A plan/spec/reviewer suggestion is not independent proof of necessity. Classify blockers by unmet acceptance, a concrete material risk or an applicable mandatory gate; suggestions/nits never automatically become prerequisites.
   ...
   - **Deferred prod action.** A production-side effect left to a manual step is a `[BLOCKER]` unless (a) the PR links an Issue naming the action, affected change, timing and completion evidence, or (b) the pipeline performs it, cited by `file:line`. For a prerequisite required BEFORE shipping, also require a PR-body `Release-requires: #N` line (multiple: `#N, #M`; absent/`none` only when no prerequisites exist). Check the dependency's technical rationale and that it is not auto-closed by this PR before the action is done. Rollout actions/post-release acceptance require a linked execution/check plan, never a circular prerequisite of their own activation. `release-blocker` is reserved for evidence of a hazard to ANY deployment, including unrelated hotfixes, with an exit condition and rationale why narrower controls are insufficient. Reassess/remove obsolete global labels without falsely closing remaining work. Routing remains `surface-decision-debt` → tracked Issue with one `source:*`, never only `DEBT.md`: #1396 left prod convergence untracked and prod diverged for 18 days (#1994). Missing tracking or an omitted real prerequisite remains a `[BLOCKER]`; deferred work alone does not justify a global stop.
