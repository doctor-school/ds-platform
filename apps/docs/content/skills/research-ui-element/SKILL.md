...
3. **Registry search (committable whitelist).** Search each and record the result — ① official **shadcn/ui** (Radix) · ② **Intent UI / JollyUI** (React-Aria) · ③ **Kibo UI**. Note license (must be MIT/permissive — ADR-0013 §5), RSC boundaries, a11y, dependency weight, maintenance freshness. If none fit, state the negative result explicitly (bespoke is then justified). **Licence claims carry the fetched artifact (#1587):** any statement about a registry's licence or relicensing includes the upstream LICENSE/LICENSING **URL + the verbatim clause + fetch date**; the lead never edits the whitelist / ADR-0013 / AGENTS.md on a summary line without that evidence (the #1582 round-1 blocker: a blanket-AGPL claim was wrong — upstream kept `apps/origin/`+`apps/ui/` MIT).
...
**Rendered options + owner pick.** <2-3 options, each tagged with any lead-stated acceptance criterion it FAILS (the lead pre-filters on these tags — an option failing an owner criterion never reaches the owner, #1587); owner pick left blank for the lead to fill after Stage A>
