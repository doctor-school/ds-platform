...
**When to run this (the D+B trigger policy — release-cycle spec §10).** The agent ships when a **releasable unit** (a vertical slice / feature-spec iteration / milestone) reaches Done, is Stage-B GO, and passes the release-readiness checklist (spec §10.4) over the whole `deployedSha..origin/main` range. For **standing-auth** change-classes (spec §10.3 — app/UI-only or additive expand-only-migration waves) the agent ships **autonomously**; for **escalate** classes (any contracting/destructive migration, auth/payment/PII flows, breaking API, infra/data-prod, or an un-certified range) it verifies scoped owner approval and asks only for any uncovered action/risks/conditions after authorized preparation. Default to **escalate** when the class is uncertain; reuse approval that still covers it. The bootstrap `## Project reality` merged-not-deployed delta is the detection signal, not a passive cue. There is **no human "operator"** — the agent drives; the owner is the circuit-breaker.

Typing **`/deploy`** ([`.claude/commands/deploy.md`](../../../../../.claude/commands/deploy.md)) means: read this skill and execute it.

**Proportionate preparation:** apply portable discipline before implementation or expanding the plan: record the minimum release path, actual risks, existing rollback/recovery evidence and estimated preparation time/compute cost. Name the uncovered risk and acceptance condition for each extra check or automation; explain why existing commands/evidence are insufficient. An accepted plan, including a rehearsal, does not establish proportionality. If preparation materially exceeds the change or the need is uncertain, present concise alternatives, cost and recommendation to the owner before expanding, without re-asking for unchanged authorized routine work. Reassess when scope grows. A reviewed, reproducible one-time migration runbook is valid; the no-workarounds rule does not require a general migration platform. Verify the applicable recovery path with bounded evidence: app rollback and major-database recovery have different guarantees.

## Release-readiness checklist (spec §10.4) — run BEFORE deciding to ship

The D-decision gate covers the **whole deploy range** (`deployedSha..origin/main`): a failing item holds deployment, not useful authorized diagnosis/preparation. Answer status questions briefly and continue the requested release unless canceled/replaced. If a gate needs an owner decision, prepare its evidence, options and exact request; explicitly wait only when useful authorized work is exhausted. Never turn a status answer into abandonment or bypass a failed gate. The change-class (standing-auth vs escalate) is judged per **spec §10.3** (the taxonomy lives there — default-escalate on any uncertainty); the bootstrap delta line pre-computes a derivable-signal class verdict, which is the **starting point**, not the judgment (Stage-B, PII-flow, and breaking-API calls are yours to make here).
...
6. **Clean deploy environment** — clean working tree + `git pull --ff-only origin main` (the pre-flight enforces this; verify it _before_ deciding to ship). "Clean" includes **untracked** files: a stray scratch file in the checkout fails the pre-flight exactly like a modified one, so `git status --porcelain` must be empty — `git stash -u` before the deploy and `git stash pop` after.
...
8. **Release gate clear (spec §10.10)** — no OPEN global `release-blocker` Issue (`gh issue list --label release-blocker --state open`), no OPEN prerequisite named by `Release-requires: #N, #M` in a selected PR body, and no selected PR's OPEN batched Stage-B gate (item 2's marker, read from PR body OR linked `Closes #N` Issue comments). The pre-flight enforces all three, fail-closed. PR prerequisites apply only to the selected `<live deployed>..<target>` range, including hotfixes; absent/`none` means no prerequisites. Track rollout actions/postchecks with timing and an execution/check plan, never circular preconditions of their own activation.

   When a release is held, name the target SHA/range, exact failed gate and evidence. Reassess every global label: it requires an evidenced hazard to ANY deployment, why narrower controls are insufficient, and an exit condition. Remove obsolete labels with an evidence comment, keeping remaining work OPEN; this is correcting applicability, not bypassing a valid hold. Deferred work, incomplete post-release checks or an unexplained CI flake alone do not qualify. Use the pinned-release/exemption path only with its required authorization. A failed test-email wait is not evidence of production mail failure.

   **The pre-flight reads Issue STATE, not its content.** A batched Stage-B gate whose owner GO is already recorded in a comment still holds the deploy while the Issue is OPEN — `tools/deploy/release-gate.mjs` never parses the GO. So when the gate has its GO **and** the owner has given the release go, **close the gate Issue first, then deploy**; closing it is the completion of the gate, not paperwork deferred to after the ship. `--release-gate-exempt "<reason>"` is for a gate that genuinely cannot be closed yet (it still tracks open work), never a shortcut around closing a satisfied one — and it is a bypass only on the **owner's explicit go**, never a lead call (release-cycle spec §10.10).
...
10. **IdP provisioning drift — pipeline-owned (#1997), verify the step's output.** `pnpm deploy:prod` now runs `infra/dev-stand/idp/provision.sh` on api-prod on **every** deploy (after `up -d`, before the truthful-success verify) and reads the password-complexity policy back from the instance, comparing it against `@ds/schemas` `PASSWORD_MIN_LENGTH` at the deployed SHA — a failed converge or a rejected read-back FAILS the deploy. So this is no longer a manual release-tail item; it is a **verification** item: check the step's output in the transcript (`password complexity policy: converged … | already …` per converge step, then `prod IdP password policy: minLength=… hasUppercase=false …`). `git diff --name-only <deployedSha>..origin/main -- infra/dev-stand/idp/provision.sh` non-empty only tells you to expect «converged» rather than «already» lines. Provisioning is **not** rolled back (converge steps are forward-only); the manual command (`infra/deploy/README.md` → step 9) stays the recovery path. Precedent #1994: the 003 EARS-36 length-only policy shipped in code, the prod instance kept the provider default, registration 422'd for 18 days until the re-run on 2026-09-07 — that gap is what the pipeline step closes.

Record three separate facts before execution: authorization (policy or owner quote/source + scope/conditions), readiness (each required check's evidence), and execution (not started / running / verified result). Standing-auth plus a green checklist proceeds. For an escalation, request approval only for the concrete action or conditions not already covered; a changed scope needs a new applicability decision. A preparation-only go never becomes a cutover go. After execution, attach the actual release/deployment and health evidence.
...
- **Or track the fix as a prerequisite of the offending PR** when a revert is disproportionate: add `Release-requires: #N` to its body and record why. Only releases including that PR wait. Use global `release-blocker` solely for evidenced risks to ANY deployment, recording why narrower controls cannot protect it and an exit condition. Remove the label when that universal risk ends; do not falsely close remaining work.

Never leave a known-broken PR on `main` without a revert or an applicable tracked release hold: the next standing-auth deploy must not ship it silently.
...
3. **Green CI for that SHA** — the pipeline queries the latest check-run per name via `gh api …/commits/<sha>/check-runs` and refuses on red/pending. Escape hatch `--skip-ci-check` logs a loud warning and requires explicit approval for that exception; an earlier ordinary release go does not cover newly failed CI.
...
5. **Release gate clear** — the pipeline checks global `release-blocker`, selected PR-body `Release-requires` prerequisites and batched Stage-B (checklist item 8, `tools/deploy/release-gate.mjs`, spec §10.10). The delta is anchored on LIVE `/v1/health`; a degraded Deployment-only basis, unreadable evidence or malformed prerequisite is UNKNOWN and holds. Escape hatch `--release-gate-exempt "<reason>"` requires explicit owner approval; its mandatory non-empty reason is printed as the audit record.
...
The pipeline is fail-closed and stops at the first red step, printing a rollback pointer: pre-flight → ship (`git archive` over SSH, no registry) → data-prod `up -d --build` → **pgbackrest pre-migrate `incr` checkpoint** → api-prod `build` (one `docker compose build <service>` per service, in compose order — at most one `next build` beside live production on the swapless 8 GB box, after a single all-at-once build stalled the site for ~17 min on 2026-09-18, #2283; images SHA-tagged `ds-api:<sha>` / `ds-portal:<sha>` / `ds-admin:<sha>` / `ds-doctor:<sha>`) → **pre-swap boot verify (#1410 — the freshly built `ds-portal`/`ds-admin`/`ds-doctor` images are each booted as a throwaway container with production's `env_file` and must answer non-5xx on `/`; a non-booting image aborts the deploy with the OLD containers still serving and the DB not yet migrated, closing the #1407 gap)** → `migrate → up -d` → compare the shipped `Caddyfile` and `centrifugo/config.json` with the files visible through the running containers, restart only consumers whose single-file bind mount is stale, then verify both mounted configs again → **IdP provision converge + policy read-back (#1997 — `infra/dev-stand/idp/provision.sh` is run on api-prod with the README step 9 env, idempotently, and the password-complexity policy is then read back from the instance on-box and compared against `@ds/schemas` `PASSWORD_MIN_LENGTH` at the deployed SHA; a failed converge or a rejected read-back fails the deploy)** → **truthful-success verify** (polls on-box until the running containers carry exactly `ds-*:<sha>` and are healthy — otherwise FAILED, never "OK") → image retention (last 3 tags/repo) → prod smoke (`--expect-sha`).
...
- **Release-milestone close-out** (#1729) — after the tag is cut, check the track-release milestones the deployed range touched. When the range ships the LAST open feature of a milestone (its gate Issue's `blocked_by` graph is drained), close the gate Issue and close the milestone: `gh api -X PATCH repos/doctor-school/ds-platform/milestones/<n> -f state=closed`. A milestone that still has open features stays OPEN — never close it early to tidy the board, and never close a gate whose `blocked_by` edges are still open. Roadmap dates are not touched here: `pnpm roadmap:forecast` owns the «Target date» field.
...
git cherry-pick -x <FIX>            # repeat for each already-merged fix, oldest first
...
`## Project reality` (bootstrap `pnpm bootstrap`, #939) derives the latest release, the deployed SHA (GitHub Deployment ⋈ `/v1/health`), and the **merged-not-deployed delta** at SessionStart. A non-empty delta renders the explicit **D-trigger verdict** (spec §10.2/§10.3): the line carries `class standing-auth (…)` or `class escalate (…)` derived from the range's touch-set (`classifyDeployRange` in `tools/project-reality.ts` — migration / backend / infra / workflow / deploy-tooling touches escalate; uncomputable defaults to escalate). Treat it as the detection signal: run the §10.4 checklist (above) over the range and, if a releasable unit is ready, ship it under valid standing/scoped approval; ask only for an uncovered decision. A requested release remains unfinished until deployment and required verification are evidenced. If a real gate prevents progress, record the remaining delta and exact decision/event awaited; a merge, handoff or status report is not release completion.
...
- **Pre-swap boot verify red (`freshly built image(s) do NOT boot: portal=EXITED`)** — the new `ds-portal`/`ds-admin`/`ds-doctor` image crash-loops or never serves (`next` 16.3.1 / #1407 shape). **Prod is untouched**: nothing was swapped, no migration was applied, the previous containers are still serving — this is a build defect to fix on `main`, not an outage to roll back. Read the printed `docker logs --tail 60` for the crashing service, fix the cause (usually a dependency bump), land it, re-run `pnpm deploy:prod`. The same failure is caught pre-merge by the `standalone-boot` CI job.
- **Smoke RED only on the doctor host after a green step (#2050)** — the deploy's smoke reports the doctor storefront `/` and the TLS handshake on `new.doctor.school` as red while `academy.`/`api.` stay green, and the #1175 Caddy step reported success. Cause: the running `caddy` container holds the **old Caddyfile inode** — the file on the box was replaced (a new inode), so the bind mount inside the running container still points at the pre-change content and no doctor-host site block is served. It is not a build defect, not a DNS/cert problem, and not a reason to roll back. Recovery: on api-prod, in the compose dir — `cd ~/ds-platform/infra/deploy/compose/api-prod && sudo docker compose restart caddy` — then re-run `pnpm deploy:prod` (idempotent) and confirm the smoke is green on all hosts.
