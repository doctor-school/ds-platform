...
8. **Release gate clear (spec §10.10)** — no OPEN Issue carries `release-blocker` (`gh issue list --label release-blocker --state open`), and no merged-undeployed PR in the range defers Stage-B to a still-OPEN batched gate (item 2's `Stage-B: batched at #<gate>` markers — read from the PR body OR a comment on a linked `Closes #N` Issue, the same accepted-source set the merge guard enforces → `gh issue view <gate> --json state`). The deploy pre-flight enforces both as a hard, fail-closed gate.

   When a release is held, name the target SHA/range, the exact failed gate and its evidence, and whether the reason is a technical dependency, a global release policy, or an unknown condition. An open Issue or release-blocker label proves a policy hold, not that its code is included in or required by this release. Inspect applicability before handing the problem to the owner; use the documented pinned-release/exemption path only with its required authorization. A failed test-email wait is not evidence of production mail failure.
...
5. **Release gate clear** — the pipeline refuses while any OPEN Issue carries `release-blocker`, or any merged-but-not-yet-deployed PR defers its Stage-B to a still-OPEN batched gate — the marker read from the PR body OR a linked `Closes #N` Issue comment (`tools/deploy/release-gate.mjs`, spec §10.10). The delta is anchored on the LIVE `/v1/health` SHA, falling back to the Deployment record. Fail-closed on an UNKNOWN (a `gh` failure, a degraded basis because the health probe failed, or no deployed SHA to anchor the delta). Escape hatch `--release-gate-exempt "<reason>"` — a non-empty reason is mandatory and the printed line is the audit record.
