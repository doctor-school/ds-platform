...
**Entry: the owner types `/wrap` — never the agent (#1746).** The lead does not start this procedure on its own: not at a wave's end, not «before a long gap», not in answer to a handoff request (that request is skill `handoff-prompt` alone — a prompt, no retro, no instruction edits). The active adapter checks owner entry where supported; inspecting this file for an explicitly requested audit is not executing wrap. Never infer trust or enforcement from configuration alone.
...
> **Codex memory prerequisite:** A direct owner request is required even after general wrap approval; no existing memory file is edited. If unavailable, retain proposed memory changes in the report and complete independently authorized repo work.
...
Present the concrete edit set and obtain explicit approval for any subset not already approved in this session; do not re-request approval for the exact authorized changes. A Codex memory mutation additionally requires a **direct owner request** to update memory, not merely `/wrap`. This gate is mandatory (#252 design note) — nothing in stage 3 lands without an explicit go-ahead. If the user trims or rejects items, carry only the approved subset forward.

### 3. Apply + compact — never just append (budget-enforced)

On approval:

1. **Apply** the approved edits. When the bootstrap flags live parallel sessions, the wrap's own repo-tracked instruction/memory edits are **worktree-first from the first edit** (never edit-on-main-then-relocate); personal memory follows the active harness policy: Codex writes only an owner-requested `<timestamp>-<slug>.md` note in `C:/Users/sidor/.codex/memories/extensions/ad_hoc/notes/`, never edits existing memory files; Claude uses its overlay policy.
2. **Compact, don't grow.** Every startup file (including the mandatory shared `agent-discipline.md`, the active harness root files, and the local Claude MEMORY.md index when present) has a hard budget (200 lines / 25 KB — `tools/lint/instruction-budget-lint.ts`, #250). When an edit adds signal, relocate equal-or-greater detail **out** of the always-on core: long detail → `.claude/rules/*.md` (add `paths:` frontmatter to make it lazy/file-scoped) or a skill; a settled fact → the active harness's authorized memory route (Codex: extension note only; no direct MEMORY.md/topic edits). Appending without relocating is a banned outcome.
...
1. **Merge outstanding approved work** — any PR with a positive Mode (a)/(b) verdict + green CI and its applicable recorded Stage-B approval merges autonomously ([`merge-when-green`](../merge-when-green/SKILL.md)); do not leave it "waiting for the human" (Theme A).
...
Run the catalog [`handoff-prompt`](../handoff-prompt/SKILL.md) skill to emit the copy-pasteable next-session prompt. It is harness-neutral and owns the ≤300-token fixed shape and the resume-side ref check. Do not hand-roll or substitute a vendor-global skill.

The handoff is the wrap's final chat message — one fenced block, nothing after it. It cites only document paths that exist at emit time (stat/`Read` each before including) and carries the canonical tracker id (GitHub Issue / Plane item) of the next task; «where we stopped» premises come from tracker comments, not the session's memory of itself. Resume side: the fresh session verifies each named Issue / PR / SHA with `gh` before its first tracker or git action, and an agent that cannot locate a cited document asks the owner instead of substituting its own reading.

**The handoff's queue is chunked into ONE wave per session** — ≤3 sequential PR-cycles, or ≤5 independent parallel Issues when their touch sets do not overlap (agent-discipline → Dispatch, models and context) — («this session: items 1–3; next: 4–6») — never one flat ranked list: the wave cap (memory `feedback_wave_plan_by_touch_set`) must fire at handoff-authoring time, not after the next session has already over-dispatched (2026-07-11 retro: a flat 6-item queue produced 5 dispatches in 5 minutes and 6 PR-cycles in one session). The dispatch-time counterpart is item 0 of memory `feedback_orchestration_brief_full_lint_before_pr`.
