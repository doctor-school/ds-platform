...
2. **For each item, classify it against the significance threshold (AGENTS.md §6)** — a tracker Issue ONLY when the debt (a) blocks / sits on the critical path of a product deliverable, (b) is user-visible or a prod risk (security/data), or (c) must be acted on before the next release — then route:
   ...
   - **Deferred prod action → ALWAYS an Issue labelled `release-blocker`** (plus `decision-debt`, exactly one `source:*`, one kind and one `track:*` label), naming the exact manual step and the surface it leaves diverged. This class has no below-threshold route: a `DEBT.md` line is invisible to `tools/deploy/release-gate.mjs`, which refuses `pnpm deploy:prod` only while an OPEN `release-blocker` Issue exists. Precedent: #1396 deferred the prod Zitadel converge with neither Issue nor DEBT line and prod stayed diverged for 18 days (#1994).
