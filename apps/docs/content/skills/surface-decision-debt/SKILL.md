...
1. **Explicit reflection.** Re-read your own session transcript (the current conversation) for moments where you took a decision that:
   ...
   - **Deferred a production-side effect of the diff to a later manual step** — the PR body, spec, design or a code comment says the prod half happens later («at the next deploy», «run by the lead», «applied to prod the same way», an ops re-run, an env/secret change, a provider-console step) while the diff itself ships only the repo-side half.
...
2. **For each item, classify it against the significance threshold (AGENTS.md §6)** — a tracker Issue ONLY when the debt (a) blocks / sits on the critical path of a product deliverable, (b) is user-visible or a prod risk (security/data), or (c) must be acted on before the next release — then route:
   ...
   - **Deferred prod action → ALWAYS a scoped Issue** (plus `decision-debt`, exactly one `source:*`, one kind and one `track:*` label), naming action, affected change, timing and completion evidence. A prerequisite needed BEFORE shipping is linked in that PR body as `Release-requires: #N` (comma-separated references allowed; absent/`none` means no prerequisites). The deploy gate checks it only when the selected range includes that PR. Rollout actions/post-release acceptance stay linked with an execution/check plan, never circular preconditions of their own activation. Use `release-blocker` ONLY for evidenced hazards to ANY deploy, recording why narrower controls cannot protect it and when the hold ends; remove the label with evidence when that risk ends, keeping unfinished Issues open. Tracking remains mandatory (not a DEBT line): #1396's untracked prod converge diverged for 18 days (#1994).
