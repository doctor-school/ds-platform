...
description: "Design: BFF over Zitadel Session/User v2 API; native-vs-custom boundary; registration + consent + mirror cascade; password / email-OTP / SMS-OTP login; __Host- cookie + server-side refresh rotation; password reset; security baseline; seams (MFA, legacy, mobile, social, step-up); own-mailer returnCode code delivery with mail.ru→Resend failover; decision-debt for ADR-0001."
...
| Password reset / forgot        | ✅ User v2 reset code (`returnCode`)             | enumeration-safe wrapper + code-only BFF-mailer delivery (§14)                      |
| Email/phone verification       | ✅ verification code (`returnCode` for email)    | mirror flag sync + code-only BFF-mailer delivery (§14)                              |
...
**Auto-verify email on a proven password reset — proof-of-mailbox-ownership (GH #1131, EARS-35).** A completed reset already proves the subject controls the mailbox: the reset code was delivered to that email (§14, EARS-11/29) and the caller returned it. So on the **same** successful `completePasswordReset` — after the code+password succeed and **only** then (no mutation before a valid token, OWASP) — the BFF marks the account's email **verified** at the IdP (Zitadel `SetUser`/verify-email over the resolved `sub`), which the EARS-19 webhook + the read-path self-heal (EARS-26) mirror onto `UserMirror.email_verified`; the flip emits the terminal `auth.account.verified` (channel `email`) audit row (EARS-3/18) and is **idempotent** — an already-verified account is a no-op flip with no duplicate row. This closes the **stuck-unverified** trap through the existing recovery path: an owner who never verified at registration but proves mailbox ownership via reset is no longer barred from the verified-only login-code path (EARS-6/EARS-34), so A and B compose — the EARS-34 mail gets an unverified user to a proven state, and B lets the reset flow itself resolve it. A bad/expired code or unknown identifier flips nothing and stays the generic enumeration-safe 400 (EARS-16). The `FakeIdpClient` mirrors the verify-on-complete invariant so a regression fails in unit tests, not only live. (Owner decision A+B, #1131 comment 2026-07-23.)
...
**Login-by-email-code for an unverified account — out-of-band verification, never a silent dead-end (GH #1131, EARS-34).** `RequestEmailOtp(identifier)` (EARS-6) resolves the identifier → `sub` through the same enumeration-safe wrapper as `requestPasswordReset`/`resendEmailVerification` and returns the uniform ack on every path. Zitadel arms the `otp_email` **login** challenge only for a **verified** email — for an existing but **email-unverified** account the historic behavior was a silent no-op: the user asked for a login code that, by IdP design, is never sent, and the `/login` screen sat waiting for a code that never arrived (the login-side twin of the #207 register dead-end). The fix routes the unverified branch to **recovery, delivered privately**: instead of arming the login challenge, the BFF re-issues the **email-verification** code (the same `returnCode`-echoed `resendEmailVerification` hop the EARS-25 resend uses) and dispatches the branded, code-only verify-to-sign-in mail out-of-band via the §14 mailer — **fire-and-forget off the response path** exactly like the EARS-23 account-exists notice, so mailer latency and outages never differentiate the response. The three server-side branches — verified (arm `otp_email`), existing-unverified (send verification mail), nonexistent (no-op) — return the byte-identical status + body + timing (EARS-16, ≤ 50 ms), so neither the response nor the `/login` UI is an existence/verification oracle; the account owner's correct next step lives **only in the mailbox**. An `otp.sent` ledger row (EARS-18) is appended only when a verification code is actually issued; the branch writes no `users`/consent row, is subject to the EARS-13 rate limit + EARS-17 bot-protection surface, and makes **no Zitadel-side behavior change** (option C — a Zitadel Action / policy change to auto-send on the challenge — was rejected, #1131). Once verified (by this mail or by EARS-35), the account's next `RequestEmailOtp` arms the login challenge normally. The `FakeIdpClient`/`FakeMailer` are no more permissive than the real adapters on the verified-vs-unverified distinction. (Owner decision A+B, #1131 comment 2026-07-23; research note in #1131.)

**The verification email is code-only and BFF-sent — no CTA, no link, no Zitadel send (GH #869 → #910 → #1128, EARS-3/25/29).** Both email-verification send hops obtain a `returnCode`-echoed code that Zitadel **generates, stores, expires, and attempt-counts but sends no email for** — the BFF composes the **branded, Russian, code-only** mail (full artifact spec: §13.3) and dispatches it over the §14 transport chain (owner decision 2026-07-15, #910). The two hops differ in **where** the code comes from (GH #1128). The EARS-1/3 registration cascade's `requestEmailVerification(sub, email, code?)` delivers the **create-time code** Zitadel already echoed on `CreateUser` (the create body carries the `email.returnCode` oneof, so the response returns the code as `emailCode`); registration hands that code straight to the mailer, so a registrant receives exactly **one** verification code — never a second generation that would silently invalidate the first before it is read. It falls back to the resend regeneration hop (`POST /v2/users/{id}/email/resend` with the `SendEmailVerificationCode` oneof `returnCode`) **only** when a create response echoes no code — the implicit retry seam. The EARS-25 `resendEmailVerification(identifier)` always regenerates through that same resend hop (a deliberate re-issue). Either way Zitadel remains the code authority and sends nothing. The subject leads with the code, the body shows it as one unbroken, enlarged token with an explicit expiry line, and the mail carries **no link or button of any kind** — fully link-free (the Notion/Slack style). Rationale: mail-provider AV scanners (mail.ru's `checklink`) prefetch **every URL in a delivered message with a GET**, so any deep-link carrying the code (or any GET-consumed secret) is burned by a robot before the human ever clicks; and Zitadel's own template renders an **unsuppressible** CTA button into its hosted login-v2 UI — a dead end for portal registrants (the original #869 defect; the empty-`buttonText` fallthrough to the bundled default label was verified live on v4.15.0). Owning the send removes the button problem at the root instead of demoting it to a subordinate label. The registrant **types the code** on the portal `/verify` screen (§8.3), where the #175 auto-login replay signs them in — manual entry **is** the journey, not a degraded path. The former Zitadel-side machinery for this mail type — the `verifyemail` message-text override (`infra/dev-stand/idp/provision.sh` step 8.ter) and the bare-`urlTemplate` hop — is retired with the switch: Zitadel no longer sends this type. The SMS/phone hops keep the bare `{ sendCode: {} }`.

**The password-reset email is code-only and BFF-sent too — it never touches the IdP's template (GH #880 → #910, EARS-11/29).** The forgot-password send (`requestPasswordReset(identifier)`) requests the code with the `PasswordResetRequest` oneof **`returnCode`** and delivers the same branded, Russian, code-only artifact as the verification email (full artifact spec: §13.4) through the BFF mailer over the §14 transport chain. Zitadel's **default** reset CTA is a link to its hosted set-password page on the identity host — a surface the product's users must never see (the portal `/reset` screen owns the whole journey, including the #221 post-reset auto-login) — and any GET-consumed link is the same scanner bait the #869 contract bans; the BFF-composed mail is fully link-free, so neither hazard exists. The security trade this accepts — the reset code transits the BFF from the IdP response — is governed by EARS-30 (in memory for the in-flight send only; never logged, never persisted) and is the owner-decided price of a fully branded, button-free artifact (#910, 2026-07-15). The former Zitadel-side machinery — the `passwordreset` message-text override (`infra/dev-stand/idp/provision.sh` step 8.sexies) and the bare `sendLink.urlTemplate` hop (`ZitadelIdpClient#passwordResetBody`) — is retired with the switch: Zitadel no longer sends this type. The user types the code on `/reset` together with the new password (EARS-12).
...
- **Email-OTP (EARS-6):** `RequestEmailOtp` → Zitadel `otp_email` send; `LoginWithEmailOtp` → verify code in session → §3 exchange. The v1 passwordless email path; **no magic link**. **Unverified-account branch (EARS-34):** when the identifier resolves to an existing but **email-unverified** account, `RequestEmailOtp` does **not** arm the `otp_email` challenge (Zitadel will not send a login code to an unverified email — the historic silent dead-end, GH #1131) — instead the BFF sends the verify-to-sign-in verification code out-of-band via the mailer (§4, EARS-29), and the synchronous ack is the same enumeration-safe response as the verified and nonexistent paths (§10, EARS-16).
...
**IdP-rendered notifications (out of `next-intl`'s reach).** The portal catalog above only governs strings the portal renders. The notification bodies Zitadel itself still renders (login email-OTP, SMS-OTP, the dormant types) come from its message-text templates, so they are localized Zitadel-side, not by `next-intl`: the IdP instance default language and allowed-languages are locked to `ru` (provisioned in `infra/dev-stand/idp/provision.sh`, #177/#181). The **email-verify and password-reset mails are not Zitadel-sent at all** — they ride `returnCode` and the BFF mailer (§4/§14, GH #910), which owns their RU copy directly (§13.3/§13.4), so those types carry no live override. The Zitadel-sent live types are overridden (ru + en) in the same script: **email-OTP (`verifyemailotp`)** with branded code-only copy (#878, §13.2), the dormant types `verifyphone`/`init` with a neutral greeting (#878), and **SMS-OTP (`verifysmsotp`)** with `Doctor.School: код для входа - {{.OTP}}, никому его не сообщайте` (no OTP-jargon, dev-domain, expiry, or WebOTP-autofill line that the bundled default leaked; #226 — the `{{.OTP}}` code variable). The `password_change` notice is not greeting-neutralized but **policy-suppressed** (never sent) via the notification policy's `passwordChange=false` flag (#896/#1061), because the user-facing password-change confirmation belongs to the BFF mailer channel, not Zitadel. Known benign Zitadel quirk: each SMS-OTP send logs 6 `VerifySMSOTP.*` "not found in language ru" warnings (the Title/PreHeader/Subject/Greeting/ButtonText/Footer email-template label fields, which do not exist for the SMS message type in any language's i18n bundle, including `en`). They are not caused by the branding, are not fixable via the message-text API (the SMS type persists only `text`), and are purely cosmetic. Upstream: Zitadel issue [#9636](https://github.com/zitadel/zitadel/issues/9636) — originally blocked the send, since downgraded to a non-blocking warning. Optional log-cleanup tracked in #230.
...
- **Login-by-email-code preserves the no-oracle property (EARS-34).** The three `RequestEmailOtp` branches — verified (arm `otp_email`), existing-unverified (send the verification mail), nonexistent (no-op) — return the identical status + body + timing. The out-of-band verification mail cannot leak as a **timing** oracle because the send is **fire-and-forget off the critical path** (dispatched after the response is computed, mirroring the EARS-23 notice), so SMTP latency, a failover hop, or a total mailer outage (EARS-31) never differentiates the response and never turns into a 500; the only observable difference between an unverified and a nonexistent identifier is that a real mailbox receives a code, which is not visible to the requester. The verify-then-sign-in guidance therefore travels only in the mailbox (OWASP side-channel-free recovery).
...
## 13. End-to-end user journeys (registration & login)

The two core journeys written **through the user's eyes** — screen → inbox → email artifact → back to the screen → signed in. Each step names the EARS handler that owns it; the verification email itself is a **specified artifact** (§13.3), not an incidental byproduct. These journeys are the product-facing contract the live e2e drives (`apps/api/test/auth/zitadel-email-verify.e2e-spec.ts`, `apps/portal/e2e/auth-journeys.e2e.spec.ts`).

### 13.1 Registration: register → inbox → type the code → signed in

1. **`/register` (screen).** The visitor enters email + password (+ the consent checkbox) and submits (EARS-1/2, 19/20). The response is the identical `pending_verification` for a new and an already-registered email (EARS-16), so the next screen never knows which visitor it is showing.
2. **`/verify` (screen).** The existence-agnostic "check your email" view (§8.3, EARS-24): a masked destination (`a•••@p•••.com`), a 6-slot code field, a resend control with a cooldown (EARS-25), and co-equal sign-in/reset actions for an already-registered owner. The entered password is held in the volatile in-memory store for the #175 auto-login replay.
3. **Inbox.** The code-only verification email (§13.3) arrives; the subject alone (`GX5AVU — код подтверждения Doctor.School`) already shows the code in the inbox list/notification preview — many users never need to open the mail.
4. **Back on `/verify`.** The user types the 6 characters exactly as the email shows them — one unbroken token (`GX5AVU`), so what is read is what is typed. The slotted `InputOTP` auto-submits on the last character (#175); the screen confirms «Код принят — входим…» only after the server accepts (EARS-3).
5. **Signed in.** The portal replays the real EARS-5 password login from the held password and lands the user on `/account` signed in (EARS-8 cookie) — or completes the carried event registration (005 EARS-2). With no held password (hard reload, abandoned flow), the fallback is a manual sign-in at `/login`.

Divergences: a wrong/expired code → the same generic error, no oracle (EARS-16); resend → a fresh mail with a fresh code, the previous one invalidated (EARS-25); an already-registered email → the account-exists notice arrives instead of a code (EARS-23) and the on-screen path is the co-equal sign-in affordance.

### 13.2 Login: identifier → password or emailed code → signed in
...
### 13.3 The verification email — specified artifact (EARS-3/25/29, GH #869 → #910)

SSOT: the BFF mailer template (`apps/api/src/mailer/` — the #910 impl slice adds the template module); pinned live by `zitadel-email-verify.e2e-spec.ts`. Composed and sent by the BFF (§4/§14, `returnCode` — Zitadel sends nothing) as an inline-CSS/table mail satisfying the same mail.ru/Yandex markup constraints Zitadel's bundled MJML template satisfied; both the initial EARS-3 send and every EARS-25 resend produce this same artifact.

| Field         | Content (ru)                                                                                                                                                                                                                                                                                                                                                    |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Subject**   | `{{.Code}} — код подтверждения Doctor.School` — the code **leads** (inbox-preview UX), rendered ~40 chars (< 50)                                                                                                                                                                                                                                                |
| **Preheader** | «Введите код на странице подтверждения Doctor.School»                                                                                                                                                                                                                                                                                                           |
| **Greeting**  | «Здравствуйте!»                                                                                                                                                                                                                                                                                                                                                 |
| **Body**      | The code as **one unbroken token** (`GX5AVU`, enlarged/letter-spaced — what the user sees is exactly what they type), the instruction to enter it on the verification page, the explicit expiry line «Код действует 1 час» (the `VERIFY_EMAIL_CODE` generator lifetime, 3600 s), and «Если вы не регистрировались на Doctor.School — проигнорируйте это письмо» |
| **Links**     | **None.** The mail contains zero `<a>` elements — no button, no navigation aid, no footer link (the owner-picked Notion/Slack style, #910)                                                                                                                                                                                                                      |

Invariants: the **code format** is `[A-Z0-9]{6}` (Zitadel `VERIFY_EMAIL_CODE`: 6 chars, uppercase + digits — Zitadel still generates and verifies the code, §14); the mail contains **no URL at all** — stronger than the former bare-link contract — so a mail scanner's prefetch consumes nothing by construction. Copy changes must preserve the stable subject tail the e2e harnesses select by (`NOTIFICATION_SUBJECTS.verifyEmail`, substring match) or update those constants in the same PR.

### 13.4 Password reset: request → inbox → type the code → new password → signed in (EARS-11/12, GH #880)

1. **`/reset` (screen).** The user enters their identifier and requests a reset (EARS-11); the ack is enumeration-safe (EARS-16) and the screen advances to the code + new-password step regardless of existence.
2. **Inbox.** The code-only reset email (below) arrives; as with registration, the subject alone (`GX5AVU — код сброса пароля Doctor.School`) already shows the code in the inbox preview.
3. **Back on `/reset`.** The user types the code and the new password; on success the BFF mints the session directly (§4, GH #221) and lands them on `/account` signed in (EARS-12) — the flow never leaves the portal.

The reset email is a **specified artifact** like §13.3 — SSOT: the BFF mailer template (`apps/api/src/mailer/`, the #910 impl slice), composed and sent by the BFF (§4/§14, `returnCode` — Zitadel sends nothing), same inline-CSS/table markup constraints:

| Field         | Content (ru)                                                                                                                                                                                                                                                                                                                            |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Subject**   | `{{.Code}} — код сброса пароля Doctor.School` — the code **leads** (inbox-preview UX), rendered ~40 chars (< 50)                                                                                                                                                                                                                        |
| **Preheader** | «Введите код на странице сброса пароля Doctor.School»                                                                                                                                                                                                                                                                                   |
| **Greeting**  | «Здравствуйте!»                                                                                                                                                                                                                                                                                                                         |
| **Body**      | The code as **one unbroken token** (enlarged/letter-spaced — what the user sees is exactly what they type), the instruction to enter it on the reset page, the explicit expiry line «Код действует 1 час» (the `PASSWORD_RESET_CODE` generator lifetime, 3600 s), and «Если вы не запрашивали сброс пароля — проигнорируйте это письмо» |
| **Links**     | **None.** The mail contains zero `<a>` elements — no button, no navigation aid, no footer link (#910)                                                                                                                                                                                                                                   |
...
The three affected IdP-port methods — `requestEmailVerification(sub, email, code?)` (EARS-1/3), `resendEmailVerification(identifier)` (EARS-25), and `requestPasswordReset(identifier)` (EARS-11) — obtain a **`returnCode`**-sourced code that Zitadel returns to the BFF while sending nothing. **Zitadel remains the code authority** — generation, storage, expiry (`VERIFY_EMAIL_CODE` / `PASSWORD_RESET_CODE`, 3600 s), attempt-counting, and verification (EARS-3/12) are untouched; the BFF is a transport. The registration hop's code source is the **create-time code** (GH #1128): the `CreateUser` body carries the `email.returnCode` oneof, so Zitadel echoes the code as `emailCode` in the create response, which `createUser` captures onto `CreatedUser.verificationCode` and `register()` passes to `requestEmailVerification(sub, email, code)` — mailed directly with **no** follow-up send hop, so the registrant gets a single code that no second generation invalidates. Only a create response that echoes no code makes the method fall back to the resend hop (`POST /v2/users/{id}/email/resend`, the `SendEmailVerificationCode.returnCode` oneof) that regenerates a fresh code — the same hop EARS-25 always uses, and the same `PasswordResetRequest.returnCode` shape the reset send uses. Each hop hands the resulting code to the `MailerModule`, which composes the §13.3/§13.4 artifact and dispatches it. The enumeration-safe wrappers are unchanged: the identifier-keyed hops still resolve identifier → `sub` without disclosing existence, and the no-op paths (unknown / already-verified) obtain no code and send nothing. The login email-OTP (EARS-6, `verifyemailotp`) and the SMS hops stay Zitadel-sent — deliberately out of this increment (requirements → Scope).

### 14.2 Mailer module (real paths) and flag-gated delivery

The channel already exists (#207/#209 — `reference_flag_gated_delivery_architecture`): `apps/api/src/mailer/` — `mailer.types.ts` (the `Mailer` port), `smtp-mailer.ts` (`SmtpMailer` — **dual transport**: the Mailpit intercept `MAILER_SMTP_*` and the real relay `IDP_SMTP_REAL_*`, selected **per send** from the `email-delivery-real` Unleash flag with the `EMAIL_DELIVERY_MODE` env default as the Unleash-unreachable fallback), `mailer.fake.ts` (`FakeMailer`, create-time invariants mirroring the real adapter), `mailer.module.ts` (wiring; `MAILER_PORTAL_BASE_URL`), `register-notice-throttle.ts` (the EARS-23 consumer). The verify/reset code sends are **new consumers of the same port**: one operator flag flip still moves every BFF send between Mailpit-intercept and the real relay, and the sink↔real semantics of #209 apply unchanged. Unlike the fire-and-forget EARS-23 notice, the code sends are **journey-load-bearing** — hence the failover chain below.

### 14.3 Transport chain — mail.ru primary → Resend failover (owner decision 2026-07-15, #910)

- **mail.ru primary.** Better deliverability to RF mailboxes (the doctor audience is overwhelmingly mail.ru/Yandex), and its dynamic, reputation-based ceiling (observed `451 Ratelimit exceeded`, ~50/day on a fresh box) **grows with real transactional volume** — warm-up, not raw limits, is the key argument. Config stays as-is: the `IDP_SMTP_REAL_*` relay (`noreply@doctor.school`, `smtp.mail.ru:465`; `reference_doctor_school_email_dns`).
- **Resend failover.** Overflow above the rising mail.ru ceiling + outage fallback. DNS is already aligned in the doctor.school zone at Beget (`resend._domainkey` DKIM live, SES bounce MX) — no DNS work owed.
- **Failover detection (EARS-31).** A rate-limit or availability rejection on the active channel — mail.ru `451 Ratelimit exceeded`, Resend `429`, any 4xx/5xx or connection failure — triggers **one channel switch within the same send**; re-trying the rejected box just re-fails, so same-channel retry is forbidden. A send counts as delivered **only on a 2xx provider acceptance**.
- **Fail-closed on the critical path (EARS-31).** The mailer now sits on the registration/reset journey. Both channels failing = the send failed: logged + metric'd with **both** provider response codes, GlitchTip event raised; the enumeration-safe API response is **unchanged** (EARS-16 — a mailer outage is never an oracle, never a 500). User recovery: the EARS-25 resend / re-requesting the reset; ops recovery: the EARS-32 alert.
- **Observability (EARS-32).** Every failover and relay failure emits a structured log + Prometheus counter labelled with the provider and response code, plus a GlitchTip event (engineering-readiness defaults) — the dashboards distinguish "healthy", "mail.ru saturated, Resend carrying", and "channel dead". No silent degradation.

### 14.4 Resend tier & limits (checked 2026-07-16, resend.com/pricing)

Free: 3,000 emails/mo with a **100/day cap** — enough for failover-only duty at current volume, but the daily cap is a real constraint the moment mail.ru saturates. Pro: **$20/mo — 50,000 emails/mo, no daily limit** (overage $0.90/1k); Scale from $90/mo (100k/mo). Plan of record: start Free while Resend is strictly overflow; move to Pro when the ramp begins (the ≥5,000/day target exceeds the Free daily cap by construction). Re-verify pricing at impl time.

### 14.5 Throughput ceiling & ramp (folds #872)

- **Sustained target: ≥ 5,000/day headroom — owner-confirmed 2026-07-16.**
- **Combined-ceiling rationale:** mail.ru's dynamic ceiling starts low (~50/day fresh) and rises with warm-up; Resend Pro carries 50k/mo with **no daily cap** (≈1,600/day sustained average, burstable far above for registration spikes). The chain covers the target even before mail.ru warm-up matures; as the mail.ru ceiling rises, more volume stays on the RF-preferred primary.
- **Warm-up plan:** keep **all** steady-state transactional volume on mail.ru (primary-first is the warm-up mechanism — never pre-split volume away from it); let overflow spill to Resend via the EARS-31 failover; re-derive the observed mail.ru ceiling from the `451` metrics (EARS-32) rather than active probing.
- #872's remaining ACs (both channels through the single swap point, SPF/DKIM/DMARC verified from received headers on mail.ru + Yandex inboxes, documented ceiling, relay-failure observability, `infra/deploy/api.env.example` update, cold prod live-verify) close in the #910 impl slices.

### 14.6 152-ФЗ posture (recorded decision, #872)

Routing through Resend (foreign infra) **as failover** is a deliberate, recorded compliance decision, not silence: these transactional mails carry only the recipient address and a short-lived one-time code — no medical, profile, or other PD payload — and the primary channel remains RF-domiciled (mail.ru). Bulk/marketing channels are out of scope (requirements → Scope). Any future mail type carrying PD beyond the address re-opens this channel decision explicitly.

### 14.7 Security invariants (EARS-30) and design-approval gates
