...
**Verification-email CTA deep-links to the portal `/verify` screen (GH #869, EARS-3).** Both email-verification send hops — the EARS-1/3 cascade's `requestEmailVerification(sub)` and the EARS-25 `resendEmailVerification(identifier)` — carry Zitadel's `SendEmailVerificationCode.urlTemplate` set to `<portalBase>/verify?code={{.Code}}&userId={{.UserID}}`. Without it Zitadel renders its **default** CTA — a link into its hosted login-v2 UI, a dead end for the portal registrant (the owner-reported #869 defect); with it the button lands on the portal's own branded screen with the code prefilled and auto-submitted. The portal origin comes from `MAILER_PORTAL_BASE_URL` (the same recipe-specific source the EARS-23 notice links use — never a hardcoded host). Zitadel's supported template placeholders are `{{.Code}}` / `{{.UserID}}` / `{{.OrgID}}` only (proto `zitadel/user/v2/email.proto`) — there is **no** login-name/email placeholder — so the deep-link identity is the opaque `userId`, and the `/v1/auth/verify` contract is extended **additively**: `{ code }` plus ONE identity, `email` (the classic post-register hop, unchanged) or `userId` (the deep-link; the sub IS the identity — no mirror lookup, the IdP's own code check is the authoritative gate, and a bogus sub fails into the same generic 400 as a bad code, EARS-16). The deep-linked screen stays existence-agnostic (EARS-24): it never probes verification state, the masked-destination copy degrades to the generic fallback (no email is known), and with no held password the success path falls back to `/login` as before. The SMS/phone hops keep the bare `{ sendCode: {} }` — no portal deep-link surface exists for them.
...
- **(a) Enter the email code** — the new registrant's path, unchanged: the slotted `InputOTP` auto-submits on completion and the portal replays the held password for post-verify auto-login (#175/#194). The common case stays slick. The email CTA's deep-link variant (#869) arrives as `/verify?code=<code>&userId=<sub>` and completes the same path hands-free: the code prefills, the submit fires once with the `userId` identity, and — the deep-link never has a held password — success falls back to `/login` for the manual sign-in.
