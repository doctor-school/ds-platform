...
**The verification email is code-only — no CTA into the flow (GH #869, EARS-3/25).** Both email-verification send hops — the EARS-1/3 cascade's `requestEmailVerification(sub)` and the EARS-25 `resendEmailVerification(identifier)` — deliver a **branded, Russian, code-only** mail (full artifact spec: §13.3): the subject leads with the code, the body shows it as one unbroken, enlarged token with an explicit expiry line, and the mail carries **no code-consuming link**. Rationale: mail-provider AV scanners (mail.ru's `checklink`) prefetch **every URL in a delivered message with a GET**, so any deep-link carrying the code (or any GET-consumed secret) is burned by a robot before the human ever clicks — and Zitadel's **default** CTA is a link into its hosted login-v2 UI, a dead end for portal registrants (the original #869 defect). The registrant instead **types the code** on the portal `/verify` screen (§8.3), where the #175 auto-login replay signs them in — manual entry **is** the journey, not a degraded path. Mechanism: the copy is a Zitadel **message-text override** applied at provisioning (`infra/dev-stand/idp/provision.sh` step 8.ter, ru+en — the #226 `verifysmsotp` precedent; custom `text` is injected **unescaped** into Zitadel's own inline-CSS/table MJML template, so `<br>`/`<strong>` render while the mail.ru/Yandex markup constraints stay satisfied by the template itself). Zitadel's button **cannot be suppressed** — an empty custom `buttonText` falls through to the bundled default label and the button row + URL render unconditionally (verified live on v4.15.0) — so the send hops set `SendEmailVerificationCode.urlTemplate` to the **bare** portal `/verify` (no `{{.Code}}`/`{{.UserID}}` placeholders, no query — a scanner GET consumes nothing) and the override demotes the button label to a subordinate navigation aid («Открыть страницу подтверждения»). The portal origin comes from `MAILER_PORTAL_BASE_URL` (the same recipe-specific source the EARS-23 notice links use — never a hardcoded host). The SMS/phone hops keep the bare `{ sendCode: {} }`.

**The password-reset email is code-only too — its CTA never lands on the IdP (GH #880, EARS-11).** The forgot-password send (`requestPasswordReset(identifier)`) delivers the same branded, Russian, code-only artifact as the verification email (full artifact spec: §13.4): the subject leads with the code, the body shows it as one unbroken, enlarged token with an explicit expiry line, and the mail carries **no code-consuming link**. Zitadel's **default** reset CTA is a link to its hosted set-password page on the identity host — a surface the product's users must never see (the portal `/reset` screen owns the whole journey, including the #221 post-reset auto-login) — and any GET-consumed link is the same scanner bait the #869 contract bans. Since the button cannot be suppressed (the #869 empirical finding), the send carries the `PasswordResetRequest` oneof `sendLink.urlTemplate` set to the **bare** portal `/reset` (no `{{.Code}}`/`{{.UserID}}`/`{{.OrgID}}` placeholders, no query — a scanner GET consumes nothing; `apps/api` `ZitadelIdpClient#passwordResetBody`), and **never** `returnCode` — the secret rides the configured notifier, not the HTTP response. The copy is the message-text override in `infra/dev-stand/idp/provision.sh` step 8.sexies (ru + en), demoting the button label to a subordinate navigation aid («Открыть страницу сброса»). The user types the code on `/reset` together with the new password (EARS-12).
...
### 13.3 The verification email — specified artifact (EARS-3/25, GH #869)

SSOT: `infra/dev-stand/idp/provision.sh` step 8.ter (ru + en mirror); pinned live by `zitadel-email-verify.e2e-spec.ts`. Rendered inside Zitadel's bundled inline-CSS/table MJML template (mail.ru/Yandex-safe); both the initial EARS-3 send and every EARS-25 resend produce this same artifact.
...
| **Button**    | «Открыть страницу подтверждения» → the **bare** portal `/verify` (no query) — a subordinate navigation aid, never the flow                                                                                                                                                                                                                                      |

Invariants: the **code format** is `[A-Z0-9]{6}` (Zitadel `VERIFY_EMAIL_CODE`: 6 chars, uppercase + digits); the mail contains **no link into Zitadel's hosted UI** (`…/ui/v2/…`) and **no GET-consumable URL** — every href is exactly `<portal>/verify` with no parameters, so a mail scanner's prefetch consumes nothing. Copy changes must preserve the stable subject tail the e2e harnesses select by (`NOTIFICATION_SUBJECTS.verifyEmail`, substring match) or update those constants in the same PR.
...
The reset email is a **specified artifact** like §13.3 — SSOT `infra/dev-stand/idp/provision.sh` step 8.sexies (ru + en mirror), rendered inside the same bundled inline-CSS/table MJML template:
...
| **Button**    | «Открыть страницу сброса» → the **bare** portal `/reset` (no query) — a subordinate navigation aid, never the flow                                                                                                                                                                                                                      |

Invariants: the **code format** is `[A-Z0-9]{6}` (Zitadel `PASSWORD_RESET_CODE`: 6 chars, uppercase + digits — the registration-code shape, distinct from the 8-digit login OTPs); the mail contains **no link into Zitadel's hosted UI** and **no GET-consumable URL** — every href is exactly `<portal>/reset` with no parameters. The send-side half of the contract is the bare `sendLink.urlTemplate` (§4, `passwordResetBody`).
