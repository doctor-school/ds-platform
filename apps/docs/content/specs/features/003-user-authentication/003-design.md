...
**The verification email is code-only and BFF-sent — no CTA, no link, no Zitadel send (GH #869 → #910, EARS-3/25/29).** Both email-verification send hops — the EARS-1/3 cascade's `requestEmailVerification(sub)` and the EARS-25 `resendEmailVerification(identifier)` — request the code with the `SendEmailVerificationCode` oneof **`returnCode`**: Zitadel generates, stores, expires, and attempt-counts the code but **sends no email**, returning the code to the BFF, which composes the **branded, Russian, code-only** mail (full artifact spec: §13.3) and dispatches it over the §14 transport chain (owner decision 2026-07-15, #910). The subject leads with the code, the body shows it as one unbroken, enlarged token with an explicit expiry line, and the mail carries **no link or button of any kind** — fully link-free (the Notion/Slack style). Rationale: mail-provider AV scanners (mail.ru's `checklink`) prefetch **every URL in a delivered message with a GET**, so any deep-link carrying the code (or any GET-consumed secret) is burned by a robot before the human ever clicks; and Zitadel's own template renders an **unsuppressible** CTA button into its hosted login-v2 UI — a dead end for portal registrants (the original #869 defect; the empty-`buttonText` fallthrough to the bundled default label was verified live on v4.15.0). Owning the send removes the button problem at the root instead of demoting it to a subordinate label. The registrant **types the code** on the portal `/verify` screen (§8.3), where the #175 auto-login replay signs them in — manual entry **is** the journey, not a degraded path. The former Zitadel-side machinery for this mail type — the `verifyemail` message-text override (`infra/dev-stand/idp/provision.sh` step 8.ter) and the bare-`urlTemplate` hop — is retired with the switch: Zitadel no longer sends this type. The SMS/phone hops keep the bare `{ sendCode: {} }`.
...
The three affected IdP-port methods — `requestEmailVerification(sub)` (EARS-1/3), `resendEmailVerification(identifier)` (EARS-25), and `requestPasswordReset(identifier)` (EARS-11) — request the code with the **`returnCode`** oneof (`SendEmailVerificationCode.returnCode` / `PasswordResetRequest.returnCode`): Zitadel returns the code to the BFF and sends nothing. **Zitadel remains the code authority** — generation, storage, expiry (`VERIFY_EMAIL_CODE` / `PASSWORD_RESET_CODE`, 3600 s), attempt-counting, and verification (EARS-3/12) are untouched; the BFF is a transport. Each hop hands the code to the `MailerModule`, which composes the §13.3/§13.4 artifact and dispatches it. The enumeration-safe wrappers are unchanged: the identifier-keyed hops still resolve identifier → `sub` without disclosing existence, and the no-op paths (unknown / already-verified) obtain no code and send nothing. The login email-OTP (EARS-6, `verifyemailotp`) and the SMS hops stay Zitadel-sent — deliberately out of this increment (requirements → Scope).
