...
4. **Back on `/verify`.** The user types the 6 characters (the email shows them grouped `GX5 AVU` for reading; they are entered without the space). The slotted `InputOTP` auto-submits on the last character (#175); the screen confirms «Код принят — входим…» only after the server accepts (EARS-3).
...
| **Body**      | The code **grouped in two triads** (`GX5 AVU`, enlarged/letter-spaced), the instruction to enter it (without the space) on the verification page, the explicit expiry line «Код действует 1 час» (the `VERIFY_EMAIL_CODE` generator lifetime, 3600 s), and «Если вы не регистрировались на Doctor.School — проигнорируйте это письмо» |
