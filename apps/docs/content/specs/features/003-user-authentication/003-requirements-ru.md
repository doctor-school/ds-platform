...
issues: [81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 131, 207]
prior_decisions:
...
- Ответы register / login / reset неразличимы (status + body + timing ≤ 50 мс) между существующим и неизвестным идентификатором (EARS-16). Регистрация на уже-зарегистрированный email не вызывает записи аккаунта/согласия/ledger; любое адресованное владельцу уведомление «у вас уже есть аккаунт» доставляется out-of-band письмом (EARS-23), никогда через ответ API, а экран после регистрации не зависит от существования аккаунта (EARS-24).
