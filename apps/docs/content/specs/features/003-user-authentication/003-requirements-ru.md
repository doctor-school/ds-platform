...
> **Конвенция нумерации:** плоская (`EARS-1`, `EARS-2`, …) по ADR-0006 §4. EARS-1…12 — функциональные хендлеры (каждый становится дочерним Issue); EARS-13…21 — сквозные ubiquitous / unwanted-behavior требования, применяемые по всей поверхности. CI-гард `ears-tests` — content-match WARN в Phase 0.
...
- **EARS-4:** Когда регистрант отправляет SMS-код верификации, система должна проверить его через Zitadel `otp_sms`, выставить `phone_verified` и записать одну терминальную строку `auth.account.verified` (канал `sms`); невалидные/истёкшие коды ведут себя как в EARS-3.
