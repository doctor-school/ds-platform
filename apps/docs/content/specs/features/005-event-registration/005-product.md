...
- [`design-source/account-my-events.dc.html`](../../../../../../design-source/account-my-events.dc.html) — the «мои события» surface (US-4): tabs Предстоящие / Записи / Сертификаты, specialty filter, day-grouped registered events rendered as `ВебинарКарточка` (variants live/registered/past).
- [`design-source/event-page.dc.html`](../../../../../../design-source/event-page.dc.html) — the registration state on the event page (US-1, US-3): the status card's `upcoming / live` states carry the register CTA and the registered/join signposting copy.
- [`design-source/unit-event-card.dc.html`](../../../../../../design-source/unit-event-card.dc.html) — the `registered` card variant («вы записаны» time plate + join CTA when live).
