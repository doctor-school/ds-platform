...
- **Resolution order.** `localStorage` key **`ds-theme`** (`"light" | "dark"`; key absent = no explicit choice) → else **dark**. A stored explicit choice always wins; the system `prefers-color-scheme` is **not** consulted at all — no media-query resolution, no live system re-resolve, no theme watcher (owner Stage-B decision 2026-07-12: dark is the product default).
- **FOUC guard.** An inline `<script>` in the portal root layout `<head>` (`apps/portal/app/layout.tsx`), executed synchronously **before first paint and hydration**, reads `localStorage` and sets the class (`ds-theme` absent ⇒ dark) — the page never flashes the wrong theme on load or reload.
