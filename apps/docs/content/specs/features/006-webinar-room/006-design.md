...
- **Operational default — RU-reachable, embedding-enabled provider (#1125).** The audience is Russian doctors, so a webinar is broadcast on a provider that is reachable **and embeddable in RU without a VPN**: **Rutube is the default** for RU events. YouTube is **not** the default — it is geo-blocked in RU (the embed renders a black screen) and, even where reachable, refuses the `/embed/` frame unless **«Allow embedding» is enabled on the broadcast**. If a YouTube stream is used anyway, «Allow embedding» **MUST** be on. Because a provider-refused `/embed/` frame is a **silent** black iframe (cross-origin — the portal cannot detect the failure from JS), the room ALWAYS renders a truthful direct-watch link (`youtube.com/watch?v=` / `rutube.ru/video/<id>/`) beneath the player as the honest escape hatch, never relying on iframe-death detection.
