...
- **Direct-link fallback (EARS-9).** Beside the embed the room renders a plain anchor to the provider's OWN watch page — `resolveDirectUrl` per provider (`rutube.ru/video/<id>/`, `youtu.be/<id>`, `vk.com/video<oid>_<id>`, or the cdnvideo player URL itself). It is a link, not a proxy/re-host, so the embed-frame boundary holds; its purpose is recovery when the embed is refused broadcaster-side (the #1125 black-screen class). It renders only when a stream resolves (the "stream unavailable" state has nothing to link to).
