...
**A structurally silent provider is graded by neither.** cdnvideo exposes no parent API at all, so a watchdog stall on it carries **no information**: the banner would appear on _every_ cdnvideo эфир regardless of whether the stream is healthy, which makes it a guess rather than a status. cdnvideo therefore mounts straight into **`Unverified`** — the watchdog does not run for it, no grade is ever assigned, nothing covers or annotates the embed, and the only affordance is the gesture-gated «Перезапустить плеер» (owner decision 2026-09-06).
...
stateDiagram-v2
  ...
  Unverified --> Unverified: «Перезапустить плеер» re-creates the embed (still unobservable)
  ...
  note right of Unverified
    ...
    Embed visible, NO banner and no
    overlay — the room claims nothing.
    Restart control only on gesture;
    no playing signal can ever arrive,
    so the state never changes.
  end note
