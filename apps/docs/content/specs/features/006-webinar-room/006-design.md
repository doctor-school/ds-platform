...
**Two failure grades keep the status truthful.** The watchdog alone cannot distinguish a genuinely-stalled stream from a healthy-but-unobservable one (vk/cdnvideo expose nothing to the parent, and a youtube/rutube embed whose handshake never arrived is equally opaque). Covering a possibly-healthy video with a "stream not loading" overlay + auto-retry would itself be untruthful (and an auto re-create would interrupt working playback). So a watchdog stall is **graded** by whether any positive signal was ever observed (`everReady`):
...
- **SUSPECTED** — a watchdog stall with **no** positive signal ever seen. The room CANNOT prove failure, so it shows a **non-covering advisory banner** beside the still-visible, still-interactive embed and does **not** auto-retry (manual «Перезапустить плеер» only). Unobservable providers (vk, cdnvideo) — and a failed youtube/rutube handshake — can only ever reach this grade.
...
- **Provider events layer on top where they exist (EARS-18.2).** A `ready`/`buffering`/`playing` signal establishes the handshake (`everReady`); an `onError`/`player:error` is a CONFIRMED failure. Observed from official provider docs (design facts, cited):
  ...
  So for **youtube** the room establishes the handshake on `onReady`/`onStateChange`, clears the watchdog on `PLAYING`, and maps `onError` to distinct CONFIRMED copy — **101/150 → «встраивание запрещено владельцем трансляции»** vs **100 → «видео недоступно»**; for **rutube** it establishes on `player:ready`, clears on `player:changeState(playing)`, and shows a generic CONFIRMED failure on `player:error` (no code to distinguish). **vk (live)** and **cdnvideo** are **watchdog-only** — unobservable providers can only ever reach the SUSPECTED grade; the room never auto-declares a confirmed failure it cannot observe, and never covers the embed. A stated capability constraint of those providers, not a TODO.

- **Bounded retry (CONFIRMED); manual-only (SUSPECTED), in-room always (EARS-18.3).** On `Confirmed`, the room **auto-retries up to `PLAYER_MAX_AUTO_RETRIES` (default 2)** with a delay (`PLAYER_RETRY_DELAY_MS`), re-creating the embed each attempt (or an in-place provider reload where supported); exhausted → the manual **«Перезапустить плеер»** affordance in the covering overlay. On `Suspected` there is **no** auto-retry (re-creating a possibly-healthy embed would interrupt it) — the advisory banner offers the same manual **«Перезапустить плеер»** from the start. Both re-create the embed and clear the status. **Never** `window.location.reload()` (a full page reload tears down the room, the chat connection, and resets the heartbeat context), and **never** a "watch on the provider's site" / off-platform link — verified in-room presence is the sponsor value, and an off-platform link leaks that audience away (owner decision 2026-07-22; §3 no-off-platform bullet).
