...
**Two failure grades keep the status truthful.** The watchdog alone cannot distinguish a genuinely-stalled stream from a healthy-but-unobservable one (an embed whose handshake never arrived — youtube, rutube, or a vk src built without `js_api=1` — is opaque to the parent). Covering a possibly-healthy video with a "stream not loading" overlay + auto-retry would itself be untruthful (and an auto re-create would interrupt working playback). So a watchdog stall is **graded** by whether any positive signal was ever observed (`everReady`):
...
- **SUSPECTED** — a watchdog stall with **no** positive signal ever seen on a provider that COULD have spoken (youtube, rutube, or a vk src built without `js_api=1`). The room CANNOT prove failure, so it shows a **non-covering advisory banner** beside the still-visible, still-interactive embed and does **not** auto-retry (manual «Перезапустить плеер» only).

**A structurally silent provider is graded by neither.** cdnvideo exposes no parent API at all, so a watchdog stall on it carries **no information**: the banner would appear on _every_ cdnvideo эфир regardless of whether the stream is healthy, which makes it a guess rather than a status. cdnvideo therefore mounts straight into **`Unverified`** — the watchdog does not run for it, no grade is ever assigned, nothing covers or annotates the embed, and the room offers **no remedy of its own** — the provider's own in-iframe controls are the only affordance. A permanently visible room-owned control is the same intrusion as a permanently visible banner, so the room renders **nothing of its own** over a cdnvideo embed (owner decision 2026-09-06).

```mermaid
stateDiagram-v2
  [*] --> Loading: observable provider mounted (config present)
  [*] --> Unverified: cdnvideo mounted — structurally silent, never graded
  ...
  note right of Unverified
    cdnvideo ONLY, from mount.
    Bare provider iframe: NO banner,
    no overlay, no room-owned control
    — the room claims nothing and
    offers nothing. No playing signal
    can ever arrive, so the state
    never changes.
  end note
...
- **Watchdog is the detection floor (parent-observable providers only).** A named portal design constant `PLAYER_WATCHDOG_MS` (default **~20 s**) bounds the time from embed mount to the first observed playing signal; elapsing with no signal grades to `Suspected` (no handshake) or `Confirmed` (post-handshake stall) (EARS-18.1). It is armed **only for a parent-observable provider**: on a structurally silent one an elapsed watchdog proves nothing, so it never runs there (see `Unverified` below). Where it does run it is the detection floor an opaque cross-origin iframe otherwise denies — but on its own it can only ever raise the SUSPECTED grade (it cannot prove a failure it cannot observe).
- **Provider events layer on top where they exist (EARS-18.2).** A `ready`/`buffering`/`playing` signal establishes the handshake (`everReady`); an `onError`/`player:error` is a CONFIRMED failure. Observed from official provider docs (design facts, cited):
  ...
  **cdnvideo is structurally silent — never graded, it rests in `Unverified`.** A byte-identical probe of the served clappr bundle (60+ s of decoded playback → **zero** parent messages) plus a static scan of all three served bundles (no parent-directed `postMessage`; the clappr hits found are hls.js **worker** traffic, not parent traffic) establishes that no parent-observable signal exists to layer on. Its own error UI renders **inside** the iframe. So cdnvideo can never be observed at all — and an advisory the room would raise on every эфир regardless of the stream health is a guess, not a truthful status. cdnvideo therefore mounts directly in **`Unverified`**: no watchdog, no grade, no banner, no overlay, no auto-retry and no room-owned control of any kind — a bare provider iframe. This is a structural capability constraint verified by probe, not a TODO and not a deferral.

- **Bounded retry (CONFIRMED); manual-only (SUSPECTED), in-room always (EARS-18.3).** On `Confirmed`, the room **auto-retries up to `PLAYER_MAX_AUTO_RETRIES` (default 2)** with a delay (`PLAYER_RETRY_DELAY_MS`), re-creating the embed each attempt (or an in-place provider reload where supported); exhausted → the manual **«Перезапустить плеер»** affordance in the covering overlay. On `Suspected` there is **no** auto-retry (re-creating a possibly-healthy embed would interrupt it) — the advisory banner offers the same manual **«Перезапустить плеер»** from the start. For the permanently-unobservable **cdnvideo** the room offers **no remedy of its own**: no advisory, no overlay and no restart control — the provider's own in-iframe controls are the only affordance. The room has nothing truthful to say about a stream it cannot see, and a control it hangs over that stream permanently is an intrusion rather than a repair — the same intrusion as a permanent banner, in another shape. Every SUSPECTED case — a failed youtube/rutube/vk handshake — keeps its advisory banner until a playing signal or a manual restart clears it, because for those providers a signal genuinely can still arrive and a self-withdrawing banner would hide a real, observable failure. Both grades re-create the embed on the manual control and clear the status. **Never** `window.location.reload()` (a full page reload tears down the room, the chat connection, and resets the heartbeat context), and **never** a "watch on the provider's site" / off-platform link — heartbeat-covered in-room time is the browser-observed sponsor evidence, not proof of physical attention; an off-platform link moves viewing outside that evidence and directs the audience away (owner decision 2026-07-22; §3 no-off-platform bullet).
...
- **The control.** The room-header toggle is the **canvas 44×44 icon-button** (`archive/archive-room-v1.dc.html` line 25, sitting in the header's icon-button family alongside the 44px mobile ✕): a `<button>` with a transparent background, a 2px border at half-strength header-foreground (the `header-hairline` token, next bullet), a full-strength `header-foreground` glyph — **☾ in the light theme / ☀ in the dark** (the canvas `themeIcon`) — and hover raising the border to full-strength header-foreground. Accessibility: `aria-pressed` reflects the dark state, the accessible name is «Переключить тему» from the typed message catalog, and focus-visible uses the DS focus ring. Activating it flips the class and writes `ds-theme`. The DS `switch.tsx` primitive remains the FORM switch — it is **not** the room-header theme control (owner Stage-B decision 2026-07-12; ADR-0013 canvas-wins). The toggle is the **only** visible theme control in the portal; rolling the control out across portal chrome is the tracked **#510** unified-portal-chrome deferral (owner decision 2026-07-11) — the mechanism itself (class, guard, storage) is portal-wide from day one, so #510 adds placement, not plumbing.
