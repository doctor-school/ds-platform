...
stateDiagram-v2
  ...
  Loading --> Playing: playing signal observed (provider event) OR before watchdog fires
  Loading --> Failed: watchdog threshold elapsed with no playing signal / provider onError
  Playing --> Failed: provider error / stall after play (where observable)
  Failed --> Retrying: auto-retry (bounded) OR «Перезапустить плеер»
  Retrying --> Playing: playing signal observed (recovery)
  Retrying --> Failed: retry exhausted / still no signal
  note right of Failed
    Truthful in-frame status overlay
...
- **Watchdog is the detection floor (provider-agnostic).** A named portal design constant `PLAYER_WATCHDOG_MS` (default **~20 s**) bounds the time from embed mount to the first observed playing signal; elapsing with no signal transitions `Loading → Failed` (EARS-18.1). This is the **only** universal detector — it works even for providers that expose nothing to the parent.
- **Provider events layer on top where they exist (EARS-18.2).** Observed from official provider docs (design facts, cited):
  ...
  | **vk**       | `js_api=1` JS API — **does NOT work for live broadcasts** | no (live)                                               | no (live)                                                                                       | no — iframe re-create only            |
  | **cdnvideo** | Clappr, **same-origin script API only** — no postMessage  | nothing from a cross-origin iframe                      | nothing                                                                                         | no (in iframe mode)                   |

  So for **youtube** the room clears the watchdog on `PLAYING` and maps `onError` to distinct copy — **101/150 → «встраивание запрещено владельцем трансляции»** vs **100 → «видео недоступно»**; for **rutube** it clears the watchdog on `player:changeState(playing)` and shows a generic failure on `player:error` (no code to distinguish). **vk (live)** and **cdnvideo** are **watchdog-only** — a stated capability constraint of those providers, not a TODO.

- **Bounded retry, in-room only (EARS-18.3).** On `Failed`, the room **auto-retries up to `PLAYER_MAX_AUTO_RETRIES` (default 2)** with a delay (`PLAYER_RETRY_DELAY_MS`), re-creating the embed each attempt (or an in-place provider reload where supported). Exhausted → the manual **«Перезапустить плеер»** affordance (canvas outline button in the status overlay) re-creates the embed. **Never** `window.location.reload()` (a full page reload tears down the room, the chat connection, and resets the heartbeat context), and **never** a "watch on the provider's site" / off-platform link — verified in-room presence is the sponsor value, and an off-platform link leaks that audience away (owner decision 2026-07-22; §3 no-off-platform bullet).
- **Recovery clears the overlay (EARS-18.4).** A playing signal observed after `Failed`/`Retrying` — from an auto-retry, the manual restart, or the provider self-recovering — transitions back to `Playing` and clears the overlay. The 2026-07-22 Rutube CDN edge self-recovered after ~22 s failover; with the watchdog + retry the doctor sees a truthful status and can restart rather than stare at a black frame while the CDN heals.
...
- **Realtime presence-count push (server-authoritative).** The live in-room counter (« · N» in the chat-column header, §8.1 — the count of **distinct doctors** currently present in the open window) is derived server-side from the **same presence aggregate** as the minutes, **never** from Centrifugo native channel presence (which counts connections/tabs, not distinct doctors, and would corrupt sponsor attendance reporting). On the beat/expiry path — when an accepted heartbeat, or a presence-window expiry, **changes** the distinct-doctor count — the api **publishes the recomputed count** to the room's realtime channel; the portal `RoomPresenceProvider` consumes the published value and renders it instantly, without waiting on the observer's own next beat. **Channel:** the count rides the **existing** room channel (`room:event:<id>`, §7) as a typed `RoomPresenceCountMessage` (a `type: "presence-count"` discriminant, so it never cross-parses a chat message) — reusing the already-provisioned Centrifugo AND the subscribe-only credential the `RoomConfig` grant already carried, so there is **no** dedicated presence channel, **no** second credential, and **no** new endpoint (the room's single Centrifugo connection, owned by the always-mounted chat panel, routes both). The api publishes only on a **change** (a per-room `lastPublished` latch); a **per-room timer**, armed at the next window-expiry instant and re-armed on each beat, catches a leave (a beat that simply stopped) and publishes the decrease within ~1 s. **Truthful degradation:** while the realtime channel is unavailable, `RoomPresenceProvider` falls back to the heartbeat-ack refresh path (#1136) — the count returned on each accepted beat — so the counter is beat-paced but **never silently frozen** on a stale value (EARS-5). Presence heartbeats stay unchanged as the 60 s attendance-control mechanism, and the sponsor minutes/report derivation source is untouched. Asserted in a two-session realtime e2e: a second doctor's join/leave moves the observer's counter within ~1 s without the observer's own beat.
...
- **Player failure states (EARS-18, `apps/portal`):** a Vitest unit test drives the player state machine on a **fake clock** — the watchdog fires the "stream not loading" overlay when no playing signal arrives within `PLAYER_WATCHDOG_MS`; a YouTube `onError` 101/150 vs 100 renders the distinct "embedding disabled" vs "video unavailable" copy; a rutube/youtube `playing` event clears the overlay; auto-retry runs the bounded attempts then surfaces «Перезапустить плеер», which re-creates the embed (asserted: **no** `window.location.reload`, **no** off-platform anchor in the DOM); a playing signal after failure clears the overlay; the heartbeat keeps firing across a player failure (presence decoupled, EARS-18.5); vk/cdnvideo assert **watchdog-only** (no provider-event path). A Playwright spec drives the visible overlay + restart affordance in the live room.
