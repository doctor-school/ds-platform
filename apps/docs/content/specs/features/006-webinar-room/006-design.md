...
- **Bounded retry (CONFIRMED); manual-only (SUSPECTED), in-room always (EARS-18.3).** On `Confirmed`, the room **auto-retries up to `PLAYER_MAX_AUTO_RETRIES` (default 2)** with a delay (`PLAYER_RETRY_DELAY_MS`), re-creating the embed each attempt (or an in-place provider reload where supported); exhausted → the manual **«Перезапустить плеер»** affordance in the covering overlay. On `Suspected` there is **no** auto-retry (re-creating a possibly-healthy embed would interrupt it) — the advisory banner offers the same manual **«Перезапустить плеер»** from the start. Both re-create the embed and clear the status. **Never** `window.location.reload()` (a full page reload tears down the room, the chat connection, and resets the heartbeat context), and **never** a "watch on the provider's site" / off-platform link — heartbeat-covered in-room time is the browser-observed sponsor evidence, not proof of physical attention; an off-platform link moves viewing outside that evidence and directs the audience away (owner decision 2026-07-22; §3 no-off-platform bullet).
