...
The room where a registered doctor actually watches the webinar: an embedded external stream (Rutube or similar — we build **no** transcoding and **no** media server), a live chat, and invisible-to-the-doctor presence capture. Access is **server-side gated** — authenticated **and** registered only; there is no legacy-style soft UI wall over the player. The player is selected by an **explicit provider enum** (`rutube | youtube | vk | cdnvideo`) configured in admin — never by URL-sniffing. Chat rides Centrifugo (already in our stack). Presence is captured from minute one via a **server-authoritative heartbeat**: the room posts immediately on entry or visible-tab resume, then every N seconds only while visible; a hidden tab pauses. The backend appends accepted beats to a durable Postgres table. Latest-beat freshness supplies the distinct-doctor live count for at most `2 × N`, while separate accepted-beat N-buckets supply sponsor minutes with no trailing grace. A still-visible tab keeps beating even if the doctor physically walks away because the browser has no truthful signal for that distinction. For the first webinar the sponsor report is a manual export from these data.
...
- **US-3** — As a **doctor**, my presence is captured automatically while the room tab is visible: immediately on entry or visible resume, then every N seconds; hiding the tab pauses capture. I never click anything to "prove" I'm there, and the platform cannot distinguish physical absence while the tab remains visible (presence-confirmation widgets are a wave-2 NMO mechanic, not a wave-1 requirement).
- **US-4** — As a **pharma sponsor**, after the webinar I receive per-doctor heartbeat-covered, browser-observed minutes (with contacts) for my report — for the first webinar, produced as a manual export from the captured data.
...
3. On room entry the client sends an authenticated heartbeat immediately, then every N seconds while the tab remains visible. Hidden pauses the schedule; returning visible sends immediately before the N-second grid resumes. Each accepted beat lands in the append-only presence table with doctor, event, and time; a still-visible tab continues even if the doctor has physically walked away.
...
- Presence heartbeats are **server-authoritative and durable**: entry and visible-tab resume send immediately, visible tabs continue every N seconds, and hidden tabs pause. A still-visible tab keeps beating even if the doctor physically walks away because no truthful browser signal says otherwise. Every accepted beat remains append-only; only the downstream live-count and sponsor-minute derivations coalesce concurrent tabs per doctor. The latest accepted beat stays fresh for the live distinct count for at most `2 × N`; that count-only grace adds no trailing sponsor minutes, which come separately from accepted-beat N-buckets.
...
- **Heartbeat cadence** (recon §10-3): _resolved._ N is server-configured and defaults to 60 s; the heartbeat schedule, `2 × N` freshness, and accepted-beat N-bucket minutes are parameterized over it. No live cadence confirmation is needed to fix the code or spec. An operator-confirmed cadence change updates the server configuration; all derivations remain parameterized.
