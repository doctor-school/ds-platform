...
**What does not change.** `PublishEvent` / `OpenRoom` / `CloseRoom` and their side effects are untouched, as is the terminal command's behaviour under its new name. There is still no unpublish, no backward move and no reopening of a hidden event. Feature 004's `published → hidden` route for a cancelled or never-aired event is unchanged; such an event is a `platform` event and never becomes `in_archive`. The admin «Архивный эфир» create surface is named here and specified in 014 (its look is Stage-A/Stage-B gated separately); the automated import of old-platform broadcasts is [#1879](https://github.com/doctor-school/ds-platform/issues/1879).
