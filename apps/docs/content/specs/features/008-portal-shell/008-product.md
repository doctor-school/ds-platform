...
- **US-2** — As a **guest**, that same persistent header shows a clear **Войти / Регистрация** button, so I always have an obvious way in.
...
1. On any portal page, the app-shell header renders: logo (→ `/`), top-nav **Эфиры**, theme toggle, and — for a logged-in doctor — an auth cluster (avatar icon with the doctor's initials plus **«Мои события»**); for a guest, a **Войти / Регистрация** button.
...
2. The same `/` renders for a guest — the discovery front-door does not branch on auth state; only the header's account affordance differs (avatar icon vs. «Войти / Регистрация»).

## Product acceptance criteria

- A **persistent header** is present across portal surfaces and carries: the **logo** (→ `/`), the top-nav **Эфиры**, a **theme toggle**, and — **logged-in** → an auth cluster: an **avatar icon (initials) that navigates to `/account`** (the profile) plus **«Мои события»** (→ `/account/events`); **guest** → a **«Войти / Регистрация»** button. This is explicitly **not a dropdown menu**, and there is **no «Выйти» in the header** (sign-out lives on the profile, feature 009). The header truthfully reflects auth state.
...
None — the shell is fully designed in the vendored canvas: the persistent header is present across all portal canvases (logo→`/`, top-nav [Эфиры], theme toggle, and — signed in — the auth cluster (avatar-icon→profile plus «Мои события») / «Войти / Регистрация» for a guest), and mobile collapses the nav into a `≡` dropdown [Эфиры] plus the auth cluster.
...
- The **persistent app-shell header** is the vendored-canvas header present across **all** portal screens — [`design-source/account-my-events.dc.html`](../../../../../../design-source/account-my-events.dc.html), [`design-source/events-feed.dc.html`](../../../../../../design-source/events-feed.dc.html), [`design-source/event-page.dc.html`](../../../../../../design-source/event-page.dc.html) — carrying logo→`/`, top-nav [Эфиры], a theme toggle, and — signed in — the auth cluster (avatar-icon→profile plus «Мои события») / «Войти / Регистрация» (guest); mobile collapses the nav into a `≡` dropdown. The avatar-icon→profile behaviour is per the owner clarification 2026-07-12.
...
**What does not change.** The persistent header and its composition, the theme toggle, the guest «Войти / Регистрация», the initials-avatar icon to `/account`, the absence of a header dropdown and of a header sign-out, the mobile `≡` collapse, and the retirement of the «Каркас приложения» scaffold. The nav gains no entry for a route that does not exist: `/projects` and `/experts` arrive with features 015 and 016.
