...
- **EARS-8** (realizes **US-4**): The system shall render `/` as the **public** discovery listing of upcoming broadcasts, reusing the feature-004 listing surface (vendored `webinars-listing.dc.html`), **identically** for a guest and a logged-in doctor; `/` shall not branch its content on auth state — only the header's account affordance differs (EARS-4 / EARS-5). _[Amended 2026-08-17 — the discovery listing is served at `/webinars`; the rendering rule is unchanged. See Amendment below.]_
...
1. **`/` is the Academy landing; `/webinars` is the canonical discovery listing.** The listing surface itself is unchanged (feature 004, vendored `webinars-listing.dc.html`) — only its route is named explicitly, as it already is in production. The header logo and the «Эфиры» nav target resolve to `/webinars`.
