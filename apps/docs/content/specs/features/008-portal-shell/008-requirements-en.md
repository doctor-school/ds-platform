...
**Consequences for the clauses above.** EARS-2's `/` nav target, EARS-7's landing target, and EARS-8's and EARS-9's `/`-as-listing premise each carry an inline pointer to this block; their `/` reading is superseded by it and by nothing else, as are the corresponding invariants and verification rows 2, 7, 8 and 9. The shipped assertions in `apps/portal/app/login/page.test.tsx` that pin the post-login target, and the `008-scenarios.feature` example asserting a landing on `/`, are updated by the feature-013 EARS-15 Issue, which owns that edit as a named deliverable — never as a silent test change.
