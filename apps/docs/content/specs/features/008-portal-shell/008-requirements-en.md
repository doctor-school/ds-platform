...
description: "Requirements: the persistent portal app-shell header (logo→/, top-nav [Эфиры · Мои события], theme toggle, avatar-icon→profile for a logged-in doctor / «Войти» for a guest) and `/` as the canonical public discovery front-door and post-login landing — both amended 2026-08-17 (feature 013): `/` is the Academy landing, discovery is `/webinars`, and the post-login landing is the visitor's return target defaulting to `/webinars`; retires the `/` «Каркас приложения» scaffold. Portal surface IA epic (ADR-0014)."
...
- **UI from the design system (ADR-0013).** The header and `/` are built from `@ds/design-system`, styled **only** via tokens (arbitrary Tailwind values are lint-blocked). Every canvas the surface renders is vendored under [`design-source/`](../../../../../../design-source/README.md) and built from the files, not from prose. The persistent header is the vendored-canvas header present across `events-feed.dc.html`, `account-my-events.dc.html`, and `event-page.dc.html`; where prose and the canvas disagree, the canvas wins.
...
- **EARS-2** (realizes **US-1**): When a user activates a header navigation target, the system shall resolve it to its **shipped** surface: the logo and «Эфиры» → `/` (the discovery front-door), «Мои события» → `/account/events` (feature 005). Every nav target resolves to a shipped surface — there is no deferred or inert target. _[Amended 2026-08-17 — the logo and «Эфиры» resolve to `/webinars`; `/` is the Academy landing. See Amendment below.]_
...
- **Depends on shipped surfaces — all already on `main`.** `/` reuses the **feature-004** listing surface (vendored `events-feed.dc.html`); «Мои события» → **feature 005** (`/account/events`); the avatar icon → the **`/account` profile** (shipped via [#770](https://github.com/doctor-school/ds-platform/issues/770), superseding feature 009). Features 004/005 and the `/account` profile are **all shipped** — every avatar/nav target this shell wires resolves to a surface that already exists, so 008 has **no unbuilt dependency**. It is a pure UI-composition slice over shipped surfaces; the header owns none of them.
- **Design source (canvas) is vendored.** The header + `/` canvases (`events-feed.dc.html`, `account-my-events.dc.html`, `event-page.dc.html`) are already under `design-source/` (ADR-0013). No DesignSync pull is a prerequisite for authoring; Stage-B render-parity (EARS-12) verifies against them.
