...
| `GET /v1/admin/auth/state`                   | public        | —                | —        | reads whichever admin cookie is present                                                                       | `AdminAuthState` read model; the enum only     |
...
| `DELETE /v1/admin/users/:id/mfa` (EARS-13)   | authenticated | `platform_admin` | **true** | high-stakes introspection + **caller's own current TOTP code in the request** (`step_up` metadata: **false**) | LD-2 recovery; emits `auth.mfa.reset`          |
...
- **`GET /v1/admin/auth/state` — the read the admin app routes on, and why it must exist.** All three admin cookies are `HttpOnly` (`__Host-ds_admin_session`, `__Host-ds_admin_pending`) or authority-free (`__Host-ds_admin_csrf`), so the admin app cannot answer "login form, enrollment, challenge, or the app?" from the browser. Without this route the client would have to infer its position from the failure statuses of calls it should never have made — probing `/v1/admin/events` to discover it is not signed in — which is both noisy in the ledger and wrong at every race. It is classified `public` because that is the honest value: it must answer a caller holding **no** credential (`unauthenticated`), which is exactly how the login screen learns to render the form. It is a read, so it owes no EARS-10 double-submit proof and asks for none.
- **The state route is bound by the EARS-7 disclosure rule, not exempt from it.** It returns `AdminAuthState` and nothing else — no attempt budget, no lock indicator, no factor id, no subject (Read models). A locked account's response is byte-identical to an unlocked one in the same state, and every unresolvable credential (absent, expired, foreign fingerprint) collapses to `unauthenticated` rather than a diagnosis. One disclosure rule binds the verify surfaces and this read alike: a uniform verify response is worthless if a neighbouring GET says "locked".
- **It joins the auth-entry exemption list, and that is a deliberate trade.** The admin app reads it on every entry, usually while holding a pending reference — so without the exemption each normal navigation would append an `auth.session.rejected` / `pending_ref` row and the spec's sharpest forensic signal would become the ledger's noisiest. The cost is bounded and named: a fingerprint mismatch presented at THIS read alone writes no row (the caller is told `unauthenticated` and nothing more), and the next admin route that browser touches records it.
...
- **EARS-13 does not ride the general step-up mechanism, by choice rather than by absence.** The general elevation mechanism now EXISTS — `AdminAuthorityGuard` (#1304) reads `stepUp` and enforces a 30-minute server-verified MFA-elevation window on the token-free admin session, answering `401 STEP_UP_REQUIRED` + `stepUpUrl`. EARS-13 nevertheless keeps its route-local proof and still declares `step_up: false`: the caller's own current TOTP code is a stronger statement than window freshness (it proves possession *now*, not that a login happened within half an hour), and it is the proof the `auth.mfa.reset` row is written against. The route is a recorded exemption in the #1304 coverage registry, and graduating it onto the general mechanism stays the tracked deferral named in Scope → Out. The reasoning below is the record of why the route was built this way when the mechanism was still absent, and why the answer did not change once it landed: Verified against the tree: `stepUp` is declarative endpoint-authz metadata only — `apps/api/src/authz/authz.types.ts` (its own comment defers the mechanism to the ADR), rendered as a matrix column by `authz.matrix.ts`. **No guard reads it**, there is no `acr` or `mfa_fresh_at` check anywhere in `apps/api`, and no route declares it. So the two obvious moves are both wrong: declaring `step_up: true` would make the generated matrix advertise a protection nothing enforces — fail-open in a security artifact, on the most destructive action in the spec — and building a bespoke freshness guard would be building the step-up vertical inside this slice.
- **What EARS-13 does instead — a route-local fresh-possession proof.** The caller supplies **their own current TOTP code** in the removal request, and the API verifies it against the caller's registered factor at call time, reusing the very verification path EARS-5/EARS-6 already build. That satisfies ADR-0001 §10's _policy intent_ — an MFA change is elevated and needs fresh MFA — for this one route, proven by something that actually runs. Failures fall under the EARS-7 uniform-failure and shared rate-limit discipline, so the endpoint is not a code-guessing oracle either. The route carries **`step_up: false`**, and a test asserts it: the honest matrix value is the one that matches reality.
- **Graduation is tracked, not hoped for.** The step-up vertical has landed (#1304 — `AdminAuthorityGuard`, the server-verified 30-minute elevation window, the `401 STEP_UP_REQUIRED` + `stepUpUrl` contract); when EARS-13 is re-decided against it, this route moves onto it and the route-local check is replaced by metadata plus guard enforcement — a named follow-up Issue (requirements Scope → Out), not a comment. Admin mutations otherwise keep the ADR-0001 §2.5/§8 introspection posture.
...
- **API + state machine (Vitest e2e + unit, `apps/api`).** Against dev-stand Zitadel + Redis + Postgres, `skipIf(!IDP_ISSUER || !REDIS_URL || !DATABASE_URL)`. The load-bearing assertions run **directly against the API**, never only through the UI:
  ...
  - **Failure discipline (EARS-7):** wrong code / unregistered factor / locked account produce an **identical** body + status within the ≤50 ms band; §7 per-user and per-IP budgets enforced and **shared** across primary auth and both verify endpoints — proven by exhausting the window through the LOGIN route alone and observing the verify route refuse with the generic throttled answer rather than handing out an allowance of its own (the verify bodies carry `{ code }` and nothing else, so the per-user dimension only engages if the subject is wired in explicitly from the pending record); **both** verify surfaces append `auth.mfa.failure` (discriminated by a `stage` metadata field, so one wire id serves both without collapsing them at read time); the threshold soft-locks, writes exactly one `auth.lockout.triggered` with `reason: "mfa_attempts"`, and notifies — the notification asserted against the mailer port rather than on the response path, because an SMTP round-trip inside the threshold-crossing request would make attempt #10 measurably slower than #1-9 and rebuild the timing oracle the ≤50 ms band exists to close; a locked account is refused on a **correct** code, and its soft-lock expires with its window (unit-level, where the 30-minute boundary is observable).
  ...
  - **Factor removal (EARS-13):** with the caller's **own current TOTP code** supplied, the target's factor is removed, exactly one `auth.mfa.reset` row is written carrying the acting operator in `by_admin`, and the target's next login enters `mfa_pending_enrollment` — the producing path for the row, which is the point of putting the action behind an endpoint. A missing / wrong / expired / **replayed** caller code refuses the removal with a response **byte-identical to an EARS-7 failure**, drawing on the same rate-limit budget; a `doctor_guest` is refused; **removing the caller's own last factor is refused**. The generated matrix row is asserted to carry **`step_up: false`** — a `true` there would advertise a guard that does not exist.
  - **Break-glass compensating path (LD-2):** while fewer than two operators hold a factor, the runbook's ops script is the only writer of `auth.mfa.reset`; its script-written row is asserted **shape-identical** to the endpoint-written one (same wire id, same `by_admin`, same `tier`), so a ledger reader cannot tell which path wrote it and the audit trail has no hole.
  - **Read-model non-disclosure (EARS-7 / Read models):** `GET /v1/admin/auth/state` (§9) is asserted across all four positions — no credential, pending enrollment, pending challenge, active — and its response for a locked account is byte-identical to the unlocked one in the same state, with no attempt-budget, lock-state, factor-existence, or subject field in the payload. The uniform-disclosure rule is tested on the state endpoint, not only on the verify endpoints.
...
| Step-up (general mechanism)  | ADR-0001 §10         | **Built (#1304), deliberately not depended on here.** `AdminAuthorityGuard` reads `stepUp`; EARS-13 keeps proving freshness route-locally with the caller's own current TOTP code, is a recorded exemption in the #1304 coverage registry, and graduating onto the general mechanism is a tracked deferral. | EARS-13 refuses without the caller's own current TOTP code, and its matrix row reads `step_up: false` — no advertised-but-unenforced protection.                                                                                |
| Operator population          | ops / AGENTS.md §7   | **Preconditions** the EARS-13 path on ≥2 enrolled `platform_admin` operators; below that the LD-2 break-glass path applies.                                                                        | The enrolled-operator count is checked before the slice ships; the runbook carries the break-glass step and its compensating `auth.mfa.reset` writer.                                                                           |
...
**Sequencing.** EARS-8 (IdP provisioning) and EARS-1/EARS-2 (the cookie tier) are independent of each other and both precede the MFA handlers: EARS-8 because enrollment/challenge cannot be exercised without the capability, EARS-1/2 because the session the handlers issue must exist first. EARS-3 (policy) gates EARS-4/5/6; EARS-7 and EARS-9 attach to the verify handlers; EARS-13 follows EARS-5 twice over — a factor must be registrable before it is removable, and the caller-code check reuses the EARS-5/EARS-6 verification path — and depends on no step-up work; EARS-10/11/12 are cross-cutting and land with the surface. `open-ears-issues` step 4 wires these as native `blocked_by` edges, each with its recorded rationale.
