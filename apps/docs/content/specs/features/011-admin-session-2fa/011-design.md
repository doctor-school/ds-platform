...
- **Graduation is tracked, not hoped for.** When the step-up vertical lands (guard, `acr=mfa-fresh`, elevated-session TTL, the `401 step_up_required` contract), this route moves onto it and the route-local check is replaced by metadata plus guard enforcement — a named follow-up Issue (requirements Scope → Out), not a comment. Admin mutations otherwise keep the ADR-0001 §2.5/§8 introspection posture.
...
| Step-up (general mechanism)  | ADR-0001 §10         | **Not built, not depended on.** `stepUp` is matrix metadata with no guard behind it; EARS-13 proves freshness route-locally instead, and graduating onto the real mechanism is a tracked deferral. | EARS-13 refuses without the caller's own current TOTP code, and its matrix row reads `step_up: false` — no advertised-but-unenforced protection.                                                                                |
