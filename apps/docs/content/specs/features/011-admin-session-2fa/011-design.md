...
- **Separation is an explicit refusal, not an absence (EARS-2).** The admin session hook reads _only_ `__Host-ds_admin_session`. Because the admin app is same-origin with its `/v1/*` proxy, a browser holding a portal session **will** attach it — so "the portal cookie doesn't work here" must be code, not luck. Symmetrically the portal hook ignores the admin cookie. Both refusals emit `admin_session_rejected` / the portal's existing rejection path.
...
- **Step-up stays orthogonal.** No `step_up: true` rows are added and no `StepUpGuard` is built (requirements Scope → Out). ADR-0001 §10 governs re-verification _within_ a session; 011 governs the _entry_. Admin mutations keep the ADR-0001 §2.5/§8 introspection posture.
...
- **API + state machine (Vitest e2e + unit, `apps/api`).** Against dev-stand Zitadel + Redis + Postgres, `skipIf(!IDP_ISSUER || !REDIS_URL || !DATABASE_URL)`. The load-bearing assertions run **directly against the API**, never only through the UI:
  ...
  - **Separation (EARS-2):** table-driven over the full 007 route set — portal cookie only → refused + `admin_session_rejected`; admin cookie on a portal route → refused; both cookies → admin principal only; admin logout leaves a concurrent portal session valid.
  ...
  - **Enrollment (EARS-5):** the offer is served once and is not re-servable; a correct first code registers the factor, writes `mfa_enrolled`, and **issues the session in place** (LD-1 — the regression that would matter is a re-introduced forced re-login); a wrong code leaves the factor provisional.
  ...
  - **Failure discipline (EARS-7):** wrong code / unregistered factor / locked account produce an **identical** body + status within the ≤50 ms band; §7 per-user and per-IP budgets enforced and **shared** across both verify endpoints; the threshold soft-locks, writes `lockout_triggered`, notifies; a locked account is refused on a **correct** code.
  - **Audit (EARS-9):** exactly one terminal row per lifecycle action with the §7.3 wire id, actor, IP, UA; a **property assertion scans every emitted row** for the secret / provisioning URI / submitted code and fails on any occurrence; PD fields masked.
...
| MFA recovery                 | follow-up   | **Defers** self-serve recovery codes; LD-2 operator path is the interim.           | A tracked follow-up Issue exists with a real done-criterion; LD-2 is documented in the module runbook.          |

**Sequencing.** EARS-8 (IdP provisioning) and EARS-1/EARS-2 (the cookie tier) are independent of each other and both precede the MFA handlers: EARS-8 because enrollment/challenge cannot be exercised without the capability, EARS-1/2 because the session the handlers issue must exist first. EARS-3 (policy) gates EARS-4/5/6; EARS-7 and EARS-9 attach to the verify handlers; EARS-10/11/12 are cross-cutting and land with the surface. `open-ears-issues` step 4 wires these as native `blocked_by` edges, each with its recorded rationale.
