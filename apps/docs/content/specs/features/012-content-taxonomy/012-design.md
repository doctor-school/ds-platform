...
The original classification for that initial set comes only from a committed, complete owner-reviewed mapping keyed by stable `event_speakers.id`. Before inserting any queue, domain or audit row, the migration compares mapping keys and source UUIDs as exact sets and rejects a missing, duplicate or extra mapping entry. The mapping carries classification, not identity: no source name, normalized/equal name, Expert/User record, heuristic or generated suggestion is consulted to derive a classification, candidate or resolution. Acceptance fixtures provide explicit reviewed UUID entries for all three classifications.

Queue installation and the initial import close the backfill race with the legacy writer. While free-text authoring remains enabled, every later `event_speakers` insert and its `speaker_migration_reviews` insert commit atomically in the same transaction, with immutable original classification `unmatched`; neither row may commit alone. Source and review rows use restrictive references and expose no physical-delete command. Review resolution updates retained review state only: the operator explicitly selects an existing unlinked Expert or creates a new Expert with structured names, then confirms role and order, or explicitly marks the source content-removed. Resolution is idempotent and writes canonical `event_experts`, reviewer identity, reviewed-at, selected/created Expert id, disposition and original classification to audit. The original classification and provenance never change.

Cutover is one executable serializable transaction behind the same writer fence. It locks the source set against concurrent legacy inserts, establishes the closed set, and verifies exact one-to-one source↔queue coverage plus a resolution of canonical `event_experts` or content-removed for every source; an unresolved source, missing queue row or concurrent insert fails without changing the projection boundary. A successful cutover then disables every legacy free-text contract, including speaker fields in event create/update schemas, dedicated mutation routes and every public/admin read DTO or resolver, before releasing the fence. Only ordered `event_experts` remains writable or readable as the current speaker projection; retained `event_speakers` and reviews remain immutable migration history.

Reject and accept paths run against real Postgres rows; no manual SQL, name merge, seed, one-off script or temporary dual projection satisfies EARS-24.
