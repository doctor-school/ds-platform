...
2. **Manual re-creation on production.** Working from the mapping table, the Tech Lead executes direct SQL statements via `psql` on data-prod after the release is deployed: one `experts` row per person with structured `family_name`, `given_name` and optional `patronymic` (§4, EARS-19–20), and one `event_experts` row per legacy row carrying the event-specific `role` and `position`. The admin Expert form is not the mechanism, and no migration-specific route, table or admin screen is introduced. The feature-010 row-level audit trigger (`audit_row_change`, migration 0013) fires on these direct statements too: the session sets `app.actor_sub` to the Tech Lead and `app.source = 'manual'`, so the audit trail names the actor, and the executed statements are additionally recorded in the deploy record. An Expert already linked to another event is reusable; only the retained duplicate pair `(event_id, expert_id)` is refused.
