...
2. **Manual re-creation on production.** Working from the mapping table, the Tech Lead creates each missing Expert in the admin Expert form with structured `family_name`, `given_name` and optional `patronymic` (§4, EARS-19–20), then links that Expert to its event through the shared event↔expert relationship command (EARS-7, EARS-22), setting the event-specific `role` and `position`. Both are ordinary audited `platform_admin` taxonomy mutations; no migration-specific route, table or admin screen is introduced. An Expert already linked to another event is reusable; only the retained duplicate pair `(event_id, expert_id)` is refused.
...
The migration is deliberately a manual one-off, proportionate to the sixteen production rows it moves, and it is not a scaffold: it uses the real admin Expert form and the real relationship command, every resulting row is an audited taxonomy mutation, and the one cutover release removes the legacy contract, every object of the withdrawn design and the legacy column and table for good. No name merge, seed script or simultaneous merged projection is allowed.
