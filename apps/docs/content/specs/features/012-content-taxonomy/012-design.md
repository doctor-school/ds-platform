...
EARS-18 Stage A is the first UI gate: before any entity, join or removal-flow UI slice, reconcile it against the vendored canvases, the Product Lead's tabbed Refine picks from #1282/#1337 and the researched `@ds/design-system` blocks and state matrices from #1578. Those assets already cover this rework, so delivery reuses them without a fresh option round; only a genuinely uncovered visual class or composition re-enters `build-ui-from-design-system`. The removal confirmation and curator-resolution controls reuse the approved dialog and form blocks and never introduce a Delete action. Stage B drives each real UI on the live stand before merge. All copy uses the typed RU catalog; blocks own focus/hover/active/disabled/loading states.
