...
Production holds only a handful of legacy `event_speakers` rows. A review queue, an owner-reviewed import artifact, a classification vocabulary, a durable cutover phase, table triggers and a deploy rollback floor would all be machinery built for a volume that does not exist, so none of them are part of this design. The migration is a one-off manual re-entry performed by the Product Lead through surfaces that already exist.

For each remaining legacy row the Product Lead creates the missing Expert in the admin Expert form with structured `family_name`, `given_name` and optional `patronymic` (§4, EARS-19–20), then links that Expert to its event through the shared event↔expert relationship command (EARS-7, EARS-22), setting the event-specific `role` and `position`. Both are ordinary audited `platform_admin` taxonomy mutations; no migration-specific route, table or admin screen is introduced. An Expert already linked to another event is reusable; only the retained duplicate pair `(event_id, expert_id)` is refused. No source name, normalized name, heuristic or generated suggestion is consulted to establish or propose identity — the Product Lead decides who each legacy string is, and nothing else does.

The code cutover runs as two releases, both deployed strictly after the manual pass:

1. **Manual re-entry on production.** Every legacy row is re-created as an `event_experts` link through the admin surface.
2. **Confirmation.** The Product Lead confirms that the re-entry is complete. This confirmation is the gate; nothing that drops data ships before it.
3. **Cutover release.** The canonical resolver of EARS-8 reads `event_experts` only, with no legacy merge branch and no phase check. The free-text speakers field leaves the event create/update Zod and OpenAPI schemas, every legacy admin write seam and every public/admin read DTO and resolver branch. This release also deletes every object of the withdrawn design, none of which ever reached production: its forward migration drops the `speaker_migration_reviews` table, the `speaker_migration_cutover` singleton and both `event_speakers` triggers with their functions from migration `0032_speaker_migration_cutover.sql`, and the code drop removes `packages/db/src/schema/speaker-migration.ts`, the `apps/api/src/events/event-speakers.reconcile.ts` seam that maintains `event_experts.legacy_speaker_id`, `tools/deploy/rollback-floor.mjs` with its `--rollback` hook in `tools/deploy/prod.mjs`, its unit test, the rollback-floor sections of `packages/db/README.md` and `tools/deploy/README.md`, and the `apps/api/test/db/speaker-migration-fence.e2e-spec.ts` fence suite, which is deleted rather than re-pointed because the handler it fences no longer exists. `event_speakers` and `event_experts.legacy_speaker_id` are deliberately left in place.
4. **Production confirmation.** The Product Lead confirms that the migrated speakers render correctly on the live site.
5. **Contract release.** One forward migration drops the `event_experts.legacy_speaker_id` column together with its composite foreign key `event_experts_event_legacy_speaker_fk (event_id, legacy_speaker_id) → event_speakers(event_id, id)` and its unique index, and then `event_speakers` itself. Dropping a data-bearing column on the retained `event_experts` join is the same deliberate exception as dropping the table: that column exists only to key the removed migration and carries no taxonomy data.

The image running in production today reads `event_speakers` (`apps/api/src/events/events.repository.ts`, `apps/api/src/storefront/doctor-events.repository.ts`, `apps/api/src/taxonomy/speaker-projection.repository.ts`), so the two releases split for a reason. Rolling back the cutover release is app-only: it removes only objects production never received and leaves every table and column the previous image reads. The contract release is the deliberate point of no return — once the column and the table are gone, the previous image cannot read the database, so recovery there is roll forward or restore the database first, never an app-only rollback. `deploy:prod` carries no rollback-floor guard after the cutover release removes it, and none is required: the boundary is a release-ordering decision, not a runtime check.
...
- **Model/migration (≤4 runtime PRs):** [#1606](https://github.com/doctor-school/ds-platform/issues/1606) delivers EARS-19–20 (optional User convergence, structured names and system-owned entity slugs); [#1607](https://github.com/doctor-school/ds-platform/issues/1607) owns the two cutover runtime PRs — the cutover release (contract, resolver and every object of the withdrawn design, including the `deploy:prod` rollback-floor tooling and the migration-fence e2e suite) deployed only after the Product Lead's manual re-entry on production and confirmation, then the contract release dropping `event_experts.legacy_speaker_id` and `event_speakers` after the Product Lead confirms the live rendering; then feature-014 [#1608](https://github.com/doctor-school/ds-platform/issues/1608) consumes canonical `event_experts`. The contract-release PR closes #1607.
...
There is no legacy matching prerequisite or simultaneous merged projection: the Product Lead re-enters every legacy row explicitly, and #1607's cutover release lands before #1608 reads canonical `event_experts`.

## 8. Verification strategy

- **DB/migration:** all four entities/five joins satisfy lifecycle CHECKs, restrictive FKs, User→Expert uniqueness, structured names and system-slug uniqueness; the cutover forward migration leaves no `speaker_migration_reviews` table, no `speaker_migration_cutover` singleton and no `event_speakers` trigger behind it while `event_speakers` and `event_experts.legacy_speaker_id` are still readable; and the contract forward migration leaves no `legacy_speaker_id` column, no `event_experts_event_legacy_speaker_fk`, no unique index over it and no `event_speakers` table.
...
3. Run #1606 → the Product Lead's manual re-entry on production → the Product Lead's confirmation that every legacy row is covered → #1607's cutover release → the Product Lead's confirmation that the migrated speakers render correctly in production → #1607's contract release → feature-014 #1608. #1608 cannot project speakers before the cutover release is live; the contract release only removes what the cutover release already stopped reading.
...
The migration is deliberately a manual one-off, proportionate to the handful of production rows it moves, and it is not a scaffold: it uses the real admin Expert form and the real relationship command, every resulting row is an audited taxonomy mutation, the cutover release removes the legacy contract and every object of the withdrawn design, and the contract release removes the legacy column and table for good. No name merge, seed script or simultaneous merged projection is allowed.
