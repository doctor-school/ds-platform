...
`speaker_migration_reviews` is a real retained admin queue keyed by stable source `event_speakers.id`. The initial source set is every retained row physically present in `event_speakers`, including a row whose legacy content was already removed; content-removed is a review disposition, never an eligibility filter that removes the source from provenance. Each source is imported once with immutable provenance (event id, source id, original position and content fingerprint) and an immutable original classification `unmatched | ambiguous | duplicate`.

The original classification for that initial set comes only from a committed, complete owner-reviewed **row artifact** whose duplicate-preserving shape is an ordered list of `{ sourceId, classification }`. Validation compares those raw rows with the source set and rejects a missing source UUID, repeated source UUID or extra UUID before materializing any keyed UUID map and before inserting any queue, domain or audit row. The rows carry classification, not identity: no source name, normalized/equal name, Expert/User record, heuristic or generated suggestion is consulted to derive a classification, candidate or resolution. Acceptance fixtures provide explicit reviewed rows for all three classifications and preserve a duplicate long enough to exercise its rejection.

A retained singleton `speaker_migration_cutover` row is the database SSOT with monotonic phase `review_open | source_closed`, version, `phase_aware_release_sha`, monotonic `phase_aware_release_ordinal`, `minimum_compatible_release_sha` and `minimum_compatible_release_ordinal`; every phase/release-floor change is audited. The expand deployment records its own immutable release SHA/ordinal as the phase-aware release. The serializable source-closure transaction atomically advances the phase and copies that pair to the minimum-compatible pair, so no `source_closed` state exists without an enforceable rollback floor.

Database enforcement is attached to the table, not left to an application helper. `event_speakers_migration_fence_before_write` is a `BEFORE INSERT OR UPDATE OR DELETE ON event_speakers FOR EACH ROW` trigger whose function locks the singleton row `FOR UPDATE`. In `source_closed` it raises a database exception for every operation, including direct DML from a pre-expand image; restore, retire and reorder are UPDATEs and therefore cannot bypass it. In `review_open`, DELETE is always refused and UPDATE is refused once the source has a retained review. `event_speakers_enqueue_review_after_insert` is an `AFTER INSERT ON event_speakers FOR EACH ROW` trigger that inserts exactly one immutable `unmatched` review in the same transaction. During the initial import the same singleton lock excludes concurrent source mutation. API-aware callers map reviewed-source rejection to 409 `SPEAKER_MIGRATION_SOURCE_IMMUTABLE`; physical deletion of source or review rows is always rejected.

Review resolution updates retained review state only. The operator may select any reusable existing Expert or create a new Expert with structured names, then confirms role and order, or explicitly marks the source content-removed. An Expert linked to another event remains eligible; only the retained duplicate pair `(event_id, expert_id)` is refused. Resolution is idempotent and writes canonical `event_experts`, reviewer identity, reviewed-at, selected/created Expert id, disposition and original classification to audit. The original classification and provenance never change.

Cutover follows the production expand/contract rule as three ordered stages:

1. **Expand deployment:** add the queue, duplicate-preserving import, retained singleton fields, both table triggers and phase-aware legacy/canonical reader branches without removing old schemas or columns. The deployed image records its immutable production release SHA/ordinal as `phase_aware_release_*`; while phase remains `review_open`, the existing minimum-compatible pair is unchanged and ordinary app-only rollback remains safe.
2. **Writer fence and closed set:** one serializable transaction locks `speaker_migration_cutover` and the source set, verifies exact source↔queue coverage and canonical/content-removed resolution for every source, atomically copies `phase_aware_release_*` to `minimum_compatible_release_*`, then advances phase to `source_closed`. Any unresolved source, missing review, concurrent INSERT or existing-row mutation fails without changing phase, floor or projection. The table trigger rejects old/new legacy DML; every release at or above the durable floor recognizes `source_closed` and resolves speakers only from ordered `event_experts`, never a merged source.
3. **Contract deployment:** `pnpm deploy:prod --rollback <sha>` must, before any provider mutation, read the retained minimum-compatible SHA/ordinal from production, resolve the retained target image's authoritative release ordinal, and fail closed when the marker cannot be read, SHA/ordinal metadata disagree, or the target ordinal is lower. Only after this executable guard and live `source_closed` evidence exist may a later app release remove speaker fields from event create/update Zod/OpenAPI schemas, legacy mutation routes, and legacy public/admin DTO/resolver branches. Retained source/review tables and phase/floor SSOT are not dropped. App-only rollback to the phase-aware expand image remains database-compatible; rollback to a retained pre-expand image is rejected before provider state changes.

The closed-set transaction does not pretend to remove static application contracts. Final cutover is complete only after the contract deployment. The `deploy:prod` rollback-floor guard is a cross-track tooling prerequisite: implementing it requires an explicit owner-approved session switch before #1607 runtime work may proceed; this spec correction neither creates nor starts that work. Reject and accept paths run against real Postgres rows; no manual SQL, name merge, seed, one-off script or simultaneous merged projection satisfies EARS-24.
...
The migration and cutover use the durable database phase as a runtime compatibility boundary across their staged deployments; it is never an in-memory flag. Speaker position conflicts are rejected by retained `(event_id, expert_id)` uniqueness and the active `event_experts(event_id, position)` uniqueness rule. Expert lifecycle commands lock the Expert and linked Events, then revalidate relation eligibility; retained legacy rows are never consulted by a `source_closed` resolver.
...
- **Model/migration (≤4 runtime PRs):** [#1606](https://github.com/doctor-school/ds-platform/issues/1606) delivers EARS-19–20 (optional User convergence, structured names and system-owned entity slugs); [#1607](https://github.com/doctor-school/ds-platform/issues/1607) owns two deliberately staged runtime PRs/deployments, expand/fence first and static contract removal only after live `source_closed`; then feature-014 [#1608](https://github.com/doctor-school/ds-platform/issues/1608) consumes canonical `event_experts`. The contract PR, not the expand PR, closes #1607.
...
There is no legacy matching prerequisite or simultaneous merged projection: #1607 resolves every retained source row explicitly, switches every rollback-eligible reader through the database phase, and completes the contract deployment before #1608 reads canonical `event_experts`.

## 8. Verification strategy

- **DB/migration:** all four entities/five joins satisfy lifecycle CHECKs, restrictive FKs, User→Expert uniqueness, structured names and system-slug uniqueness; duplicate-preserving mapping rows fail before keyed-map materialization; the review queue covers every retained source row; every existing-row mutation is refused after import; the durable writer fence covers old/new writers; and source closure refuses unresolved or missing reviews.
...
- **Browser:** Playwright BDD creates all kinds, exercises structured names/system links/media controls, all relationship directions, migration review reject/accept/staged cutover, Expert removal, retire/restore, and proves no Delete/free-text speaker path after contract deployment. Controlled migration e2e executes both writer-first and close-first fence interleavings and app-only rollback to the phase-aware expand image.
...
3. Before #1607 runtime work, obtain owner approval to switch to the cross-track deploy-tooling prerequisite and make `deploy:prod --rollback` enforce the retained minimum-compatible release marker; this docs iteration does not create or start it. Then run #1606 → #1607 expand deployment → #1607 live trigger-fenced source closure/floor advance → #1607 contract deployment → feature-014 #1608. Source closure requires every retained source resolved/content-removed and exact phase-aware release metadata; #1608 cannot project speakers before contract deployment.
...
The migration is real, not a scaffold or manual step: it uses committed duplicate-preserving reviewed rows, retained source identity, an operator review queue, audited resolution, table-level triggers, an executable retained release floor and a separate contract deployment; no name merge, seed or simultaneous merged projection is allowed.
