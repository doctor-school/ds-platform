...
Historical `data.event_speakers.*` plaintext cannot be called remediated by an ordinary UPDATE, DELETE, compensating event or access-control view. Because feature 010 is live, #1305 must add a formal production amendment that admits one exact monotonic redaction transition while retaining each ledger row and event fact. An append-only `audit_pd_remediation_manifests` row lists every target ledger PK/field, SHA-256 preimage and approved legal basis/term/owner. One SECURITY DEFINER function under a dedicated migration role may replace only that listed plaintext leaf with `{ masked: true, legacyCiphertextRef, preimageHash }`, where `legacyCiphertextRef` addresses AES-256-GCM ciphertext under one retained per-row `audit_retention_keys` DEK; it must leave ledger identity, event fields, non-PD diff, timestamps and every unlisted byte unchanged and append a no-plaintext before/after-hash evidence row in the same transaction. The enforcement trigger permits the transition only while the matching manifest is active, the old preimage hash matches and the fixed envelope validates; consuming the manifest makes replay or any later UPDATE impossible, and DELETE remains impossible. No trigger disable, raw DBA update or plaintext compensating row is permitted. At the recorded legal term, Vault destroys the audit-retention DEK, leaving the retained ledger/envelope/manifest rows unreadable but intact. Future rows contain only `{ masked: true }`. Marker, manifest-completeness and restore tests must prove no unclassified plaintext before affected writes or publication.
...
- **API/worker e2e:** real Postgres/object storage/Vault fake with production-parity fencing; raw-input claim binding before exact JSON/multipart still-only canonical normalization, durable cleanup, pd-officer/fresh-step-up one-use plan approval and queued-plan expiry, all-and-only subject derivation, acquisition/pre-response claim races, curator resolution, DEK zeroization and external acknowledgements; publication/curator/speaker invariants; exact 012 base DTOs with no surface aggregates; lifecycle-token abuse and exact 412 replay; global UUID cross-actor/expiry behavior; bounded upload authorization and late-owner quiescence. Tests use `it('EARS-N: ...')`.
