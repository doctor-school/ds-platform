...
A retained singleton `speaker_migration_cutover` row is the database SSOT with monotonic phase `review_open | source_closed`; its phase transition and version are audited. The database writer-fence function reads and locks this row, and every legacy INSERT, UPDATE, restore, retire, reorder or physical-delete path is enforced through it even when called by an older application image. During the initial import the same fence excludes concurrent source mutation. In `review_open`, a later INSERT and its `speaker_migration_reviews` row commit in the same transaction with immutable original classification `unmatched`; neither may commit alone. As soon as any source has its immutable review, every UPDATE, restore, retire or reorder of that existing source is rejected with 409 `SPEAKER_MIGRATION_SOURCE_IMMUTABLE`; review invalidation is not an alternative policy. Physical deletion of source or review rows is always rejected.
...
1. **Expand deployment:** add the queue, row-artifact import, singleton phase, writer fence and phase-aware legacy/canonical reader branches without removing old schemas or columns. The database fence protects rollback to the immediately preceding image while phase remains `review_open`. Before source closure, the phase-aware expand image is established as the oldest rollback-eligible image, so every image eligible after closure recognizes both phases; a pre-expand image is no longer an allowed rollback target.
2. **Writer fence and closed set:** one serializable transaction locks `speaker_migration_cutover` and the source set, verifies exact source↔queue coverage and canonical/content-removed resolution for every source, then advances the durable phase to `source_closed`. Any unresolved source, missing review, concurrent INSERT or existing-row mutation fails without changing phase or projection. Database enforcement rejects every legacy write from both expand and newer images; every rollback-eligible reader sees `source_closed` and resolves speakers only from ordered `event_experts`, never a merged source.
3. **Contract deployment:** only after the closed phase is observed live, a later app release removes speaker fields from event create/update Zod and OpenAPI schemas, legacy mutation routes, and legacy public/admin DTO/resolver branches. Retained source/review tables and the phase SSOT are not dropped. App-only rollback to the phase-aware expand image remains database-compatible and cannot reopen legacy writes or reads because that image obeys the durable `source_closed` phase; no routine DB rollback is required.

The closed-set transaction does not pretend to remove static application contracts. Final cutover is complete only after the contract deployment. Reject and accept paths run against real Postgres rows; no manual SQL, name merge, seed, one-off script or simultaneous merged projection satisfies EARS-24.
...
3. Run the model/migration wave in strict order #1606 → #1607 expand deployment → #1607 live writer-fence/source closure → #1607 contract deployment → feature-014 #1608. Source closure requires every retained `event_speakers` row resolved or content-removed and a phase-aware rollback floor; #1608 cannot project speakers before the contract deployment.
...
The migration is real, not a scaffold or manual step: it uses committed duplicate-preserving reviewed rows, retained source identity, an operator review queue, audited resolution, a durable database fence and a separate contract deployment; no name merge, seed or simultaneous merged projection is allowed.
