...
The code cutover is one release, deployed strictly after the manual pass:
...
3. **Cutover release.** The canonical resolver of EARS-8 reads `event_experts` only, with no legacy merge branch and no phase check. The free-text speakers field leaves the event create/update Zod and OpenAPI schemas, every legacy admin write seam and every public/admin read DTO and resolver branch. One forward migration drops the `speaker_migration_reviews` queue, the `speaker_migration_cutover` singleton and both `event_speakers` triggers introduced by migration `0032_speaker_migration_cutover.sql`, and drops `event_speakers` itself as its last step.

Because that drop is the last step of the forward migration and the new code never reads the legacy table, an app-only rollback to the previous image remains database-compatible for every read it performs. There is no rollback-floor guard in `deploy:prod` and none is required.
...
- **DB/migration:** all four entities/five joins satisfy lifecycle CHECKs, restrictive FKs, User→Expert uniqueness, structured names and system-slug uniqueness; and the cutover forward migration leaves no `speaker_migration_reviews` queue, no `speaker_migration_cutover` singleton, no `event_speakers` trigger and no `event_speakers` table behind it.
...
3. Run #1606 → the Product Lead's manual re-entry on production → the Product Lead's confirmation that every legacy row is covered → #1607's cutover release → feature-014 #1608. #1608 cannot project speakers before the cutover release is live.
