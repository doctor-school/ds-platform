...
1. Production holds a handful of legacy rows, so the migration is a one-off manual re-entry, not a machine. For every remaining legacy row the Product Lead creates the missing Expert in the existing admin Expert form with structured names and links that Expert to its event through the shared event↔expert relationship command with the event-specific role and position. These are ordinary audited taxonomy mutations: there is no review queue, no import artifact, no unmatched/ambiguous/duplicate classification, no cutover state record, no table trigger and no deploy rollback floor. Identity is decided by the Product Lead — no name matching, no User or Expert inference and no generated suggestion establishes or proposes it. An Expert already linked to another event is reusable; only a duplicate retained event-to-Expert pair is refused.
2. The Product Lead confirms that the re-entry is complete. That confirmation is the gate: nothing that removes the legacy contract or drops data ships before it.
3. The cutover then runs as two releases. The **cutover release** makes ordered `event_experts` the only speaker path — the canonical resolver reads it alone with no legacy merge branch and no phase check, and the free-text speakers field leaves the event create/update contract, every admin write seam and every public and admin read DTO. Its forward migration removes the objects of the withdrawn design (the review queue, the cutover singleton, both legacy triggers and the deploy rollback-floor guard), which production never received, and keeps the legacy table and its join column, so a rollback to the image running in production stays app-only. After the Product Lead confirms that the migrated speakers render correctly in production, the **contract release** drops the legacy join column with its foreign key and unique index and then the legacy table itself. That contract release is the deliberate point of no return: below it a rollback is no longer app-only and requires a database restore.
...
- Speaker output is the ordered `event_experts` relation only. New free-text authoring is removed; the Product Lead re-enters the handful of legacy rows by hand through the existing admin surfaces, and the cutover and contract releases then remove the legacy field, its join column and the legacy table — no name merging anywhere.
...
- **Speaker migration:** `event_speakers` is legacy input only. The Product Lead re-enters those few rows by hand as Experts and ordered `event_experts` links through the existing admin surfaces, never name-merging; the cutover release then makes `event_experts` the only speaker path, and the later contract release drops the legacy join column and the legacy table.
