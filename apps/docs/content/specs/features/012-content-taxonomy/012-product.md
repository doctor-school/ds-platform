...
1. Every retained source row, including already removed legacy content, appears once in a paginated review queue with immutable provenance. Existing rows receive their original unmatched, ambiguous or duplicate classification only from complete owner-reviewed duplicate-preserving stable-source-UUID rows; a missing, repeated or extra row stops migration before a keyed map or any data mutation, and the UI never derives or suggests identity from any name, User or Expert.
2. A new legacy row committed while review is open is atomically queued as unmatched by a database insert trigger. A table-level write trigger blocks direct INSERT/UPDATE/delete after source closure and blocks edit/restore/retire/reorder once a source has its immutable review. Operator explicitly selects any reusable existing Expert or creates one with structured names, sets event role/order, or marks the source content-removed; only a duplicate retained event-to-Expert pair is refused. Reviewer and resolution are audited without overwriting the original classification, and retained source/review history cannot be deleted.
3. Cutover is staged for safe app-only rollback: a backward-compatible expand release records its exact production SHA/ordinal in retained cutover state; serializable closure advances both the phase and minimum-compatible SHA/ordinal; before any provider change, the documented rollback command rejects a retained image below that floor; a later contract release removes all static free-text fields, routes and read shapes. Thus even old direct DML or a retained pre-expand rollback cannot reopen legacy writes or merged reads, and only ordered `event_experts` remain current. Implementing the deploy-command guard is a cross-track prerequisite requiring an owner-approved session switch; this spec correction does not start it.
...
- Speaker output is the ordered `event_experts` relation only. New free-text authoring is removed; a one-time migration preserves each source row's provenance and records ambiguous, duplicate and unmatched rows for explicit resolution without name merging.
...
- **Speaker migration:** `event_speakers` is retained migration input only. One executable migration creates canonical ordered `event_experts`, preserves provenance, never name-merges, and audits ambiguous/duplicate/unmatched rows; public and admin speaker authoring then use only `event_experts`.
