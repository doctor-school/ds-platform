...
2. A new legacy row committed while review is open is atomically queued as unmatched. Once a source has its immutable review it cannot be edited, restored, retired, reordered or deleted. Operator explicitly selects any reusable existing Expert or creates one with structured names, sets event role/order, or marks the source content-removed; only a duplicate retained event-to-Expert pair is refused. Reviewer and resolution are audited without overwriting the original classification, and retained source/review history cannot be deleted.
3. Cutover is staged for safe app-only rollback: a backward-compatible expand release installs one durable database phase and writer fence understood by every rollback-eligible version; a serializable command locks and closes the exact resolved source set; a later contract release removes all static free-text fields, routes and read shapes. Once closed, even an app rollback cannot reopen legacy writes or merged reads, and only ordered `event_experts` remain current.
