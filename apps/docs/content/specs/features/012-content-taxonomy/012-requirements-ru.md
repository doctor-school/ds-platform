...
- Общие поля сущности: `status: draft | published | retired`, nullable `deleted_at`, nullable set-once `first_published_at`, монотонная `version` и timestamps. Создание начинает с `draft`; восстановление возвращает в `draft`, не очищая `first_published_at`.
...
- **Инвариант куратора опубликованного проекта.** В каждом committed-состоянии у published-проекта ровно один допустимый куратор. Retire этого эксперта и retire/понижение единственной curator-связи запрещены, пока не выполнена атомарная замена либо не retired сам проект.
...
- **LD-4 — атомарная замена куратора.** `ReplaceProjectCurator(projectId, expertId)` блокирует проект на версии из `If-Match`, проверяет published/non-retired нового эксперта, повышает или восстанавливает целевую связь, понижает прежнюю curator-связь до `member` и увеличивает затронутые версии в одной транзакции. Committed-состояния с нулём или двумя кураторами нет.
...
| `ProjectCuratorReplaced`                                   | Одна транзакция повышает допустимую замену и понижает прежнего куратора, не делая проект невалидным.                |
...
- **EARS-13** _(realizes: US-7)_ — Когда оператор запрашивает retire сущности или связи, admin сначала должен показать `RetirementImpact` для той же версии и потребовать confirmation; сервер должен запретить retire/понижение единственного допустимого curator-эксперта/связи опубликованного проекта, иначе один раз выставить `retired` + `deleted_at`, сохранить строки/FK, не менять lifecycle связанного и убрать цель из defaults/current public только фильтрацией.
...
- **EARS-16** _(realizes: US-1, US-2, US-3, US-4, US-5, US-6, US-7, US-8, US-9, US-12)_ — Admin reads/commands должны быть `authenticated` / `platform_admin` (401 unauthenticated, 403 non-admin), а public reads — zero-auth `public`; malformed поля/медиа/cursor должны давать 400, unknown/non-public — 404, duplicate pair/slug, неполнота публикации, попытка смены immutable slug, нарушение curator-инварианта и invalid transition — 409, missing precondition — 428, stale version — 412 в RFC 7807 со стабильными `errorCode`, ошибками по полям где применимо и `traceId`.
...
- Slug редактируется только пока `first_published_at IS NULL`; retire и restore никогда не сбрасывают этот постоянный identity-lock.
...
| ----- | ----------------------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1–5   | Vitest e2e + schema tests                 | отдельный bounded entity-kind suite на EARS                                        | Точные поля/опциональность/лимиты медиа, генерация и постоянный lock slug, публикация без связей, допустимый куратор, allow-list и draft/retired 404. |
| 6–11  | Vitest e2e + DB constraints               | отдельный bounded join/projection suite на EARS                                    | Каждый join, обе cardinalities, атомарная замена куратора и запреты, legacy constraints, порядок LD-2, изоляция тем и отсутствие каскадов.            |
...
| 13–14 | Vitest e2e + migration test               | `apps/api/test/taxonomy/lifecycle.e2e-spec.ts`                                     | Preview-before-confirm, запрет для единственного куратора, retire/restore, explicit retained reads, restrictive FK и отсутствие DELETE.               |
