...
Feature: Operators maintain one retained taxonomy that every Academy surface can query
  ...
  Rule: An operator creates a standalone Expert and copies its public address

    @EARS-2 @EARS-19 @EARS-20 @happy
    Scenario: A standalone Expert retains its structured names and generated public link
      Given a platform_admin operator in the admin app
      When the operator opens the standalone Expert creation form
      Then the Expert form has no slug editor and explains its generated address
      When the operator saves unique Cyrillic Expert names without selecting a User
      Then the saved Expert detail retains those names as a draft with initials and no User
      And the Expert detail exposes its generated canonical public link without a slug editor
  ...
  Rule: An operator restores a retired project Expert relationship
    ...
    Scenario: A retired project Expert relationship returns as the same retained row
      Given a platform_admin operator in the admin app
      When the operator creates a draft project and draft Expert for relationship restoration
      And the operator links that Expert to the project as a member
      Then the project roster shows that member and records the relationship identity
      When the operator retires that project Expert relationship
      Then the active project roster is empty and the retired relationship is hidden
      When the operator reveals retired project Expert relationships
      Then that retired relationship appears with the same identity
      When the operator restores that project Expert relationship
      Then the active relationship returns with the same identity
  ...
  Rule: Remaining taxonomy contract scenarios
    ...
    Scenario: Object-storage failure has an exact replayable outcome
      ...
      And no taxonomy entity relationship or domain audit row changes
    ...
    Scenario: Structured Expert and speaker link values are audited
      When Expert fields or an event_experts link created during the manual cutover change
      Then structured names role position and the acting operator are audited as ordinary taxonomy mutations
    ...
    Scenario: The approved Refine composition is implemented and re-confirmed live
      ...
      And the product owner confirmed the approved tabbed Refine compositions and researched design-system blocks as the admin Stage-A baseline
    ...
    Scenario: The free-text speakers field is absent from the event contract after the cutover
      Given the cutover release is deployed
      When a client submits a free-text speakers field to event create or event update
      Then the request is rejected by the event Zod and OpenAPI contract
      And no legacy speaker row is written
