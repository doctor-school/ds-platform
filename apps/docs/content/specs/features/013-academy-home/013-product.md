...
The canvas is **not yet vendored**: Claude Design canvas documents are not exposed through the project's DesignSync file listing, so the `design-source/` copy is pending an owner-side export handover. Implementation may not begin until the canvas is vendored into `design-source/` and referenced here (AGENTS.md §6 — UI design is approved before it is built), and every canvas-carried resolution (feed placement and card count, final lead-form field set, per-breakpoint composition) is read off the vendored copy — never off this PRD's placeholders.
