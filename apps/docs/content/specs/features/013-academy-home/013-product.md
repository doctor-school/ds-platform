...
- The stub's own scope lives in its six stub stories, [`013-interim-stub-stories.md`](./013-interim-stub-stories.md), and in the git history of the INTERIM triplet (spec PR [#1315](https://github.com/doctor-school/ds-platform/pull/1315), delivery PR [#1318](https://github.com/doctor-school/ds-platform/pull/1318)). The triplet files in this directory — `013-requirements-en.md` / `013-requirements-ru.md` / `013-design.md` / `013-scenarios.feature` — now carry the **full home** formalized from this PRD, having superseded the interim text under [#1324](https://github.com/doctor-school/ds-platform/issues/1324).
...
- **US-10** — As an **authenticated user**, logging in takes me back to the page I was trying to open, and to `/webinars` when there is nothing to return to — the landing is an entry point for visitors and never a detour on my way to my events. (Owner rule 2026-08-17, [#1326](https://github.com/doctor-school/ds-platform/issues/1326): return-to-origin platform-wide, `/webinars` as the default only.)
...
2. Logging in from anywhere → the doctor arrives at the page they were consuming, or at `/webinars` when there is none. **This requires a change 013 owns, not a property it inherits — and the regression is already live.** The login page sends a successful login to `/` (`apps/portal/lib/registration-resume.ts` → `DEFAULT_LANDING = "/"`), and the old `/` → `/webinars` redirect is **already gone** — `apps/portal/app/page.tsx` serves the interim Academy stub — so a doctor completing login today already lands on the marketing stub, the exact regression US-10 forbids. Feature 008 **EARS-7 still pins that target to `/`**, so 013 re-points the default and records the change as an amendment of 008 (008 is live in production).
...
- **The post-login landing works by an in-scope change of this feature, not by inheritance — and it is a live defect today.** The `/` → `/webinars` redirect is already gone while the login default is still `/`, so 013 re-points the post-login target **from `/` to the visitor's return target, with `/webinars` as the default**: the login redirect default (`DEFAULT_LANDING` in `apps/portal/lib/registration-resume.ts`) plus every shipped assertion pinning the old route (`registration-resume.test.ts`, `login/page.test.tsx`, `verify/page.test.tsx`, the 008-owned `e2e/shell/post-login-landing.spec.ts` with its steps, and 008 verification row 7), and an **amendment** of feature 008's EARS-7 landing target — an amendment rather than an inline rewrite because 008 is live in production. Because the defect is already in production, this fix is independently shippable and does not wait for the landing build. A doctor landing on the marketing page after login is the failure this criterion exists to prevent.
...
The canvas is **vendored verbatim** at [`design-source/academy-home.dc.html`](../../../../../../design-source/academy-home.dc.html) (pulled 2026-08-13 via DesignSync from project `8cc2f39a`, canvas file `academy-home.dc.html`; the canvas prop default `variant: v` matches the owner's pick). That copy is the fidelity spec 013 builds against (AGENTS.md §6 — UI design is approved before it is built): every canvas-carried resolution (feed placement and card count, final lead-form field set, per-breakpoint composition) is read off the vendored copy — never off this PRD's placeholders.
