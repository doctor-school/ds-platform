...
**Idempotency.** The re-run guard is the LD-1 partial unique index on `(event_id, kind) WHERE deleted_at IS NULL` plus the eligibility read that precedes every command — a re-run of the same manifest produces `skipped` on every row and writes nothing, and a partially failed run is safely resumed by re-running the whole manifest. The `Idempotency-Key` is fresh per attempt, not derived from `(event, kind)`: it fences one command against a concurrent or crashed attempt, exactly as it does over HTTP, and never replays a stored outcome — a deterministic key would also fence the legitimate second command of the same slot (the «publish the existing draft» row above) and silently write nothing. The driver never writes `live_at`, `starts_at`, `state` or `origin`; the honest timestamps are the ones the room recorded.

**Not decided here.** A `published` platform event whose date has passed and which never went live is refused, not repaired: its exit is the `published → hidden` edge owned by [#1814](https://github.com/doctor-school/ds-platform/issues/1814). The delivery form of the run is the owner’s Stage-A choice recorded in the requirements Scope (2026-09-05, «1»): **an operator CLI in `apps/api`** — a `tsx` script under `apps/api/scripts/` in the pattern of the existing `reconcile:sweep` / `seed:events` commands, taking the manifest path and a dry-run flag. It boots the HTTP-less Nest application context and calls the `AttachRecording` / `PublishRecording` handlers of the recording service **in-process** — the same handlers that serve the admin routes, with no HTTP hop, no admin session and no second write path. Audit attribution follows feature 010’s request-less door: the driver opens the audit context itself with `actorSub` = the `platform_admin` named on the command line and `source: system:recordings-backfill`, so every captured row names the operator, never `db-direct`; the per-row `Idempotency-Key` is a fresh lease per attempt, exactly as over HTTP (see «Idempotency» above) — the re-run guard is the LD-1 index plus the eligibility read, never a replayed outcome. No admin action, no admin route; the §3 diagram and §11 error set are unchanged by that choice.
...
No Delete route exists for any 014 resource. `/v1/public/events` and `/v1/public/events/:idOrSlug` are extensions of the existing feature-004 controllers, not new ones; `/v1/me/events` extends the existing feature-005 controller and keeps that feature's `doctor_guest` classification, so the endpoint-authz matrix carries one wording for it rather than two. EARS-29 adds no row to this table: the backfill calls, in-process, the same `AttachRecording` / `PublishRecording` handlers that serve `POST …/recordings` and `POST …/recordings/:rid/publish`, and its entry point is the operator CLI of §3.2 (owner Stage-A choice, 2026-09-05) — not an admin route and not an HTTP client of those routes. The three legacy routes extend the feature-007 admin controller: `/v1/admin/legacy-broadcasts` is the «Архивный эфир» creation entry (EARS-24), and `/v1/admin/events/:id/archive-legacy` / `…/hide-legacy` carry the two commands of the legacy machine (EARS-25).
