...
- **How far back «Мои события» reaches, and tab vs. one merged list** — **decided:** the **full** registration history — «Предстоящие» nearest-first, «Записи» newest-first — inside the **two canvas tabs** «Предстоящие» / «Записи» as `my-events.dc.html` draws them — not one merged list and not a bounded window. Deciding clauses: EARS-9 and lead decision LD-9 (`GET /v1/me/events` gains `tab=upcoming|recordings` over the existing registrations join).
