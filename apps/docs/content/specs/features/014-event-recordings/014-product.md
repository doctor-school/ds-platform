...
- **US-20** — As a **content operator**, I add an эфир that was held before the platform existed by checking «Это архивный эфир» on the ordinary create-event form — title, date held, duration, speakers, recording — and it stays invisible to everyone until I archive it, so the archive has content from day one.
...
1. Operator opens the ordinary create-event form, checks «Это архивный эфир», and fills in the title, the date it was held, its duration, its speakers and its recording.
...
- **Presentation of the secondary recording** — **decided** by the approved canvas: all three design-brief forms (spoiler / tabs / bottom section) are encoded as the `secondaryUi` prop of `event-page-recording.dc.html`, and its default **`spoiler`** is the operative pick, written into the requirements' scope as `secondaryUi: spoiler`.
...
- **How far back «Мои события» reaches, and tab vs. one merged list** — **decided:** the **full** registration history — «Предстоящие» nearest-first, «Записи» newest-first — inside the **two canvas tabs** «Предстоящие» / «Записи» as `account-my-events.dc.html` draws them — not one merged list and not a bounded window. Deciding clauses: EARS-9 and lead decision LD-9 (`GET /v1/me/events` gains `tab=upcoming|recordings` over the existing registrations join).
...
- **Upcoming-tab wording.** `academy-project.dc.html` and `academy-expert.dc.html` label their pair «Будущие · N | Прошедшие · N»; `account-my-events.dc.html` says «Предстоящие». The tab mechanism is identical, so this is copy, not a composition fork: `/webinars` and «Мои события» use **«Предстоящие»**; the 015/016 pages carry «Будущие» as their own Stage-A item.
- **The admin composition — decided, option B.** The recordings panel and the legacy-broadcast lifecycle commands («Архивировать» / «Скрыть», US-20, US-22) have **no canvas by design** (the neo-brutalist set is portal-facing; admin is stock Refine, ADR-0004 §3). Three compositions were put to the Product Lead — A: a section on the existing single detail page · B: an own «Записи» tab in a tabbed event detail, mirroring the 012 pick · C: a separate Refine resource with an event picker — with the lead recommending **B** for consistency with the 012 admin decision (#1282, 2026-08-17: option B tabbed detail, modal confirmations, retired rows hidden by default, no Delete anywhere). **Decision (Product Lead, working chat 2026-08-17, verbatim «Вариант B.»): option B** — an own «Записи» tab in the tabbed event detail; the lifecycle bar renders only the commands of the event's own machine (platform or legacy), each shown only when applicable, with a modal confirmation. Recorded on #1337; the mark-ended command that composition originally covered was withdrawn 2026-09-02 (#1741).
