...
- **Presentation of the secondary recording** — resolved at Stage A within the approved canvas: all three forms from the design brief (spoiler / tabs / bottom section) are encoded as the canvas `secondaryUi` prop, canvas default `spoiler` = operative pick (see Approved mockup); open only insofar as the owner may override the default before implementation.
...
- **The form of the «Прошедшие» tab.** The facets' form is resolved at Stage A by the vendored canvas «ФильтрЭфиров» (see Approved mockup); the tab's own presentation — tab versus toggle, and whether tab + facet selection survive in the URL — is carried by no canvas and still needs its Stage-A resolution before the listing surface is built.
- **Facet behavior at the edges** — the canvas shows per-option counts and keeps zero-count options visible and clickable (count simply absent), and its single-select-per-facet model implies AND across kinds; whether the product keeps zero-yield values clickable (leading to an empty result) or disables them is unconfirmed.
...
The second surface — the project/expert/topic **facets** on `/webinars` (design brief section 5) — is resolved by canvas **«ФильтрЭфиров»**, **vendored verbatim** at [`design-source/events-filter.dc.html`](../../../../../../design-source/events-filter.dc.html) (pulled 2026-08-13, canvas file «ФильтрЭфиров.dc.html»): three searchable dropdown facets (Проект / Эксперт / Тема) with per-option counts, single-select per facet, active-filter badges with per-badge clear and «Сбросить всё», stacking to full-width rows on mobile — built as a reusable controlled unit (props `project`/`expert`/`topic` + `onChange`, injectable option lists and counts), which is exactly the shape the shared event-list unit consumes. The **«Прошедшие» tab's own presentation** (tab vs toggle, URL persistence) is **not** carried by any canvas and remains an open question below — it needs its resolution before the listing surface is built.
