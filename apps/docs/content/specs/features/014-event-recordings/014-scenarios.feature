...
Feature: A finished broadcast keeps its value as a recording, and the archive is browsable
  ...
  Rule: A guest discovers a past broadcast through the public archive

    @EARS-11 @core @happy
    Scenario: A guest follows the golden archive card to its matching past event page
  ...
  Rule: Recording lifecycle, playback and remaining archive contracts
