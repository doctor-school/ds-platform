...
**The catalog** — canvas **«Проекты»**, vendored verbatim at [`design-source/academy-projects.dc.html`](../../../../../../design-source/academy-projects.dc.html) (pulled 2026-08-13 via DesignSync from project `8cc2f39a`, canvas file «academy-projects.dc.html»). This canvas carries **no composition fork** — a single composition, recorded as such: blue poster header with the scale line, a `repeat(auto-fill,minmax(min(300px,100%),1fr))` card grid, and the closing CTA plate whose both actions point at the landing's lead form.

**The card** — canvas **«ПроектКарточка»**, vendored verbatim at [`design-source/unit-project-card.dc.html`](../../../../../../design-source/unit-project-card.dc.html) (`dc-import`ed by the catalog canvas, vendored with it per the vendor-every-rendered-canvas rule). It is a reusable unit with injected props (`kind`, `title`, `description`, `count`, `partnerName`, `emblem`, `href`, `dark`); the partner attribution is conditional on `partnerName` being non-empty, and the cover plate falls back to a two-letter emblem derived from the title.

**The project page** — canvas **«Проект»**, vendored verbatim at [`design-source/academy-project.dc.html`](../../../../../../design-source/academy-project.dc.html) (canvas file «academy-project.dc.html»). It carries **two composition forks as canvas props, and the canvas default of each is the operative Stage-A pick** _(carried by the canvas default, matching the 013/014 pattern — no separate owner verbatim quote exists for these forks)_:
