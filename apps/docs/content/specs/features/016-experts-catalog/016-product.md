...
**The catalog** — canvas **«Эксперты»**, **vendored verbatim** at [`design-source/academy-experts.dc.html`](../../../../../../design-source/academy-experts.dc.html) (pulled 2026-08-13 via DesignSync, canvas file «academy-experts.dc.html»). It carries **no composition fork — a single composition**, recorded as such: blue poster header («Врачи, которые ведут эфиры» + a stat block), the name-search input and the single-select «Проект» chip row («Все» default), the card grid `repeat(auto-fill, minmax(min(240px,100%),1fr))` rendering `ЭкспертКарточка` via `dc-import`, the count note «Показаны N из M», the dashed two-case empty state with its reset, and the closing «Хотите вести эфиры? → Стать экспертом» plate.

**The expert page** — canvas **«Эксперт»**, **vendored verbatim** at [`design-source/academy-expert.dc.html`](../../../../../../design-source/academy-expert.dc.html) (pulled 2026-08-13, canvas file «academy-expert.dc.html»). Its composition fork is carried by a canvas prop, and the **canvas default is the operative Stage-A pick**:
...
**The expert card** is the **shared unit** — canvas **«ЭкспертКарточка»**, already vendored at [`design-source/unit-expert-card.dc.html`](../../../../../../design-source/unit-expert-card.dc.html) for feature 013 (the home page `dc-import`s it) and **reused here unchanged**: the catalog grid, the home page's expert grid and the project page's team grid are three instances of one card, not three cards. That sharing is the point — a change to how a person is presented happens once.
