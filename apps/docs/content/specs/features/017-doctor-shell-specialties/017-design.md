...
`apps/doctor` is a new Next.js 15 application (ADR-0015 §2). Its route-group layout mounts the **shared storefront-shell package** with a `host: doctor` config object — the host supplies VALUES only, never its own header/footer/theme implementation; every doctor-facing route of 017 and of features 018–021 renders inside it. There is no second header, no per-page footer and no host-local shell re-implementation.
...
**Shell composition** (`ds-shell.dc.html`, `host=doctor` host-config map):

| Slot                                           | Guest                                                                                                               | Signed in                       | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BBM topbar                                     | «BBM: Академия смыслов»                                                                                             | same                            | host-agnostic — renders on both `doctor` and `academy` hosts (owner product decision, 2026-09-10)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Logo                                           | present                                                                                                             | present                         | links to `/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Nav                                            | «Эфиры»                                                                                                             | same                            | release-1 set; grows as further sections ship (owner, 2026-09-10) — no «Проекты» on this host                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Search slot                                    | reserved, **empty**                                                                                                 | reserved, **empty**             | LD-6 — the input lands with the feature that owns the results surface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Theme control                                  | present                                                                                                             | present                         | design-system primitive, ships inside the shell package                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Action cluster (auth-cluster host-config slot) | «Войти» + «Регистрация»                                                                                             | points plate + «Личный кабинет» | EARS-1 — exactly one cluster renders; the branch is server-resolved, not flickered. «Личный кабинет» opens the doctor-host `/account`, which in release 1 is a **projection of the 003 «Account profile v1» surface** through the shared account-profile block — not a doctor-specific cabinet; feature 022 ([#1791](https://github.com/doctor-school/ds-platform/issues/1791), R5) replaces the projection with the full doctor cabinet; #2027 later swaps the slot's filler for the `packages/auth-flow` cluster _[Amended 2026-09-14 — the guest cluster is ONE «Войти / Регистрация» control. See the Amendment at the end of this document.]_ |
| Footer                                         | logo + host foot-note («Бесплатное образование для врачей.» + «© Doctor.School, 2026»), «Разделы» column (from nav) | same                            | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Footer col.                                    | «Документы и контакты»                                                                                              | same                            | agreement, PD policy, contacts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Footer cross-link                              | «Экспертам и партнёрам» → «Academy.Doctor.School ↗»                                                                 | same                            | EARS-12 / LD-4 — the single Academy crossing on this surface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Footer wordmark                                | giant «Doctor.School»                                                                                               | same                            | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
## Amendment — 2026-09-14: one guest control «Войти / Регистрация» → `/login` (source: canvas `ds-shell.dc.html` line 220 `guestCluster.primary`, 017 US-7, owner Stage-B round 1 on PR [#2198](https://github.com/doctor-school/ds-platform/pull/2198))

> **Status:** feature 017 is live in production, so this is recorded as an amendment rather than an inline rewrite (AGENTS.md §6). Everything above remains the decision as originally taken. The requirements-side counterpart is [`017-requirements-en.md`](./017-requirements-en.md) → «Amendment — 2026-09-14», with the identical contract.

**The amendment.** The auth-cluster host-config slot's **guest** filling is a single control labelled «Войти / Регистрация», not a «Войти» + «Регистрация» pair — the shape the vendored canvas draws (line 220, `guestCluster.primary`) and the one **US-7** asked for; the pre-amendment EARS-1 pair contradicted US-7 and is retired in its favour. The control routes to `/login`, the shipped sign-in surface, which links on to `/register`; the combined `auth.dc.html` surface is tracked as [#2080](https://github.com/doctor-school/ds-platform/issues/2080).

**What does not change.** «Exactly one cluster renders, server-resolved and never flickered» stands unchanged — this amendment changes what the guest cluster contains, not the branch. The signed-in cluster (points plate plus «Личный кабинет» → the doctor-host `/account`), the release-1 projection of the 003 account surface, feature 022's later replacement ([#1791](https://github.com/doctor-school/ds-platform/issues/1791)) and [#2027](https://github.com/doctor-school/ds-platform/issues/2027)'s swap of the slot filler for the `packages/auth-flow` cluster all stand as written.
