...
| Action cluster | «Войти» + «Регистрация»    | points plate + «Личный кабинет» | EARS-1 — exactly one cluster renders; the branch is server-resolved, not flickered. «Личный кабинет» opens the doctor-host `/account`, which in release 1 is a **projection of the 003 «Account profile v1» surface** through the shared account-profile block — not a doctor-specific cabinet; feature 022 ([#1791](https://github.com/doctor-school/ds-platform/issues/1791), R5) replaces the projection with the full doctor cabinet |
