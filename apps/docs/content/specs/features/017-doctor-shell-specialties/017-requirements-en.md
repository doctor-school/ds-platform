...
- No «Применить» control exists in the admin application; every filter is applied on change.
...
| 16   | Playwright + axe                    | `apps/admin/e2e/taxonomy-lists.spec.ts`                                                             | Two-line desktop rows with no truncation; record cards below the mobile breakpoint and no horizontal table scroll in any state; no «Действия» column on a single-action list and the row click opening the record; the navigation entry reading «Связи специальностей».                                               |
| 17   | Playwright                          | `apps/admin/e2e/taxonomy-filters.spec.ts`                                                           | Text search applying after the debounce with no submit control; no «Применить» anywhere in the admin tree; active filters rendered as removable chips with «Сбросить всё»; removing the last chip restoring the unfiltered list without a reload.                                                                     |
| 18   | Playwright + Vitest e2e             | `apps/admin/e2e/taxonomy-record.spec.ts`, `apps/api/test/taxonomy/adjacency.e2e-spec.ts`            | No tab bar on a single-tab record; «Вид связи» served from the closed enum with RU labels and explanation lines, a non-member value refused at the API; «Вес» and the page address absent from the rendered DOM of list, record and create; status chips on semantic tint tokens, readable over the row hover ground. |
| 19   | Playwright + Vitest e2e             | `apps/admin/e2e/taxonomy-specialties.spec.ts`, `apps/api/test/storefront/specialties.e2e-spec.ts`   | The Минздрав list rendering every seeded entry with no create, edit, delete or import control present in the DOM; a write attempt against the book refused at the API.                                                                                                                                                |
| 20   | Playwright + tree scan              | `apps/admin/e2e/taxonomy-lists.spec.ts`, `apps/admin/e2e/admin-sections.spec.ts`                    | Each admin section rendering the shared blocks rather than a section-local list, filter or chip; seed and e2e fixtures carrying realistic Russian titles with real transliterated addresses and no technical placeholder rows.                                                                                        |
