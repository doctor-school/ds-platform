...
- **Canvas:** [`design-source/doctor-feed.dc.html`](../../../../../../design-source/doctor-feed.dc.html) — screen `#d-feed`, drawn by the owner from the screen prompt [`02-d-feed-ru.md`](../../product/two-site-ia/design-prompts-ru/02-d-feed-ru.md); the feed's composition drawn from scratch on the tokens of `ds-foundation.dc.html`; the shell is taken from the `#d-home` canvas and not redrawn.
- **Units reused as-is:** the storefront shell (from `#d-home`), the event card (`unit-event-card.dc.html`, with the sign-up counter always visible), the expert card (`unit-expert-card.dc.html`) where mentorship shows a person, and the points plate in its base form (its anatomy is decided on the `#d-lesson` canvas).
