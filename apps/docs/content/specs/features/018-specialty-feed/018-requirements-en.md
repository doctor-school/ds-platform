...
>
> PRD source: [`018-product.md`](./018-product.md) (US-1…US-15). Epic: [Two-site IA — product brief](../../product/two-site-ia/brief.md). 018 is the **second feature of wave 1** and consumes the exact landed 017 child artifacts: shell [#1478](https://github.com/doctor-school/ds-platform/issues/1478), specialty choice [#1482](https://github.com/doctor-school/ds-platform/issues/1482), and targeting/adjacency [#1484](https://github.com/doctor-school/ds-platform/issues/1484). It does not wait for every 017 child to complete. It puts a screen in front of a doctor, so `surface: user-facing`.
...
- The **nearest-events block** at the top of the feed per F-018-1: the reused event card unit from `design-source/unit-event-card.dc.html` with the sign-up counter visible at all times, the offline colleagues' meet-up rendered as a **format of event**, and «Все события по специальности →» into feature 019.
...
- The **«Зачем это мне» value block** — a new specialty, an international placement, mentorship with an expert, a document at the end of a course — closing on «всё бесплатно для врача» and naming no financier.
...
- The **doctor content card** as a `@ds/design-system` unit in Stage-A variant **Б**, with the state set normal / hover / focus / started (progress) / completed / gate / «скоро», exported for 023, 024, 025 and the home page's selections; event surfaces use Feature 004's separate `WebinarCard`.
...
- **The «what is new since last time» affordance of PRD US-2** — the landing half of US-2 is delivered by EARS-1, the novelty half is **deferred**: no recency marker, no «новое с прошлого раза» badge, no last-visit state and no per-viewer read state ships in 018. It is per-viewer state, so it is a product decision rather than a rendering detail — it would also be the first per-doctor differentiator on a feed LD-1 keeps identical for every doctor of the specialty. Tracked as **[#1513](https://github.com/doctor-school/ds-platform/issues/1513)**, blocked by the feed route; no 018 clause claims `realizes: US-2`.
- **The legal marking of partner material** — CON-8 / CON-14 assign the marking obligation (legal qualification, responsible party, ERID, ОРД) to features **040** / **024**. 018 carries no partner material: the value block is editorial copy, so it renders no marking and models no marking field.

## Constraints

- **Only the consumed 017 artifacts must land first.** 018 consumes shell [#1478](https://github.com/doctor-school/ds-platform/issues/1478), specialty choice [#1482](https://github.com/doctor-school/ds-platform/issues/1482), and targeting/adjacency [#1484](https://github.com/doctor-school/ds-platform/issues/1484); completion of the whole 017 parent is not a blanket gate. No 018 clause may re-implement the header, navigation, footer, catalog, or managed targeting, and none may be satisfied by building inside `apps/promo` or `apps/portal`.
- **No financing statement, anywhere.** The interface states that learning is free for the doctor; it never names who pays, and it carries no partner marking of its own (the marking obligation of CON-8 / CON-14 belongs to features 040 / 024, and 018 hosts no partner material). The vendored canvas's «обучение оплачивают партнёры платформы» is a **known canvas defect that is not built** (LD-9).
...
- **Feature 019:** owns the events feed the events block links to and widens Feature 004's shared `WebinarCard`; it does not consume 018's doctor content card. **Features 023 / 024 / 025** own the school screen, the learning module and the lesson screen, and consume the doctor content card unit 018 delivers.
...
- **LD-9 — the canvas payer line is a known defect and is not built.** The operative copy is «всё бесплатно для врача», and nothing else — 018 renders no partner marking beside it: CON-8 / CON-14 scope the legal marking of partner material (its qualification, responsible party, ERID and ОРД transfer) to features **040** / **024**, and the editorial value block of 018 carries no partner material to mark. A «Партнёр» label rendered next to «всё бесплатно для врача» would also reconstruct «бесплатно за счёт партнёров» by juxtaposition — the exact implication this decision exists to prevent. The canvas's «обучение оплачивают партнёры платформы» is not implemented in any state, any breakpoint or any theme, and its owner-side canvas fix is tracked in the PRD's open questions. Where the canvas and this spec disagree on that line, this spec wins.
...
- **EARS-2** _(realizes: US-15, US-3)_ — The platform shall deliver the **doctor content card** as one unit in `@ds/design-system`, built in Stage-A variant **Б** — kicker, title, metadata and counter, with no cover-image slot — exposing the states normal, hover, focus, started with its progress, completed, gate and «скоро», and shall have the doctor-content blocks of 018 and every later doctor-content consumer (features 023, 024, 025 and the home-page selections) compose that unit unchanged; event surfaces, including 019, shall instead compose Feature 004's `WebinarCard`, and a screen-local re-implementation of either card shall be a defect.
...
| 7    | Playwright + full-text scan         | `apps/doctor/e2e/feed-value-block.spec.ts`                                                                | Four value items rendered; the block closes on «всё бесплатно для врача» with no marking or attribution beside it; a scan of every rendered state × breakpoint × theme finds no payer statement and not the canvas line.             |
...
- **018 consumes exact landed 017 children, not parent completion.** Shell [#1478](https://github.com/doctor-school/ds-platform/issues/1478), specialty choice [#1482](https://github.com/doctor-school/ds-platform/issues/1482), and managed targeting/adjacency [#1484](https://github.com/doctor-school/ds-platform/issues/1484) are the prerequisites; no 018 clause may re-implement them or seed adjacency by hand.
...
- **EARS-4 waits for its feed integration and real destination.** Route composition [#1496](https://github.com/doctor-school/ds-platform/issues/1496), the widened Feature-004 `WebinarCard` [#1517](https://github.com/doctor-school/ds-platform/issues/1517), and product-complete `/events` [#1516](https://github.com/doctor-school/ds-platform/issues/1516) must exist before events render first with a working «Все события по специальности →» link. No blank or placeholder route satisfies the destination.
- **Features 023 / 024 and the home-page selections consume EARS-2's doctor content card unit** and must not re-define it. Events, including Feature 019, use Feature 004's separate `WebinarCard`.
...
- **How a returning doctor sees what is new since the last visit (PRD US-2)** — deferred out of 018 and tracked at [#1513](https://github.com/doctor-school/ds-platform/issues/1513). The definition of «new», where the last-visit state lives, what an anonymous reader sees and how a per-viewer marker squares with the identical-for-everyone order of LD-1 are all owner decisions; 018 ships the return screen without a novelty affordance rather than inventing one.
