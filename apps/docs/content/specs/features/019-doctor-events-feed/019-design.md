...
| Day-group listing composition                        | `apps/portal/components/discovery-listing.tsx`                                                                                                                                                                                    | Extract the route-independent day-group renderer to `packages/design-system/src/blocks/event-listing.tsx`; Portal and Doctor adopt it in the same implementation slice                                                                                             | Doctor owns placement, headings/copy and route links only                                                                                                                      |
...
1. **Cross-front inventory and route-independent substrates:** before code, verify and update §1.1, then make each touched row true in the same slice (ADR-0013 A1). EARS-2 / #1517 widens `WebinarCard` at its canonical owner; EARS-7 / #1522 builds `events-filter` at its named shared target; the listing slice extracts `discovery-listing` into `event-listing` and migrates Portal and Doctor together. Neither substrate Issue mounts or publishes `/events`.
