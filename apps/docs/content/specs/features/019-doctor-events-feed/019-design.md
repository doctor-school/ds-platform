...
| Event list / day-group composition                   | `packages/design-system/src/blocks/event-list.tsx` (+ `day-agenda.tsx`) — the shared controlled unit landed (#1346, PR #1625); `apps/portal/components/discovery-listing.tsx` is the Academy host projection over it                                                                                                                          | Canonical: `packages/design-system/src/blocks/event-list.tsx` + `day-agenda.tsx` — landed (#1346); 019 consumes or additively widens that same unit                                                                                                                            | Doctor owns placement, day headings/copy and route links only; #1516 mounts the landed unit rather than creating a rival list                                                                                                                                                                           |
| Event card                                           | `packages/design-system/src/primitives/webinar-card.tsx` — widened to the 019 format vocabulary (offline city, seats) — landed (#1517, PR #1711)                                                                                                                                                                                              | Keep `WebinarCard` in `@ds/design-system`; any further format/state widening stays additive at that canonical owner                                                                                                                                                            | Doctor supplies data and destination, not JSX fork                                                                                                                                                                                                                                                      |
...
| Facets                                               | `packages/design-system/src/blocks/events-filter.tsx` — the canonical reusable block with the full REQ-138 facet set — landed (#1522, PR #1717)                                                                                                                                                                                               | Keep `events-filter` canonical in `@ds/design-system`; the shared query vocabulary is extracted to `event-listing-query.schema.ts` (#1523, open)                                                                                                                               | Doctor mounts `full`; later hosts mount another declared fill state, never fork controls                                                                                                                                                                                                                |
| LIVE resolution and room entry                       | `apps/api/src/room/room.service.ts`, `room.repository.ts`; `apps/api/src/registration/registration.service.ts`; `packages/schemas/src/events/room.schema.ts`, `registration.schema.ts`; `apps/portal/app/webinars/[slug]/room/**`                                                                                                             | Keep room truth/entry policy in `RoomService` + `RegistrationService` and portable contracts in `packages/schemas`; extract only a reusable presentation strip to `packages/design-system/src/blocks/live-event-strip.tsx` when the second consumer is implemented             | Doctor reads/adapts canonical room state; a registered doctor enters the room on the doctor storefront host (`apps/doctor`), unregistered readers go to 020; Doctor creates no room, room engine or LIVE state machine                                                                                  |
| Live room UI                                         | `apps/portal/app/webinars/[slug]/room/**` (`page.tsx`, `room-view.tsx`, `room-header.tsx`, `room-chat.tsx`, `room-presence.tsx`, `presence-heartbeat.tsx`, `presence-channel.ts`, `display-name-prompt.tsx`) over the presentational primitive `packages/design-system/src/primitives/webinar-room.tsx` — the composition is host-local today | Extract the composed room UI into a shared cross-front unit under `packages/` (design-system block + shared room client) over the unchanged 006 engine, tracked by #1722; `apps/portal` and `apps/doctor` then both mount thin host routes over that one unit                  | The doctor enters the room on the doctor storefront host (`apps/doctor`, `new.doctor.school`) through a thin route over the shared unit; room truth, lifecycle and entry policy stay in 006 (`RoomService` + `RegistrationService`) and Doctor builds no second room engine, resolver or forked room UI |
| Past/archive recording projection                    | Feature 014 owns `apps/api/src/recordings/RecordingsProjectionService` and `packages/schemas/src/recordings/`; Academy's `/webinars` past tab and the shared archive/`past` state of the controlled `EventList` landed (#1346, PR #1625)                                                                                                      | Keep publication/edited-over-raw truth in `RecordingsProjectionService`, schemas in `packages/schemas/src/recordings/`, and keep the controlled `EventList` archive state canonical in `packages/design-system/src/blocks/event-list.tsx` — landed through Feature 014 / #1346 | EARS-10 reads the 014 projection and renders the shared card/list state; it creates no Doctor recording projection, archive unit or playback policy                                                                                                                                                     |
