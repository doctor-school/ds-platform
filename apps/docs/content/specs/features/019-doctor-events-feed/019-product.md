...
The event card is Feature 004's **shared `WebinarCard` unit** [`design-source/webinar-card.dc.html`](../../../../../../design-source/webinar-card.dc.html) (`dc-import` «ВебинарКарточка»); Feature 004 / #1050 already owns `MonthCalendarGrid` and `MonthDotGrid`, while Feature 014 / #1346 owns the still-open shared `EventList` + archive state and Feature 019 owns the first canonical `events-filter` block. 019 waits for those owners, widens the shared card's format vocabulary and composes the units; it does not fork them, and #1485 does not first create the calendar.
