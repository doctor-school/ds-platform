...
The screen is a composition, not a new engine. The Academy already ships the listing/calendar/card precedents; Feature 014 / #1346 owns the still-open extraction of the controlled shared `EventList` and its archive state. Feature 019 waits for and consumes that canonical unit inside the doctor shell (017), then widens only the existing card's format vocabulary — an **offline colleagues' meet-up (Doctor Club)**, a **congress** and a **podcast broadcast** are first-class formats in the same feed, not sub-kinds of a webinar (REQ-15, REQ-2).

Owner decision **D-3** makes that reuse capability-wide, not merely visual: Academy and Doctor use one canonical event-query core, portable URL-state codec, calendar/card/listing blocks, room live-state and registration/entry lifecycle. Each storefront may wrap that core in a thin host projection for its own defaults, response envelope, route, copy, specialty targeting and composition. If a reusable part is still app-local in `apps/portal`, the 019 implementation extracts it to the named shared owner before `apps/doctor` consumes it. A second query engine, room, URL-state codec or live-state machine is not acceptable.
...
| --- | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F-1 | How the facet panel fits the full REQ-138 set | `panelVariant`: **А** — chip row above the feed · **Б** — left sidebar on desktop, sheet on mobile · **В** — view row + «Фильтры» button, facets inside a disclosure | **А — строка чипов**       | **Б — sidebar/sheet** — owner pick 2026-08-25, interactive walkthrough; verbatim: «Б — боковая панель (Рекомендуется)»                                                                                                                                                                 |
| F-2 | Relationship of week and month                | `layoutVariant`: **А** — one view at a time via a switch · **Б** — both at once (month as navigation, day feed as body)                                              | **А — переключатель вида** | **Б — both at once**, plus an owner condition: a dedicated calendar page analogous to the Academy one. Owner pick 2026-08-25, interactive walkthrough; verbatim (RU): «Б норм, но тогда надо предусмотреть отдельую страницу с календарём по анаогии с той, что мы сделали в Академии» |
| F-3 | Where «Идёт сейчас» lives                     | `liveVariant`: **А** — block above the feed · **Б** — pinned strip visible while scrolling · **В** — highlighted card inside today's day group                       | **А — блок над лентой**    | **А — block above the feed** — owner pick 2026-08-25, interactive walkthrough; verbatim: «А — блок над лентой (Рекомендуется)»                                                                                                                                                         |
...
The event card is Feature 004's **shared `WebinarCard` unit** [`design-source/unit-event-card.dc.html`](../../../../../../design-source/unit-event-card.dc.html) (`dc-import` «ВебинарКарточка»); Feature 004 / #1050 already owns `MonthCalendarGrid` and `MonthDotGrid`, while Feature 014 / #1346 owns the still-open shared `EventList` + archive state and Feature 019 owns the first canonical `events-filter` block. 019 waits for those owners, widens the shared card's format vocabulary and composes the units; it does not fork them, and #1485 does not first create the calendar.
