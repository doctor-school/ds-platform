...
- **Feature 017** owns the shell, specialty books, remembered choice, and managed targeting/adjacency delivered under #1484. **Feature 004** owns the listing engine and base `WebinarCard`; 019 widens that shared card's event-format vocabulary. **Feature 018** owns its doctor content card and rendered adjacent-areas block, neither of which is a substrate of this feed. **014** owns the recordings and **006** the room. 019 composes these artifacts and re-models none.
...
| ---- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Playwright + tree scan              | `apps/doctor/e2e/events-layout.spec.ts`                                                                    | Route renders inside 017's shell; breadcrumbs, title and tense row present and no «Неделя / Месяц» control rendered; the canvas composition order holds; the tree scan finds exactly one header/footer implementation and none owned by 019.                              |
...
| 3    | Vitest e2e + Playwright             | `apps/api/test/storefront/doctor-events-feed.e2e-spec.ts`, `apps/doctor/e2e/events-feed.spec.ts`           | Day grouping within the horizon; targeting and directed adjacency match 017's managed #1484 rows; an adjacency-less specialty yields no adjacent items; «показать ещё» extends the range in the URL; no ranking field in the response.                                    |
| 4    | Playwright                          | `apps/doctor/e2e/events-month-beside-feed.spec.ts`                                                         | Month grid and day feed visible together at 1440; «Сегодня» marked; a day with a live эфир marked; selecting a day changes the URL and moves the feed body without reloading the shell; no «Неделя / Месяц» switch and no view-switch-only rendering exists.              |
...
| 8    | Playwright + Vitest e2e             | `apps/doctor/e2e/events-url-state.spec.ts`, `apps/api/test/storefront/doctor-events-query.e2e-spec.ts`     | Tense, facets, day and horizon round-trip through the URL; a pasted link reproduces the screen in a fresh context; back moves between feed states; the 021 return lands on the exact prior URL with the action resumed.                                                   |
...
- **The acyclic same-WBS order is substrate → read → presentation/state → first publication.** Route-independent shared substrates [#1517](https://github.com/doctor-school/ds-platform/issues/1517) (`WebinarCard` format widening) and [#1522](https://github.com/doctor-school/ds-platform/issues/1522) (`events-filter`) precede read [#1518](https://github.com/doctor-school/ds-platform/issues/1518), which consumes #1517, #1522, and #1484. Calendar/presentation [#1519](https://github.com/doctor-school/ds-platform/issues/1519), URL state [#1523](https://github.com/doctor-school/ds-platform/issues/1523), and honest states [#1524](https://github.com/doctor-school/ds-platform/issues/1524) each consume #1518. Product-complete integration [#1516](https://github.com/doctor-school/ds-platform/issues/1516) then publishes `/events` inside #1478's shell, consuming #1519, #1523, #1524 and sequencing correction [#1620](https://github.com/doctor-school/ds-platform/issues/1620).
