...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Public event read                | `apps/api/src/events/events.service.ts`, `events.repository.ts`, `events.public.controller.ts`; `packages/schemas/src/events/public-page.schema.ts` — `PublicEventPageSchema`, an allow-list projection already carrying `recording: RecordingProjectionSchema`  | Widen **in place** at that same owner into `EventPageView` (LD-1): storefront, format block, CTA policy object, sign-up proof, med gate, «Вокруг события» links. No 020-local read model and no second projection.                                                | Each host route calls the one read and supplies only its envelope; `/v1/public/events/:slug` stays the Academy projection and the doctor storefront read is its sibling adapter, not a second engine.              |
| Page composition                 | `apps/portal/app/webinars/[slug]/page.tsx` with `register-one-tap.tsx`, `register-action.ts`, `recording-gate.tsx`, `recording-player.tsx` — the composition is host-local today                                                                                 | Extract the composed page into shared blocks at `packages/design-system/src/blocks/` (event page shell, sticky sign-up card, format block, speakers/«Ведёт» card) built to the variant-А canvas; both hosts mount the same unit (LD-1, LD-10).                    | `apps/doctor` and `apps/portal` supply route, header, envelope and copy defaults only. A second page composition in either app, or an `apps/doctor` → `apps/portal` import, fails EARS-1.                          |
| Participation CTA policy         | Implicit today: 004's one-CTA contract resolved inside the portal page and `register-one-tap.tsx`                                                                                                                                                                | New server-side resolver `ParticipationCta` in `apps/api/src/events/` with its portable contract in `packages/schemas/src/events/` (LD-2) — `register` · `registered` · `enter-room` · `switch-to-online` · `sold-out` · `unavailable`.                           | The client renders the given policy object and computes no eligibility. Any host-side branch on lifecycle, registration, format or seats is a defect.                                                              |
...
stateDiagram-v2
  ...
  Ended --> Archived: archived by 004 / 007
  ...
  state Archived {
    [*] --> ArchiveNotice: 004 EARS-5 public notice, not a 404 and not a redirect
  }
