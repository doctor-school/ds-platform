...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Public event read                | **Widened in place (slice 1, #1764).** `packages/schemas/src/events/public-page.schema.ts` now exports `EventPageViewSchema`, carrying `format` (`online`/`offline`/`hybrid`), `seatsLeft` and — since slice 1 of EARS-2 (#1765) — `links: AroundEvent`, the per-host «Вокруг события» set resolved by `apps/api/src/events/around-event.resolver.ts` from a route table each storefront controller owns (every key absent on both hosts today), beside the existing allow-list and `recording`; `PublicEventPageSchema`/`PublicEventPage` remain aliases of it, so feature 004's consumers are untouched. Storage: `participation_format` + `seats_left` on `events` (`packages/db/src/schema/events.ts`, additive migration, back-fill safe). | Same owner, further widening as its own EARS lands: the `FormatBlock` sub-block union (EARS-8, #1771), sign-up proof (#1767), med gate (#1774). Still no 020-local read model and no second projection.                                                                                                                                                                                                                                                                                                                                                                                                                   | **Realised.** `GET /v1/storefront/doctor/events/:idOrSlug` delegates to the one `EventsService.publicEventPage`; `020 EARS-1` in `apps/api/test/events/event-page-view.e2e-spec.ts` pins deep-equal bodies across both hosts, and no-auth reachability per state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Page composition                 | `apps/portal/app/webinars/[slug]/page.tsx` with `register-one-tap.tsx`, `register-action.ts`, `recording-gate.tsx`, `recording-player.tsx` — the composition is host-local today                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **Realised (slice 2, #1764).** The composed page is now four shared blocks at `packages/design-system/src/blocks/` — `EventPageShell`/`EventPageHero`, `EventSignupCard`, `EventSpeakerCard`, `EventFormatBlock` (online only; offline and hybrid arrive with EARS-8, [#1771](https://github.com/doctor-school/ds-platform/issues/1771)) — built to the variant-А canvas and exported from `blocks/index.ts`. This is the one canonical composition both hosts mount (LD-1, LD-10); Hosts re-mounted (slice 3, #1764): `apps/portal/app/webinars/[slug]/page.tsx`, `apps/doctor/app/(storefront)/events/[slug]/page.tsx`. | `apps/doctor` and `apps/portal` supply route, header, envelope and copy defaults only. A second page composition in either app, or an `apps/doctor` → `apps/portal` import, fails EARS-1.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Participation CTA policy         | **Server-resolved (slice 1, #1764).** One pure resolver `apps/api/src/events/participation-cta.resolver.ts` over lifecycle × registration × format × seats; portable contract `ParticipationCtaSchema` in `packages/schemas/src/events/participation.schema.ts` (LD-2) — `register` · `registered` · `enter-room` · `switch-to-online` · `sold-out` · `unavailable`, with RU label/reason defaults carried by the server.                                                                                                                                                                                                                                                                                                                       | Same owner. **Transport:** a per-viewer SIBLING read on each host — `GET /v1/public/events/:idOrSlug/participation` and `GET /v1/storefront/doctor/events/:idOrSlug/participation`, both `@Public()` with an OPTIONAL principal and `Cache-Control: private, no-store`. Not a field of the page body (that would break 004 EARS-1's guest/principal byte-identity and poison its shared cache), and not an extension of 005's authenticated `GET /v1/events/:idOrSlug/registration` (a guest there gets 401 instead of «Участвовать»).                                                                                    | The client renders the given policy object and computes no eligibility. Each host contributes only its route table (`eventPath` / `registrationEntry` / `roomPath`); on the doctor host `roomPath` resolves to `/events/[slug]/room` (impl slice 3 of [#1722](https://github.com/doctor-school/ds-platform/issues/1722)), on the Academy host to `/webinars/[slug]/room`; a `null` `roomPath` yields `enter-room` with `href: null` — absent, never dead (EARS-4). Any host-side branch on lifecycle, registration, format or seats is a defect. `apps/portal/lib/registration-handoff.ts` no longer builds a register href — the portal-local `buildRegistrationHref`/`resolvePrimaryCta` pair was deleted in slice 3 and both hosts consume this policy. |
...
| Room state and room entry        | `apps/api/src/room/room.service.ts`, `room.repository.ts`, `presence-*.ts`; `packages/schemas/src/events/room.schema.ts`; composed room UI `apps/portal/app/webinars/[slug]/room/**` over the primitive `packages/design-system/src/primitives/webinar-room.tsx`, whose extraction target is the shared package `packages/room` (`@ds/room`)                                                                                                                                                                                                                                                                                                                                                                                                    | Room truth, presence and entry policy stay at feature 006. The composed room **UI** is extracted to the shared cross-front package `packages/room` (`@ds/room`), tracked by [#1722](https://github.com/doctor-school/ds-platform/issues/1722); both hosts then mount thin routes — `/events/[slug]/room` on the doctor host, `/webinars/[slug]/room` on the Academy host.                                                                                                                                                                                                                                                 | The doctor enters on `doctor.school` through the `apps/doctor` thin route `/events/[slug]/room`, mounted at `apps/doctor/app/(room)/events/[slug]/room/` — a sibling root route group OUTSIDE 017's `(storefront)` shell, because the room is viewport-bounded (006 EARS-11) (LD-3); an Academy-host doctor through the equivalent `apps/portal` route `/webinars/[slug]/room`. 020 builds no room, presence mechanism or live-state machine.                                                                                                                                                                                                                                                                                                              |
...
sequenceDiagram
  ...
  participant RM as room route (apps/doctor /events/:slug/room over packages/room, #1722)
  ...
  P-->>API: cta { action: 'enter-room', href: /events/:slug/room on doctor.school, presenceCount }
  R-->>G: room entry with the presence count
  G->>RM: enters /events/:slug/room on doctor.school — never academy.doctor.school (LD-3)
```
...
5. **Room entry**, sequenced behind [#1722](https://github.com/doctor-school/ds-platform/issues/1722): the doctor-host thin route `/events/[slug]/room` (`apps/doctor/app/(room)/events/[slug]/room/`, outside 017's shell) over the extracted shared room unit `packages/room` (`@ds/room`). Before that extraction lands, a doctor-host entry route would be a second room composition, so EARS-7's entry ships with it rather than as a fork.
...
### 6.1 Room-entry resolution and the doctor auth/unregistered branch
