...
**The canvas.** The «Успех» artboard of [`design-source/auth.dc.html`](../../../../../../design-source/auth.dc.html) is no longer built by any route. The canvas file is not edited here — the artboard is retired in Claude Design by its owner and re-vendored through the normal DesignSync route (ADR-0013), never by hand-editing the vendored bytes.
