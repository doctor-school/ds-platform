...
| Sent / confirmed states  | The canvas's «письмо отправлено» / «почта подтверждена» states, plus the success block                                              | base canvas + the 021 success block | **[Amended 2026-09-17 — the post-confirmation success state is withdrawn on this host: a confirmation that yields a session navigates directly to the honoured destination. See Amendment — 2026-09-17 at the end of this file.]** |
...
The statement of a consent item is not a second copy blob standing beside the door: it is composed from the SAME resolved copy the door renders (`resolveAuthFlowCopy` → `consentStatementOf({ label, help })`, in the order the canvas draws them), so the sentence the doctor reads and the sentence the record stores are one value and cannot drift. The item names the exchange, not the composition of the shared data — that composition is disclosed in the policy text and by the platform manager. A wording change therefore happens in one place and moves one version with it: the host's `wordingVersion` and the server-stamped per-purpose constants are bumped together (`2026-09-22` for the partner-data and marketing wordings; the medical-worker declaration keeps `2026-09`, its words unchanged). Withdrawal is not a surface: the interface renders the manager-request statement and no toggle — a withdrawal reaches the store as a manager-side operation of feature 037.
...
5. **The honest-state surface** (EARS-12, EARS-13, EARS-14) — errors, the per-item post-submit statements of the unmet access conditions, the enumeration-safe already-registered path, the preserved failed submission. **[Amended 2026-09-22 — the submit is live in every state and there is no disabled-submit reason; an unmet mandatory condition is stated under the row it belongs to, after the press. See Amendment — 2026-09-22 at the end of this file.]**
...
## Amendment — 2026-09-17: the post-confirmation surface is a navigation, not a screen (source: [#2027](https://github.com/doctor-school/ds-platform/issues/2027), PR [#2239](https://github.com/doctor-school/ds-platform/pull/2239))

> **Status:** feature 021 is live in production (Витрина R1, `release-2026.09.08-1`), so this is recorded as an amendment rather than an inline rewrite (AGENTS.md §6). Everything above remains the design as originally taken, with the post-confirmation step read under this amendment. The requirements-side counterpart is [`021-requirements-en.md`](./021-requirements-en.md) / [`021-requirements-ru.md`](./021-requirements-ru.md) → «Amendment — 2026-09-17», with the identical contract and the owner's verbatim decision.

**The mechanism.** The confirm round trip is unchanged: the code goes to feature 003 with the carried target, the server re-validates that target through the shipped LD-3 guard and answers with the destination it honoured. What changes is what the host does with that answer — it **navigates** (a replacing navigation, so a spent code form is not reachable by Back) instead of painting a success screen with the same destination behind a button. This is the Academy `verify` mechanism, and the shared `packages/auth-flow` standard **S5** now pins both hosts to it.

**Which value is navigated to.** The destination comes from the **confirm response**, not from the client-side completion of the return target: the round trip is the only participant that re-validated the carried target and therefore the only one that knows a target went stale (ended / full / unpublished / missing) — the LD-8 branch of §3. A cold confirmation with no target in hand falls back to the LD-4 landing the door already resolved.

**What this removes from the design above.** §1's composition table no longer carries a post-confirmation success block; §2's sequence diagram's last hop (`success state: credited amount + profile-completion line + return action`) becomes a navigation to the honoured destination; §3's degraded branch (`Degraded --> [*]: nearest honest destination + stated reason`) keeps its destination and loses its _stated reason_, which had the success block as its only carrier — tracked as the OPEN Issue [#2272](https://github.com/doctor-school/ds-platform/issues/2272) rather than replaced by an invented notice; §6's credit column loses its surface entirely, so when feature 025 ships, the credited-as-fact statement needs a surface named by 025 rather than by 021. The form-side promise of §6 is untouched.

**The canvas.** The live [`design-source/auth.dc.html`](../../../../../../design-source/auth.dc.html) has no success screen: after the code is accepted it shows «Код принят — входим…» and the host navigates. The «Успех» screen exists only in [`design-source/archive/archive-auth-v1.dc.html`](../../../../../../design-source/archive/archive-auth-v1.dc.html) (archived 2026-09-15 with #2080); the vendored bytes were verified identical to the Claude Design project on 2026-09-17, so no canvas change accompanies this amendment.

## Amendment — 2026-09-22: the submit is live and the unmet condition is stated on its own row (source: [#2027](https://github.com/doctor-school/ds-platform/issues/2027), PR [#2338](https://github.com/doctor-school/ds-platform/pull/2338))

> **Status:** feature 021 is live in production (Витрина R1, `release-2026.09.08-1`), so this is recorded as an amendment rather than an inline rewrite (AGENTS.md §6). Everything above remains the design as originally taken, with the submit group read under this amendment. The requirements-side counterpart is [`021-requirements-en.md`](./021-requirements-en.md) / [`021-requirements-ru.md`](./021-requirements-ru.md) → «Amendment — 2026-09-22», with the identical contract and the owner's verbatim decision.

**The mechanism.** The owner's canvas `design-source/auth.dc.html` is the decision for this surface's rendering (submit at 490, the per-row warning lines at 193/201, the danger box border at 497, the `submitted` state at 369). The submit group carries **no reason line and no disabled state**: the button is drawn live in every state, the press runs the form's own validation, and an ungranted mandatory consent is reported **under its own row** — the catalog sentence, the control marked `aria-invalid` and pointing at that message through `aria-describedby`, the checkbox box painted in the danger tone while the statement stands. Granting a condition clears only its own statement, and the optional marketing row never reports. The command is not sent while any mandatory condition is ungranted.

**Where it lives.** In the shared `RegisterCard` block (`packages/design-system/src/blocks/register-card.tsx`) and the register door of `packages/auth-flow`, so both storefronts get it by construction — the host varies only the SET of consent rows, never how a row speaks. The `unmetPrecondition` prop and the `submitReason` test id are removed from the block's public surface; the catalog sentences survive as the per-row statements.

**The misconfigured host.** A mandatory condition the host words but its read model carries no statement for has no row to report on; that case is stated in the card's form-level error banner (canvas 56-61) and still sends no command, so a misconfiguration fails loudly at the door rather than silently at the server.
