...
**What this removes from the design above.** §1's composition table no longer carries a post-confirmation success block; §2's sequence diagram's last hop (`success state: credited amount + profile-completion line + return action`) becomes a navigation to the honoured destination; §3's degraded branch (`Degraded --> [*]: nearest honest destination + stated reason`) keeps its destination and loses its _stated reason_, which had the success block as its only carrier — recorded as decision debt rather than replaced by an invented notice; §6's credit column loses its surface entirely, so when feature 025 ships, the credited-as-fact statement needs a surface named by 025 rather than by 021. The form-side promise of §6 is untouched.
