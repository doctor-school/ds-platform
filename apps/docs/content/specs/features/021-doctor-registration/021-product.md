...
**Return to the point of interest** (owner edit R2 №1). Confirming the email returns the doctor to the lesson, эфир or ticket they came from — not to the account page. «To my account» stays a secondary action. A direct arrival with nothing to return to is a first-class state, not an edge case. _(Amended 2026-09-17: the doctor now lands on that point of interest **directly** — the post-confirmation success state and its secondary «в личный кабинет» action are withdrawn, the destination unchanged. See [`021-requirements-en.md`](./021-requirements-en.md) → Amendment — 2026-09-17.)_
...
- **Starting points are credited for registering**, and the success state names what completing the profile adds and what it unlocks (certificates, НМО). The values are fixed by the owner (2026-08-25): **+20 Pul for registration, +30 Pul for profile completion**; advance availability is a per-event configurable flag (REQ-49 record in Open questions). _(Amended 2026-09-17: the success state that would have carried the credit is withdrawn with the post-confirmation screen; the points promise lives on the form, and a credited statement needs a surface named by feature 025. See [`021-requirements-en.md`](./021-requirements-en.md) → Amendment — 2026-09-17.)_
...
| --- | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **How the mandatory consent block is presented** | **А** — flat checkbox list (as in the R4 layout): honest and compact, but three checkboxes in a row read as fine print. **Б** — two levels: access conditions (medical worker + partner data) framed above the button, the marketing opt-in separately below; separates the meanings, lengthens the form. **В** — expandable data composition: one consent line plus a «what exactly is shared» disclosure; shorter, but hides something material — risky for provability. | **А** (`consentVariant`)                     | **Б — two tiers** — owner pick 2026-08-25, interactive walkthrough; verbatim: «Б — два уровня (Рекомендуется)»                                                                                                                |
| 2   | **Where the return context lives**               | **А** — a line above the form («you will return to …»). **Б** — the left half of the split, with the card of the content the doctor is registering for; holds the intent harder, and the canvas draws its mobile treatment as a background plate above the form at 390.                                                                                                                                                                                                    | **А** (`returnVariant`)                      | **Б — split card, WITHOUT any back-navigation button on the card** — owner pick 2026-08-25, interactive walkthrough; verbatim (RU): «Вариант Б, но на карточке не должно быть кнопки, которая уводит назад - это странный UX» |
| 3   | **How starting points are shown**                | On the **success state** after email confirmation only — or **also promised on the form** («+N for registering»); the promise motivates but adds a reward element the R4 layout does not carry.                                                                                                                                                                                                                                                                            | **success state only** (`promisePoints` off) | **Promise on the form** («+N за регистрацию») — owner pick 2026-08-25, interactive walkthrough; verbatim (RU): «Обещание на форме». The value of N stays an open REQ-49 fork (accruals)                                       |
