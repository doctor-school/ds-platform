...
- The R1 list on **both** hosts holds exactly **one published entry — Политика персональных данных и согласия**; each 021 consent text and the photo/video consent is its own document page reached from the 021 registration checkboxes and from links inside the policy, never an extra index row (fork 2 of prompt 21, canvas-settled). «Лицензия на образовательную деятельность», «Пользовательское соглашение» and «Правила начисления очков», drawn on the canvases, are slice 2 (owner, 2026-09-06) and appear in neither list nor footer in R1.
- Every document is reachable at a **stable per-document URL** that does not change when its text is corrected; consent checkboxes and the Academy lead form link a **specific document**, never the index. The index page and its row unit are canvas-settled; the per-document URL itself is a design-level detail fixed by the triplet (`028-design.md`), not an open product question.
...
- **The licence block** ships in slice 2 — the «Лицензия на образовательную деятельность» documents-list row plus the licence number in the requisites line, presenting the licence itself, the issuing authority, a registry verification link and the scan. Its content is an **owner/lawyer input** (Q-31); the row is absent from both hosts until it is supplied, never a placeholder (owner, 2026-09-06: «Данных лицензии нет, не выводим её сейчас!»).
...
| Развилка 2 промпта 21 · разделение согласий | `doc`              | длинный — политика (с оглавлением) · короткий — текст согласия                                                             | Canvas default is the decision — each 021 consent text and the photo/video consent is its own document page in the short `doc` variant, reached from the registration checkboxes and from policy links, never an extra index row. |
...
- **Contacts are channels, no form** (`d-docs · контакты` / `a-docs · контакты`): `support@doctor.school` / `academy@doctor.school` as mailto, plus Telegram / ВКонтакте / RuTube chips. The doctor page carries a «клиникам и организациям → Academy.Doctor.School ↗» exit; `a-docs · выходы` carries the two CJM exits (Инвесторам → `#a-invest`, Экспертам → `#a-apply`).
- **The doctor documents list carries no exit to the Academy (decided).** The doctor canvas closes the list (`d-docs · документы`) with a caption naming the Academy documents page, and that caption is **not built**: the owner's live verdict is that the storefront keeps **exactly one** Academy link, the footer one (REQ-24), so a second exit from this page contradicts it. The Academy canvas draws no counterpart either. REQ-24 therefore stays at its two placements — the storefront footer link and «Стать экспертом» in the cabinet.
- **Requisites are one faint tabular line** at the foot of the page (`· реквизиты`): «<юрлицо> · ИНН … · Лицензия на образовательную деятельность № …». The canvas placeholder names another entity; the CONTENT is the owner decision recorded above (ООО «Ивекскон»), the LAYOUT is the canvas decision. In R1 the line reads «<юрлицо> · ИНН … · ОГРН … · <адрес>» only; the licence number joins it in slice 2 once Q-31 is settled.
- **The licence block is a documents-list row**, not a separate panel — plus the licence number in the requisites line. Canvas note (doctor): «На её основании Doctor.School выдаёт сертификаты и передаёт сведения в НМО»; canvas comment on the Academy screen: `Q-31: лицензиат и эмитент — сама платформа, без оговорок`. The Q-31 wording stays an owner/lawyer input; the layout is settled. Deferred to slice 2 (owner, 2026-09-06: «Данных лицензии нет, не выводим её сейчас!»).
...
- **Q-31 (owner + lawyer, open): who is the licensee / НМО provider.** Two entities exist in the legacy material — ООО «Ивекскон» (personal-data operator, full requisites) and АНО ДПО «Академия Доктор Скул (Школа)» (licence entity, requisites blank). The operating entity on the documents is decided (Ивекскон); **which entity is named as licensee on the published block is not**. Accepted **ADR-0016 §7** already records the partial answer — Doctor.School holds the educational licence and is the issuer of record — so the open part is narrower: the entity naming. This is an **owner-input dependency** of **slice 2** (the licence itself is out of slice 1), deliberately not modelled as a `blocked_by` edge.
...
- **Whether the licence scan is a file, an image or a registry deep link (slice 2)**, and whether a scan is published at all before the licensee question is settled — **not drawn on any canvas**: it is fork 3 of prompt 21, and the «Документ» canvas does not draw it either — its document footer is the requisites line only, with no licence number, issuer, «проверить в реестре» link or scan block.
