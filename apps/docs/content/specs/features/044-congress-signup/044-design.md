...
- **`registrations.intake_origin`** — `site` | `desk`, written by the intake use-case; `NULL` stays the platform-origin row. The card shows it; the roster does not.
...
**Production amendment (2026-09-25, #2377) — event-bound registrar.** The boundary above is the running-production baseline. After the amendment `event-registrar` is bound to one event through `event_role_grants` (EARS-38). The role check stays in the `Authz` decorator (Layer 1); the event check is a second, server-side step on every desk route, each of which names its event in the path — the roster, the participant card, the attendance mark and the manual registration. The step loads the principal's grant and refuses when the path's event is not the bound one, and for the card and the attendance mark it also refuses a registration that does not belong to that event, so a guessed registration id of another event reads nothing. A principal with the role and no grant row is refused on every route except the session endpoints; the event list and every other admin section stay refused. The admin reads the bound event id from its session projection and draws exactly one nav link — that event's roster — and still only as a projection of the server refusal. The platform administrator bypasses the event step. The `endpoint-authz` matrix gains the desk routes with the event-bound column.
