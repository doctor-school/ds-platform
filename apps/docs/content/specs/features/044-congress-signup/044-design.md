...
Evidence: <!-- LEAD: dev-stand check result -->

If the live check shows the configuration refuses a credential-less human, the fallback is not a password the participant never learns: it is an IdP-configuration change on our own instance, because a stored unknown credential would be an unusable secret on a real account and a recovery path nobody can drive. That fork is recorded here so it is decided with evidence rather than at implementation time.
...
The roster is an HTTP route over the existing in-process `eventRoster()` read model (`apps/api/src/registration/registration.service.ts:178-181`), widened from its current PII-free `(doctor, event, registeredAt)` fact to the answers the registrar needs. It renders on `AdminDataList` (`apps/admin/components/admin-data-list.tsx:62-371`), whose query state today carries `q`, `status`, `includeRetired`, `page` and `pageSize` and no sort field at all. 044 adds sort to that query state and to the server query — by surname and by registration time, both directions — composable with search and the new filters (specialty, city/region, confirmation-mail outcome). Because the sort is server-side, the printed sheet and the screen agree on ordering across pages.

The print view is the same filtered, sorted query rendered for paper: the browser's own print path plus a print stylesheet, which is the first `@media print` rule anywhere in `apps/admin` or `packages/design-system`. Nothing is exported to a file. The sheet's layout and the roster's column set and order are owner decisions, held as `PENDING owner Stage-A (#2287)` in the requirements rather than invented here.
