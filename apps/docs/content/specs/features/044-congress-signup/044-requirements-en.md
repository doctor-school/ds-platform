...
description: "Requirements for the congress sign-up vertical slice: a public unauthenticated intake endpoint reached same-origin from the congress site, a passwordless account created through the shared 003 engine, typed answers and consent stored on the event registration, a confirmation email whose outcome is recorded, and a restricted event-registrar role bound to one event with a roster and a registration desk in apps/admin — attendance marks per congress day, manual registration of walk-ins and a participant card (the desk extends the live roster by the 2026-09-25 amendment, #2377)."
...
prior_decisions:
  ...
  - "ADR-0013 — the admin roster, participant card and desk form are built from `@ds/design-system` primitives, tokens only"
...
>
> PRD source: [`044-product.md`](./044-product.md) (US-1…US-16). The intake form itself is hosted on the congress site, but this feature's own screens — the admin roster, the participant card and the manual-registration form — live in `apps/admin`, so `surface: user-facing`, matching the functional-map registry row's classification for 044.
...
This amendment overrides three decisions running in production: it narrows the read-only roster of EARS-24 to exactly two registration mutations — EARS-34 and EARS-35; it replaces the column order of EARS-25 with the set of EARS-37; and it extends the `event-registrar` refusal of EARS-19, and with it the roster route of EARS-18, by a binding of the role to one event (EARS-38). With them it overrides the deny-by-default invariant of that role, verification rows V-11 and V-13, the roster scenarios in `044-scenarios.feature` and the «Authorization boundary», «Roster projections» and «Data model» sections of `044-design.md`. The running-production baseline of those decisions is retained below; the new handlers live in work package #2377. Roster search (EARS-21) is unchanged. The printable attendance sheet (#2291) never shipped and is cancelled: «печатный лист #2291 отменяем - рабочей админки достаточно». The product owner decided on 2026-09-25:
...
- The organizing team sees exactly one thing in `apps/admin` — the roster of the one event its role is bound to, with a read-only participant card, per-day attendance marks and manual registration of walk-ins — through a new coarse role that is refused everywhere else.
...
- Normalisation of the submitted contact phone and the read-time «возможный дубль» derivation over it, surfaced on the roster and on the participant card.
...
- The `event-registrar` role in `apps/api/src/authz/authz.types.ts:18-27`, an HTTP route over `RegistrationService.eventRoster()` (`apps/api/src/registration/registration.service.ts:178-181` — no route today), the binding of that role to one event, role-aware admin nav, the roster screen, the participant card, the per-day attendance mark and the manual-registration form in `apps/admin`.
...
- **CSV / export** — owner, verbatim: «пока, наверное, этого не делаем». The roster in the admin is the v1 output.
...
**Commands** — `SubmitCongressSignUp` (public, unauthenticated), `RegisterForEvent` (existing, authenticated doctor), `ListEventRoster` (registrar, bound event), `OpenParticipantCard` (registrar, read), `MarkAttendance` (registrar, per congress day), `RegisterAtDesk` (registrar, the same intake with the «desk» origin).

**Events** — `CongressAccountCreated` (passwordless), `CongressRegistrationRecorded`, `CongressConsentCaptured`, `ConfirmationEmailDispatched` / `ConfirmationEmailFailed`, `AttendanceMarked`.

**Read models** — the event roster (registration answers joined to the registration fact, carrying the read-time «возможный дубль» marker and the per-day attendance), the participant card, the public specialties list.
...
- **EARS-19** (`realizes: US-8`) — THE SYSTEM SHALL refuse a principal holding only `event-registrar` on every API endpoint other than the roster route and the session endpoints needed to hold a session — including every create, update and delete endpoint of the admin surface.
- **EARS-20** (`realizes: US-8, US-16`) — THE SYSTEM SHALL render the admin navigation from the signed-in principal's role and event binding, showing a principal holding only `event-registrar` the roster entry of its bound event (EARS-38) alone and no other section or event link, and a principal holding the role with no binding row no roster entry at all; the client-side gate is a projection of the server refusal in EARS-19 and EARS-38, never its substitute.
...
| V-12 | Playwright / E2E — admin   | Registrar signs in, sees only the bound event's roster nav entry, reads the columns in the 2026-09-25 amendment order, searches, filters one text column by contains-search and one select column (specialty), sorts by ФИО and by registration time in both directions, pages (EARS-20…EARS-23, EARS-37, EARS-38).                                                                                                                                                                        |
...
| V-20 | Playwright / E2E — admin   | The registrar reads the «возможный дубль» indicator on the affected roster rows and in their cards and filters the roster down to them (EARS-31, EARS-36).                                                                                                                                                                                                                                                                                                                                 |
...
| V-27 | Vitest e2e — `apps/api`    | The card returns every stored field, the origin, the consent, the mail outcome, the «возможный дубль» marker and the attendance history with author and time from the 010 audit; a site registration and a platform-feed registration written before the `intake_origin` migration read back as origin «site» and «platform feed» respectively; neither the card nor the desk response carries whether the account pre-existed; no endpoint changes the stored answers (EARS-35, EARS-36). |
...
The 2026-09-25 amendment rescopes the open Issues #2313, #2316, #2317, #2318 and #2319 and their parents #2289 and #2290; the tech lead re-grades them after it merges.
