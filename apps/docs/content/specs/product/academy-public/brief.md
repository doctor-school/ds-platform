...
- **The landing takes `/`** (supersedes the 2026-07-17 verdict that `/` redirects to `/webinars`). `/webinars` remains the canonical discovery listing, and the doctor still arrives at `/webinars` after login — but that outcome **must be re-implemented, not inherited**. Today login targets `/` and only the `/` → `/webinars` redirect delivers the listing; feature 008's EARS-7 pins that target to `/`. Taking `/` for the landing therefore obliges **feature 013 to re-point the post-login target to `/webinars` directly** (login redirect default + its tests + a delivery-time inline rewrite of 008 EARS-7, which 013 supersedes) — an in-scope item of 013, named in its PRD. The landing is an entry point for visitors, never a trap for authenticated users.
...
- **Data model: first-class entities + m2m joins, never string tags.** `projects`, `experts`, `partners`, `topics`, plus joins `event_projects`, `event_experts` (carrying the person's **role** on that event), `project_partners`, `event_topics`, `event_recordings` (`kind: edited | raw`), and `leads`. An event may belong to several projects; an expert to many events. Existing free-text `event_speakers` migrates into `experts` gradually with the free-text fallback retained. `specialties[]` on events **stays** — it is the audience axis, a separate dimension from topic.
...
Five features, numbered from 012. PRDs are authored wave by wave — 013 first (owner priority), the rest follow.

| #   | Feature                                                          | One-liner                                                                                                                                                                                                             |
| --- | ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 012 | `content-taxonomy`                                               | `projects` / `experts` / `topics` / `partners` entities + m2m joins + admin CRUD + public read API — the substrate every other feature reads.                                                                         |
...
| 014 | `event-recordings`                                               | `event_recordings` (edited / raw), the archived-event page state, the login-gated player, and recordings in «Мои события» — covers [#1188](https://github.com/doctor-school/ds-platform/issues/1188) / Plane DSP-229. |
...
Webinars-listing filters/facets and the «Прошедшие» tab are assigned to **014 or 015 at spec time** — they belong to whichever ships first and are not a separate feature.
