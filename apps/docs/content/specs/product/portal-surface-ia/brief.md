...
- The webinar surfaces shipped as product-grade rooms and pages (epic 004–007), but the portal **around** them is still scaffold. `/` is a placeholder «Каркас приложения» card whose only action is a "go to sign in" button (`apps/portal/app/page.tsx`) — **still unbuilt, this epic's 008 deliverable**. `/account` **was** a raw session-claims debug dump (raw `sub`, a `roles[]` array, an `mfa` boolean straight from `GET /v1/auth/session`); it has since been **replaced by a real product profile, shipped via [#770](https://github.com/doctor-school/ds-platform/issues/770)** (superseding feature 009). The remaining scaffold gap is the portal shell itself.
...
- **A persistent app-shell header binds the portal.** It carries the logo (→ `/`, the discovery front-door), the top-nav **[Эфиры · Мои события]** («Школы» is not in the v1 nav — owner 2026-07-15), a theme toggle, and — for a logged-in doctor — an **avatar icon showing initials that navigates directly to the profile `/account`** (just an icon → profile, not a dropdown menu); a guest sees a **«Войти»** button instead. Sign-out lives on the profile screen (feature 009), not in the header. This header is the spine that binds the shipped surfaces into one LK.
- **The LK is composed from surfaces that already exist plus this shell:** the public event page `/webinars/:slug` (feature 004), «Мои события» at `/account/events` (feature 005), the webinar room (feature 006), and the read-only profile at `/account` (**shipped via #770**, superseding feature 009). The discovery listing reuses the listing surface already vendored for feature 004.
- **The profile is read-only in v1** — the doctor sees their identity and security state as product copy and can sign out; this shipped via #770 (editing remains a tracked future iteration, never a silent stub).
- **This is a vertical slice, not a re-skin.** The profile slice (feature 009) needed its own authenticated own-profile read endpoint — **now shipped via #770** (`/account` reads a real profile, not session claims). The remaining slice, 008, is the persistent shell — its header (logo→`/`, nav, theme toggle, avatar-icon→profile for a logged-in doctor / «Войти» for a guest) **and** the discovery front-door `/` — is **already designed** in the «Doctor.School визуальный язык» canvas and vendored (the header is present across every vendored portal canvas; `/` is `events-feed.dc.html` = canvas `events-feed.dc.html`). This epic **builds on** that existing shell — it does not invent it. The one surface with no canvas — the read-only profile — is composed from the established neo-brutalist design language (owner decision 2026-07-12).
...
| 009 | [`doctor-profile`](../../features/009-doctor-profile/009-product.md) | **Delivered via [#770](https://github.com/doctor-school/ds-platform/issues/770)** (feature 009 _Superseded_). `/account` is a read-only doctor profile — name, email, phone as product copy, MFA as a product state, and sign-out; the session-claims debug dump is retired and a `GET /v1/me/profile` endpoint (EARS-27/28) backs it. |
...
- **No scaffold or debug surface remains reachable in the portal** — the `/account` session-claims dump is already retired (#770); the `/` «Каркас приложения» placeholder is retired by 008.
- The profile shows the doctor's **real name, email, phone, and security state as product copy** — never raw claims — served by a real authenticated endpoint (shipped via #770).
