...
| [`canvas-reconciliation-ru.md`](./canvas-reconciliation-ru.md) | — (создан здесь, этап 2)             | **Сверка канвасов `design-source/` с макетом R4** ([#1434](https://github.com/doctor-school/ds-platform/issues/1434)): экран R4 → канвас и канвас → экран R4, вердикт по каждой строке, предложенный порядок отрисовки для владельца, продуктовые развилки сверки                                              |
| [`functional-map-ru.md`](./functional-map-ru.md)               | — (создан здесь, этап 3)             | **Функциональная карта экран → REQ → фича** ([#1435](https://github.com/doctor-school/ds-platform/issues/1435)): 24 экрана R4 разложены на предлагаемые фичи 017…044, волны сборки, полное покрытие REQ-1…142, судьба фич 013/015/016, развилки для владельца. Статус — предложение, волну 1 выбирает владелец |
...
- Этап 1 (ниже) **закрыт**: топология двух витрин — [ADR-0015](../../../adr/0015-two-storefront-topology-ru.md) ([#1432](https://github.com/doctor-school/ds-platform/issues/1432)) и базовая модель предметной области — [ADR-0016](../../../adr/0016-core-domain-model-ru.md) ([#1433](https://github.com/doctor-school/ds-platform/issues/1433)) на `main`.
- Этап 2 (ниже) **закрыт**: сверка канвасов `design-source/` с макетом R4 — [`canvas-reconciliation-ru.md`](./canvas-reconciliation-ru.md) ([#1434](https://github.com/doctor-school/ds-platform/issues/1434)). Итог: из 21 канваса 14 остаются, 5 переделываются, 2 остаются опорой текущих фич (006 и 012/014); из 24 экранов R4 15 рисуются заново, 5 переделываются, 4 годны как есть — заново рисуется почти вся витрина врача. Порядок отрисовки (§C) и развилки (§D) подтверждены владельцем 2026-08-22.
- Этап 3 (ниже), **часть 1 закрыта**: функциональная карта экран → REQ → фича — [`functional-map-ru.md`](./functional-map-ru.md) ([#1435](https://github.com/doctor-school/ds-platform/issues/1435)). 24 экрана R4 разложены на 28 предлагаемых фич (017…044) и 7 волн; REQ-1…142 покрыты без пропусков; 013 переспецифицируется как 029, 015 → 030, 016 → 031; шесть развилок владельца закрыты — пять решены, F-6 отложена до проработки ролевой модели. **Ждём выбора владельцем волны 1** — рекомендация карты: 017+018+019+020+021 (витрина врача от входа до участия в событии). **Часть 2 закрыта (2026-08-24):** пакет промптов [`design-prompts-ru/`](./design-prompts-ru/README-ru.md) написан, владелец нарисовал все 20 канвасов, они завендорены в `design-source/` ([#1450](https://github.com/doctor-school/ds-platform/issues/1450)) — дизайн полный, как решил владелец (сначала все экраны, потом сборка). Остаток этапа 3 — бриф эпика и PRD фич волны 1 ([#1435](https://github.com/doctor-school/ds-platform/issues/1435)); Stage-A выбор развилок фиксируется в PRD.
...
3. **Функциональная карта экран → REQ → фича** (на базе `screens-ru.md`) и **нарезка фич по штатному конвейеру ADR-0014**: `do-product-discovery` → `author-product-spec` (`brief.md` / `brief-ru.md` эпика + `NNN-product.md` на фичу) → `author-ears-spec` → `open-ears-issues`. Здесь же — пакет промптов Claude.Design `design-prompts-ru/` (фиксированная преамбула + промпт на каждый экран в порядке §C сверки), он пишется после функциональной карты.
