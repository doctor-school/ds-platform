...
status: "Discovery closed (DSP-252) · wireframe R4 owner-approved (DSP-251) · stage 1 done: ADR-0015 topology (#1432) + ADR-0016 core model (#1433) on main · stage 2 done: canvas ↔ R4 reconciliation (#1434) · stage 3 part 1: functional map on main (#1435), wave-1 pick pending"
lang: ru
---
...
- Этап 3 (ниже), **часть 1 закрыта**: функциональная карта экран → REQ → фича — [`functional-map-ru.md`](./functional-map-ru.md) ([#1435](https://github.com/doctor-school/ds-platform/issues/1435)). 24 экрана R4 разложены на 28 предлагаемых фич (017…044) и 7 волн; REQ-1…142 покрыты без пропусков; 013 переспецифицируется как 029, 015 → 030, 016 → 031; шесть развилок владельца закрыты — пять решены, F-6 отложена до проработки ролевой модели. **Ждём выбора владельцем волны 1** — рекомендация карты: 017+018+019+020+021 (витрина врача от входа до участия в событии). Часть 2 (бриф эпика, PRD фич, пакет промптов `design-prompts-ru/`) пишется после выбора.
