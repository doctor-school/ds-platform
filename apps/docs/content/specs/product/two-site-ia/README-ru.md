...
- Этап 2 (ниже) **закрыт**: сверка канвасов `design-source/` с макетом R4 — [`canvas-reconciliation-ru.md`](./canvas-reconciliation-ru.md) ([#1434](https://github.com/doctor-school/ds-platform/issues/1434)). Итог: из 21 канваса 15 остаются, 4 переделываются, 1 снимается, 1 остаётся опорой фич 012/014; из 24 экранов R4 16 рисуются заново — почти вся витрина врача. Предложенный порядок отрисовки (§C) и продуктовые развилки (§D) ждут подтверждения владельца.
