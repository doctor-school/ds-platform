...
- Stage 2 (below) **done**: reconciliation of the `design-source/` canvases against wireframe R4 — [`canvas-reconciliation-ru.md`](./canvas-reconciliation-ru.md) ([#1434](https://github.com/doctor-school/ds-platform/issues/1434)). Outcome: of 21 canvases 15 stay, 4 are reworked, 2 stay as the working base of in-flight features (006 and 012/014); of the 24 R4 screens 16 are drawn from scratch — nearly the whole doctor storefront (if the owner picks the shared event canvas in fork D-4 — 15 from scratch and 5 reworked). The proposed drawing order (§C) and the product forks (§D) await the owner's confirmation.
