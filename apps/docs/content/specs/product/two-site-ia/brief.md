...
- **The doctor's trace of the Academy is limited to three fixed placements** (REQ-24, NG-2) — the storefront footer link, «Стать экспертом» in the cabinet, and (028) the documents-list caption on `#d-docs` naming the Academy documents page. The Academy never becomes a second doctor catalogue; the doctor storefront never exposes backstage.
