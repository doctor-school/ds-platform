...
| **SMS**                         | **SMS-Aero** (smsaero.ru Gate API v2)                                      | SMSC.ru / SMS.ru (interchangeable RF fallback)                           | Signed per provider                             | **Decided** (supersedes the old "implementation moment" placeholder; Plane DSO-26/57/58 are cross-tracker references only). Gate API v2 — HTTP Basic (`email:api_key`), `POST https://gate.smsaero.ru/v2/sms/send`, params `number`/`text`/`sign` (default sign `SMS Aero`); env `SMSAERO_EMAIL`/`SMSAERO_API_KEY`/`SMSAERO_SIGN`. RF, 152-FZ-compliant. Circuit-breaker / failover pattern in identity-auth-rbac-design §5. **Dev-stand:** the local stand never reaches SMS-Aero — Zitadel's generic HTTP SMS provider posts to a local `sms-sink` catcher (the SMS analogue of Mailpit; `infra/dev-stand`), so SMS-OTP (003 EARS-7) is live-testable without a real send. |
