...
Brainstorm-исследование (резюме в `outputs/2026-05-12-ai-agent-dev-readiness-research.md`) подтвердило, что под AI-agent driven development стандарт 2025-2026 — расширенная обвязка с фокусом на supply chain security, OpenTelemetry GenAI, prompt-injection protection, autonomy ladder и spec-driven workflows.
...
- **Postgres outbox** для critical jobs (ADR-0003 Amendment A2/§A).
...
| **SMS**                         | TBD (SMS.ru / SMSC.ru / MTT / BeelineBusiness — взаимозаменяемы)             | TBD (второй из списка)                                                            | Подписывается per provider                | Выбор — implementation moment. Все RF, все 152-ФЗ-compliant by default. Circuit-breaker pattern в identity-auth-rbac-design §5.                            |
...
- Каталог ADR в репо (`docs/adr/`) + индекс
- Spec-формат (наследуется от `docs/superpowers/specs/` текущей структуры)
...
13. **Host-only `__Host-` cookie per app + OIDC silent re-auth** (ADR-0001 §6 + Amendment A2) — нет shared cross-subdomain cookies.
