...
| 2   | **ADR-0001 §7.1**                         | The `step_up_required` matrix field (per-endpoint `acr=mfa-fresh` requirement).                                                               |
...
| `step_up`        | authored                   | `true` \| `false` (default `false`)                                 | Whether a fresh step-up authentication (`acr=mfa-fresh`, ADR-0001 §7.1) is required to call the endpoint. Read by the `StepUpGuard` seam (003-design §7). v1 has no step-up endpoints; the first arrives with the first high-risk authenticated action.                                                 |
...
| PATCH /v1/courses/:id   | authenticated | doctor_guest   | policy     | course.author_id==actor.id | false   | high-stakes | EARS-…        |
```
...
1. **ADR-0001 §2.5** — the stub's six-field row and `auth_check ∈ {fast-path, cerbos}` are superseded by this spec's eight-column row with engine-neutral `auth_check ∈ {none, fast-path, policy}` and the explicit `access` discriminator. Align the stub wording and its illustrative role list (`guest/...`) to the shipped role model (`doctor_guest`, SSOT `packages/db`).
2. **ADR-0001 §7.1** — the `step_up_required` field is defined here as the `step_up` column; additionally, §7.1 cites "ADR-0001 §10 (step-up authentication)" but §10 is "Risks and open questions" — there is no step-up section. Fix the broken internal cross-reference.
...
| ADR-0001 §7.1 (`step_up_required` field)                                 | §3 (`step_up` column) |
