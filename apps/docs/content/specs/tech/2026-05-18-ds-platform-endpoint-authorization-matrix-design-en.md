...
| `auth_check`     | authored                   | `none` \| `fast-path` \| `policy`                                   | **Engine-neutral.** `none` = no subject/role/policy evaluation (only valid with `access: public`). `fast-path` = JWT/role claim check only (RBAC, in the guard, no external call). `policy` = the role alone is **not sufficient**; the row's `object_attrs` selects which of the two sub-modes applies (see _The two `policy` sub-modes_ below).                                                                                                                                                                                                                                                                                                                                                                        |
...
| `audit`          | authored                   | `none` \| `low-stakes` \| `high-stakes`                             | Determines whether an entry in `auth_audit` (the `audit_ledger` projection, ADR-0003 §6) is required. `high-stakes` ⇒ a terminal ledger entry is mandatory, **unless** the route carries an explicit `none-by-spec` entry in the emission-coverage registry (§4) stating — with the citation of the spec clause that says so — that the command emits nothing durable. That exception is reviewed and cited, never silent: an entry that claims "no row" without a reason and a spec citation is rejected, as is a bare empty emission list. A route that owes no row is classified `high-stakes` with that entry, **not** demoted to `low-stakes` — demoting it would remove it from the completeness guard altogether. |
...
**The two `policy` sub-modes.** `auth_check: policy` records the authorization **shape** — the role is a necessary precondition but not the whole decision — not the evaluation venue. Which venue evaluates the rest is determined by `object_attrs`:

- **Object-level `policy`** (`object_attrs` non-empty). The decision binds the object to the actor through an ABAC predicate (e.g. `course.author_id == actor.id`). The predicate is evaluated through the `IPolicyEngine` (ADR-0002 §3.2); the concrete engine (Cerbos / OPA / OpenFGA / SQL) is DSO-27, and the guard **fails closed** on such a route until the engine is wired — no object-level route ships before it.
- **Resource-scoped `policy`** (`object_attrs` = `—`). The decision depends on the resource's own state, not on an object↔actor attribute predicate — e.g. the 006 webinar-room admission gate: `authenticated ∧ registered ∧ live` (006-design §2). The guard enforces `required_roles` as the necessary precondition and lets the handler run; the classified handler/service evaluates the resource-scoped domain rule against its read models and refuses server-side. This mirrors how a `fast-path` route's domain gating (e.g. 005's published/live registration check) lives in its service — the matrix value records that the role alone is not sufficient, while the handler enforces the resource-specific rule.
...
- **Global `AuthzGuard` (`APP_GUARD`)** reads `AUTHZ_KEY` and enforces: authenticate (unless `@Public`), check `required_roles`, and for an **object-level** `check: "policy"` (`objectAttrs` present) delegate the predicate to `IPolicyEngine` — failing closed until DSO-27 wires the engine. A **resource-scoped** `check: "policy"` (no `objectAttrs`) passes the guard on the role check; the classified handler then evaluates the resource-scoped domain rule (§3). A handler with **no** `AUTHZ_KEY` metadata is **denied** (fail-closed) — the runtime mirror of the Layer-2 gate.
- **`audit` emission.** The `audit` class records how a route's terminal `auth_audit` row is accounted for — owed and emitted, or owed by nobody because the governing spec says the command emits nothing durable (§3) — not how it is written. **Auth & security events emit explicitly at the command site** (the `AuthAuditLog` port; `auth/session/auth-audit.*`) — their subjects/reasons are heterogeneous (`login.failure` has no subject and a masked identifier; `lockout` fires once on the tripping transition; `otp.sent` has no subject yet), so a generic per-route interceptor cannot build them uniformly. A `@Authz({ audit })`-driven interceptor applies only to **uniform-subject resource routes**, where the terminal access row is the resolved request subject. Emission completeness over the `audit: high-stakes` set is enforced by a CI guard, not by the decorator: the reviewed emission-coverage registry lives at `apps/api/src/authz/audit-emission-coverage.ts` — each entry names the emitted event(s) plus the covering e2e `it`, a tracked deferral, or a cited `none-by-spec` — with its schema validated by `apps/api/src/authz/audit-emission-coverage.spec.ts` and the registry cross-checked against the real router by `apps/api/test/authz/audit-emission-coverage.e2e-spec.ts`.
...
Every v1 auth endpoint is `auth_check ∈ {none, fast-path}` and `step_up: false`: 003 needs neither `policy` sub-mode nor step-up. The `policy` rows (§7.3) and `step_up: true` therefore arrive with later feature sets, not 003.

### 7.3 `policy` rows, worked (both sub-modes)

**Resource-scoped `policy` — the 006 webinar-room admission gate.** The room's three protected operations share one server-side gate (`authenticated ∧ registered ∧ live`, 006-design §2); the role is necessary but not sufficient, and the registered-and-live rule is evaluated in-service:
...
**Object-level `policy` — illustrative future row.** No shipped route declares `object_attrs` yet (the guard fails closed on one until DSO-27):
...
ADR-0010 §6 forward-references this spec for the rule that PD-accessible tool calls are authorized independently of LLM output (`subject_id` taken from the authenticated session, never from LLM text). Tool-exposing endpoints are ordinary rows in this matrix: an endpoint that backs a P-LLM tool is classified `auth_check: policy` in the **object-level** sub-mode (§3) — an `object_attrs` predicate binding the resource to the **session** subject (e.g. `record.subject_id == session.subject_id`), `audit: high-stakes`. The matrix is therefore the enforcement record of ADR-0010's hard-constraint #6: the authz decision is in the endpoint's `@Authz`, evaluated by the guard/`IPolicyEngine`, and is structurally unable to depend on LLM output.
