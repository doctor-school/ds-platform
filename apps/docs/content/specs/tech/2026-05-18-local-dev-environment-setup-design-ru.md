...
**Auto-snapshot:** `/root/dev-stand-maintenance.sh` на TrueNAS, запускается ~через 30 мин после каждой загрузки через Post-Init init-script (`systemd-run --on-active=30min`). Для каждого dataset'а `Daily SSD/dev-*` создаёт снапшот, только если свежайший снапшот `@auto-dev-*` старше 7 дней, затем прунит до последних 3. Каданс фактически «недельный, относительно аптайма». Известное ограничение: если коробка работает > 7 дней без перезагрузки, boot-триггер повторно не срабатывает — приемлемо для dev-бокса, который регулярно выключают; при необходимости покрыть долгий аптайм — добавить `systemd` `OnUnitActiveSec=1week` для повторной проверки.
