...
DS Platform — выбранный стек (ADR-0001..0011): NestJS+Fastify api+worker, Postgres17+Drizzle+pgvector, Redis, MinIO-compatible object storage, Zitadel (закрыто по ADR-0001 §8 / DSP-209), Centrifugo, Cerbos, четыре Next.js 15-приложения (promo/portal/admin/cms), RN+Expo (Pre-pilot phased to PWA), observability stack (Loki+Grafana+GlitchTip+Outline+Unleash, Pre-pilot).
...
| `idp` (Zitadel, per ADR-0001 §8) | `ghcr.io/zitadel/zitadel:latest` (single Go binary)              | 8080 (HTTP+gRPC h2c)            | 8080      | 256–512 MB (binary) + shared Postgres | api (JWT issuer), все web-apps (OIDC login)                         |
...
Это специфика конкретного deployment'a. У Tech Lead's recipe (TrueNAS Scale 24.10) — firewall менять не нужно: на коробке нет host-level inbound-фильтра, LAN-порты доступны как есть (см. §5.4). У host-only recipe — Windows Firewall не трогается (localhost-only).
...
> **Note (реализация DSP-155):** SSD-пул называется `Daily SSD` (с пробелом), а не `Daily` — dataset'ы это `Daily SSD/dev-*`. Пробел остаётся только внутри `zfs`-команд (всегда в кавычках); каждый dataset несёт явный `mountpoint=/mnt/dev-stand/<name>`, чтобы пути Docker bind mount были без пробела.
...
- TrueNAS Scale 24.10 **не имеет host-level inbound firewall** — в живом nftables-ruleset только Docker'овские bridge-isolation цепочки, цепочки `filter hook input` нет. LAN-клиенты на `192.168.1.0/24` достают опубликованные порты (5432/6379/9000-9001/9080/9443/8000/3100/4000/1025/8025/3592/3593) без дополнительной настройки — «открыть firewall» для этого recipe это no-op. Если inbound-фильтрация когда-нибудь понадобится — добавить её явно кастомным nftables init-скриптом.
- Экспозиция в интернет регулируется роутером (нет port-forwarding на коробку TrueNAS), а не host-firewall'ом.
- DHCP reservation `192.168.1.115` для MAC TrueNAS на роутере.

### 5.5 Snapshot & backup (Tech Lead's recipe — ZFS-based)

**Почему не TrueNAS Periodic Snapshot Task:** коробка TrueNAS работает не 24/7. Cron-задача с фиксированным временем молча пропускается в каждый день, когда в этот час коробка выключена — у TrueNAS Periodic Snapshot Task нет catch-up. Recipe вместо этого использует boot-triggered maintenance-скрипт с проверкой возраста, так что покрытие зависит от аптайма, а не от времени суток.

**Auto-snapshot:** `/root/dev-stand-maintenance.sh` на TrueNAS, запускается ~через 30 мин после каждой загрузки через Post-Init init-script (`systemd-run --on-active=30min`). Для каждого dataset'а `Daily SSD/dev-*` создаёт снапшот, только если свежайший снапшот `@auto-dev-*` старше 7 дней, затем прунит до последних 3. Каданс фактически «недельный, относительно аптайма»: коробка настроена на ежедневное автовыключение, поэтому boot-триггер срабатывает каждый день, а проверка возраста (7 дней) гейтит фактический снапшот. Аптайм не превышает ~1 дня, поэтому edge case с пропущенным окном отсутствует.

**Off-pool replication:** тот же скрипт — если последняя реплика старше 7 дней — выполняет инкрементальный `zfs send -I … | zfs recv` `Daily SSD/dev-postgres` → `Media/backups/dev-stand/postgres`, затем прунит снапшоты на цели старше 4 недель. Это backup на случай corruption пула Daily SSD, не disaster recovery.

**Pre-migration manual snapshot:** перед `pnpm drizzle:migrate` AI-агент / dev вызывает `pnpm dev:snapshot pre-mig-<desc>`. Wrapper SSH'ит на TrueNAS, `zfs snapshot "Daily SSD/dev-postgres@<...>"`. Откат: `pnpm dev:rollback <snapshot>` (требует `docker compose stop postgres` перед `zfs rollback`).
...
- `infra/dev-stand/.env` на TrueNAS — содержит `POSTGRES_PASSWORD`, `MINIO_ROOT_PASSWORD`, `ZITADEL_MASTERKEY` (32-char секрет для encryption-at-rest Zitadel; ADR-0001 §8 / DSP-209) и др. Файл вручную создаётся при bootstrap из `.env.example`. **Не коммитится** (gitignore).
...
- IdP-сервис добавляется в compose (DSP-157) → строка `idp` в §3.1 становится активным Zitadel-сервисом (закрыто по ADR-0001 §8, DSP-209).
...
- Конкретная wiring IdP в dev-stand compose (DSP-157) — Zitadel per ADR-0001 §8 (DSP-209).
