...
status: Implemented — revised 2026-09-03 (owner-planned dates on the dependency chain, #1838)
...
| **Start date**  | date (custom)            | Roadmap bar start. Set **only** on feature-level and release-gate Issues; the **planned** start from the planning grooming — the day the previous item on the track's dependency chain ends (see below).                                                          | planning grooming (owner), sequential on the chain                                                                             |
| **Target date** | date (custom)            | Roadmap bar end. Set **only** on feature-level and release-gate Issues; the **owner's planned end** = planned start + the duration the owner gave in the planning grooming (see below).                                                                           | planning grooming (owner); `pnpm roadmap:forecast` fills only undated items                                                    |
...
#### Date semantics (owner plan on the dependency chain)
...
Owner decision (2026-09-03, #1838) on what the dates mean: the roadmap is a **Gantt chart of the plan** — releases of one track follow each other, features inside a release follow the dependency order, and a bar's length is the **owner's estimate**, never a measured fact or a throughput extrapolation. Verbatim: «если у нас есть критический путь по задачам, то они же явно будут выполняться последовательно одна за другой, значит на роадмапе они должны выглядеть как на правильно построенной диаграмме Ганта».

- **Durations come from the planning grooming.** Per release the owner names a duration in chat (one question per message; grooming record `specs/product/two-site-ia/backlog-grooming-<date>-ru.md`); the lead proposes the split between the features inside the release, in dependency order. Agent-side estimates are a proposal only — never written to the board before the owner has replaced or confirmed them.
- **Start date** = the planned start: the first item of a track starts today, every next item starts the working day after the previous one's Target date; releases of one track never overlap. It is a plan date, not the day work actually began.
- **Target date** = Start date + the owner's duration. The milestone `due_on` equals the Target date of its release gate.
- **Every dated Issue carries the body line `Target date: owner-set`**; `pnpm roadmap:forecast` skips those and may fill only Issues with no owner-set date (a throughput hint for the undecided tail, posted as a comment on the gate Issue). A board date written by an agent without that grooming trail is a defect to revert, not a forecast.
- **Re-planning** happens at the next grooming: a slipped release moves every later release of the same track by the same delta; the owner confirms the new chain.
...
| ---------------- | -------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Roadmap**      | PM/owner | **Roadmap**    | Filter `label:roadmap` only — Done stays visible so a shipped release still renders its bar. Group by **Milestone**. Date fields **Start date → Target date**; milestone markers on. The PM-facing plan surface. |
...
**Roadmap readability is the design constraint.** Only feature-level Issues appear on the Roadmap view — the `label:roadmap` filter is what makes it legible, and EARS work is visible instead as the parent feature's native sub-issue progress bar. The `roadmap` label is the ONE tooling-written attribute the view selects on: it is added by `pnpm issue:create` to a feature Issue or a release gate, audited in both directions by `pnpm backlog:triage` (`missing-roadmap-label` / `stray-roadmap-label`), and never set by hand — the view filters on no milestone name, title pattern or org Type. Beyond the two nearest releases of a track, features sit in the track's «· Позже» milestone with no dates, so the timeline never renders speculative bars.
...
Five Issue levels exist. Each has exactly one place in the hierarchy, one title pattern, one dates rule and one answer to «does it carry the `roadmap` label» — this table is the contract the board, the views and `pnpm issue:create` all read.

| Level             | Org Type | Title pattern                                          | Parent                       | Milestone                                                                 | Start / Target dates | `roadmap` label | On Roadmap view              |
| ----------------- | -------- | ------------------------------------------------------ | ---------------------------- | ------------------------------------------------------------------------- | -------------------- | --------------- | ---------------------------- |
| **Epic**          | Feature  | `epic: <English title>`                                | none                         | **none** — an epic spans releases (§3.2)                                  | no                   | no              | no                           |
| **Feature**       | Feature  | `[Академия][NNN] <English title>` / `[Витрина][NNN] …` | the epic, when one exists    | the release it ships in                                                   | **yes**              | **yes**         | **yes**                      |
| **EARS task**     | Task     | `[NNN] EARS-k: <English clause>`                       | its feature                  | inherits the parent feature's                                             | no                   | no              | no (sub-issue progress only) |
| **Release gate**  | Task     | `gate: <milestone name>`                               | none                         | the milestone it gates                                                    | **yes**              | **yes**         | **yes**                      |
| **Platform task** | Task     | `<English title>` (no track prefix)                    | none, or the blocked feature | the release it blocks (via `blocked_by`), else «Platform ops & hardening» | no                   | no              | no                           |
...
**Release gate rule.** Every milestone has exactly one gate Issue. It carries native `blocked_by` edges to every feature in the release, so "is the release ready" is answered by the native dependency graph rather than by prose. It holds the milestone's Start / Target dates — the release bar of the Roadmap view, which is why the gate title repeats the RU milestone name (owner: the roadmap shows milestones, hence RU). Reference examples: #1671, #1672, #1790, #1839–#1846.

**Planning rule.** Dates are the owner's plan on the dependency chain (§3.2 Date semantics): sequential releases per track, sequential features inside a release, owner-given durations, `Target date: owner-set` on every dated Issue. `pnpm roadmap:forecast` is advisory for undated items only and never overwrites an owner-set date.
...
4. **View creation.** For each view in §3.4, the script creates the view itself through the public `createProjectV2View` mutation (`name` + `layout`) and then converges its filter through `updateProjectV2View` (`viewId` + `filter`), skipping the mutation when the live filter already matches; both were verified against the live board on 2026-09-02, and each falls back to printed UI steps when refused. So the §3.4 filters — notably the `Roadmap` `label:roadmap` readability constraint — are reproducible from the script. The grouping, the `Roadmap` date-field pair and its milestone markers have no mutation input and stay UI-only settings.
5. **Backfill.**
...
4. All views (§3.4) are configured with the correct filter / group / layout — including `Roadmap` (layout roadmap, group by Milestone, filter `label:roadmap`, dates Start → Target).
...
| An agent writes board dates that are not the owner's plan (a fact-of-start, a throughput extrapolation).         | §3.2: dates come only from the planning grooming and carry `Target date: owner-set`; `pnpm roadmap:forecast` skips them. A date without the grooming trail is reverted.                |
