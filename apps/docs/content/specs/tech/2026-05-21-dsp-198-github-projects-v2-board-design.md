...
4. **View creation.** For each view in §3.4, the script creates the view itself through the public `createProjectV2View` mutation (`name` + `layout`, verified against the live board on 2026-09-02) and falls back to printed UI steps when the mutation is refused. The grouping, the filter, the `Roadmap` date-field pair and its milestone markers are not part of that mutation's input and stay UI-only settings.
5. **Backfill.**
