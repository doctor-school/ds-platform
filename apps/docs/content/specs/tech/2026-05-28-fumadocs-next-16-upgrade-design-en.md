...
description: "Single bundled PR upgrading apps/docs (fumadocs-mdx 14→15, fumadocs-core 15→16, fumadocs-ui 15→16, next 15→16), locked together by peer-dep coupling. Captures peer-cascade rationale, breaking-change matrix against actual usage surface, and verification plan."
...
1. `apps/docs` builds cleanly on Next 16 + fumadocs 16.
2. Existing fumadocs-rendered routes (`/`, `/adr/<slug>`, `/architecture/*`, etc.) render identically — no visual or routing regression.
3. All CI jobs that touch docs continue to pass: `docs-build`, `glossary-mdx`, `glossary-roundtrip`, `spec-link`, `module-readme`.
4. Single revertable commit (or single PR) — rollback is one `git revert`.
...
| Default caching: `fetch` no longer cached by default                    | Next 15→16                | `apps/docs` does not use `fetch` in SSR paths today   | No-op.                                                                                                                                                                        |
| Turbopack as default dev/build engine                                   | Next 16                   | `apps/docs`                                           | It uses stock `next dev`/`next build`. If Turbopack regresses on fumadocs MDX pipeline, fall back to webpack via `--webpack` flag.                                            |
| React 19.2 peer requirement                                             | fumadocs-core 16, next 16 | `apps/docs`                                           | Already on `react@19.2.6` / `react-dom@19.2.6`. ✓                                                                                                                             |
...
| ------ | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **R1** | fumadocs-core/ui 16 API drift on our 7-file surface                                                     | Low — stable export paths                       | `pnpm --filter docs build`; dev-server open `/`, `/adr/0004`, `/adr/0006` (heaviest ADR pages) |
| **R2** | fumadocs-mdx@15 codegen shape change                                                                    | Low — minor major, post-split                   | `pnpm --filter docs typecheck` after `postinstall`                                             |
| **R3** | pnpm install hard-errors on `zod: 4.x.x` optional peer                                                  | Low                                             | If it occurs, add `pnpm.peerDependencyRules.ignoreMissing: ["zod"]` (scoped)                   |
| **R4** | CI-chain regressions (`docs-build`, `glossary-mdx`, `glossary-roundtrip`, `spec-link`, `module-readme`) | Low — none of these touch Next runtime directly | Full CI run on branch before merge                                                             |
...
2. `pnpm --filter docs typecheck` — catches R2 cheapest.
3. `pnpm --filter docs build` — covers R1 + fumadocs static export.
4. `pnpm --filter docs dev` — open `/`, `/adr/0004`, `/adr/0006`; check console for hydration errors.
5. Push branch, watch full CI matrix.

Promotion gate: all four local checks green **and** all CI jobs green → `gh pr merge --auto`.
...
### 10.1 `fumadocs-ui/provider` → `fumadocs-ui/provider/next` (R1 materialised)
...
### 10.2 Turbopack incompatible with fumadocs-mdx@15 SSG pipeline (R2 materialised)
...
Mitigation per §5: pin `--webpack` in `apps/docs` scripts (`dev` and `build`).
