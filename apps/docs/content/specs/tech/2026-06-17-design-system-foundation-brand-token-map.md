...
| `color.brand` / `color.brand-foreground`        | `{color.blue.700}` `#114D9E` / `{color.white}`   | Strong brand fill for marketing/auth panels (the dark brand blue); white text on it is 8.14:1 (AA-normal), so panel copy is unconstrained by size — unlike `primary` #2D84F2 (white only AA-large). Added for the #237 auth split-screen. |
