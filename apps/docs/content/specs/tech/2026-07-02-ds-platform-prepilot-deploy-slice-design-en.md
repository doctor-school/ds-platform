...
Per the brainstorm decision (#7) and the dev-stand precedent, secrets are **`.env` files provisioned out-of-band** onto each VPS (root-owned, `0600`), referenced by docker-compose `env_file:` — **never** committed, **never** in Terraform state:
