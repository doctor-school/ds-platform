...
8. **Release gate clear (§10.10).** No OPEN Issue carries `release-blocker`, and no merged-but-not-yet-deployed PR in the range defers its Stage-B to a still-OPEN batched gate. Enforced mechanically by the `deploy:prod` pre-flight (`tools/deploy/release-gate.mjs`, #1662), fail-closed on an UNKNOWN; the only bypass is the explicit `--release-gate-exempt "<reason>"`, which is loudly printed and is an owner-approved decision, never a convenience.
...
- **A `release-blocker` label instead, when a revert is disproportionate.** When the revert would be larger or riskier than the defect (a deep migration in the range, a wide refactor, a defect whose fix is already in flight and lands within the same session), the defect Issue carries the `release-blocker` label instead, and prod simply waits. The label is a HOLD on the whole release train — it is not a severity marker and is not a substitute for fixing or reverting; it is applied with a comment naming why a revert was disproportionate, and removed by closing the Issue.
- **The choice is recorded, never implicit.** Whichever branch is taken (revert PR / `release-blocker` label), the reason lives on the defect Issue.

**Mechanically enforced at deploy time.** The `deploy:prod` pre-flight (`tools/deploy/release-gate.mjs`, checklist item §10.4-8) HOLDS the deploy while either signal stands:
...
The merged-undeployed delta is anchored on the **live** deployed SHA — `GET /v1/health → {version}`, falling back to the latest `production` GitHub Deployment record — the same `healthSha ?? deploymentSha` basis rule the `## Project reality` bootstrap section uses (`tools/project-reality.ts`, step 4). Health is ground truth and the Deployment record only recorded intent: an app-only `pnpm deploy:prod --rollback` records **no** Deployment, so the recorded SHA can be newer than what actually runs, and a Deployment-only basis would narrow the range and let undeployed PRs escape the batched check — a fail-open in the one scenario where the train is already known-sick. Falling back to the record therefore marks the basis DEGRADED, which is an UNKNOWN and HOLDS.
...
§10.10 makes `main` deployable by default and gives two responses when it is not: revert the offending PR, or hold the whole train behind a `release-blocker`. Both assume the release UNIT is the whole `deployedSha..origin/main` range (§10.2). This section adds the third case that neither covers: `main` is fine, prod is simply BEHIND it by more than the fix, and one already-merged change must reach production now without shipping everything else merged since.
