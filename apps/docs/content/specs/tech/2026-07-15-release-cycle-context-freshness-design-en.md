...
2. any merged-but-not-yet-deployed PR whose body carries `Stage-B: batched at #<gate>` for a still-OPEN gate Issue — the failure names PR → gate pairs. The batched-Stage-B carve-out (AGENTS.md §6) defers the owner's live verdict for **merging**; it never granted a deferral for **shipping to prod**, and this gate is what makes that distinction real rather than a convention.

The gate is **fail-closed** like the эфир hold (§10.4 item 7): a `gh` failure, or no production Deployment record to anchor the merged-undeployed delta, HOLDS. The single bypass is the explicit `--release-gate-exempt "<reason>"` flag (mandatory non-empty reason, loudly printed as the audit record, mirroring `--mode-a-exempt`) — an owner-approved decision, never a silent auto-detection.
