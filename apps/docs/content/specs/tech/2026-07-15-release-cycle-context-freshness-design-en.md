...
### 10.10 `main` stays deployable — the revert norm and the release-blocker gate (#1662)

§10.2 states the corollary "keep `main` continuously shippable" but never said what happens when it stops being shippable. This is that rule.

**`main` is deployable by default.** The unit of a deploy is the whole `deployedSha..origin/main` range (§10.2), so ONE broken merged PR holds every other merged PR out of prod. Therefore:

- **A merged PR discovered broken ahead of its fix is REVERTED from `main`.** The revert is the default response, not the fallback — it restores the invariant immediately and costs nothing but a re-land. `gh pr revert` is not used: open a normal revert PR (`git revert <squash-sha>`), which follows the normal rules — Conventional-Commit title, `Closes`/`Part of` the defect Issue, Mode-a review, green CI, a changeset when the reverted change was user-facing, and Stage-B when the revert itself changes a visual surface. The re-land then comes back as its own PR with the fix.
- **A scoped prerequisite instead, when a revert is disproportionate.** Link the fix Issue through `Release-requires: #N` in the offending PR body and record why a revert is disproportionate. Only releases containing that PR wait. Keep the Issue open until the prerequisite is evidenced complete, never auto-close it merely because the repository-side half merged.
- **Global `release-blocker` only for an evidenced hazard to ANY deployment**, including unrelated hotfixes. Record the universal risk, why revert/scoped prerequisites cannot protect it, and the exit condition. Deferred production work, unexplained CI flakes and incomplete post-release acceptance alone do not qualify. Reassess before each release and when evidence changes: remove an obsolete label with an evidence comment, keeping unfinished work OPEN. The label is not task priority and never substitutes for fixing/reverting.
- **The choice is recorded, never implicit.** The defect Issue records the rationale for revert, scoped prerequisite or global hold.

**Mechanically enforced at deploy time.** The `deploy:prod` pre-flight (`tools/deploy/release-gate.mjs`, checklist item §10.4-8) HOLDS the deploy while any signal stands:

1. any OPEN Issue carrying `release-blocker` — the failure names Issue number + title;
2. any merged-but-not-yet-deployed PR carrying `Stage-B: batched at #<gate>` for a still-OPEN gate Issue — the failure names PR → gate pairs. The marker is read from the **same accepted-source set the merge guard enforces** (`tools/lint/stage-b-lint.ts`): the **PR body OR a comment on any Issue the body links with a `Closes #N` keyword**. Reading only the body would let a PR that legitimately merged on a linked-Issue record ship un-live-verified — the gate would print "clear" on exactly the case it exists to catch. The batched-Stage-B carve-out (AGENTS.md §6) defers the owner's live verdict for **merging**; it never granted a deferral for **shipping to prod**, and this gate is what makes that distinction real rather than a convention.

3. any OPEN prerequisite referenced by `Release-requires: #123, #456` in a PR body from the selected deployment range. Multiple lines are unioned and deduplicated per PR; absent/`Release-requires: none` means no prerequisites. HTML template comments are ignored. Invalid declarations, unreadable PR/Issue evidence and unknown Issue states fail closed. Output names PR → prerequisite pairs. A prerequisite attached to a PR outside this range cannot stop this deployment.

**Deferred action classification (#2187, owner correction 2026-09-11).** Every deferred production action remains tracked by an Issue naming the affected change, action, timing and completion evidence. Only conditions required BEFORE shipping use `Release-requires`. Rollout actions and post-release acceptance stay linked in prose with an execution/check plan, never circular preconditions of their own activation; pipeline-owned actions cite `file:line`. Review blocks missing tracking or omitted real prerequisites. This replaces the blanket “deferred action → global label” interpretation introduced after #1396/#1994 without losing action tracking.

The selected delta is `<live deployed>..<target>` (normally `origin/main`, or the chosen hotfix SHA), with PR numbers extracted from squash commit subjects by the existing shared release-notes helper. Cherry-picked squash subjects retain their PR number; unrelated main PRs are outside a hotfix's range. The basis comes from `GET /v1/health → {version}`, falling back to the latest `production` GitHub Deployment record. Health is ground truth: an app-only rollback records no Deployment, so a record-only basis may be too narrow. Falling back therefore marks the basis DEGRADED, an UNKNOWN which HOLDS.

The gate is **fail-closed** like the эфир hold (§10.4 item 7): a `gh` failure (including an unreadable linked Issue — an accepted marker source that could not be checked), a degraded delta basis, or no deployed SHA at all to anchor the delta, HOLDS. Every `gh`/`git` call is bounded (15 s per call, 8 s for the health probe) so a hung call degrades to that hold instead of stalling the pre-flight. The single bypass is the explicit `--release-gate-exempt "<reason>"` flag (mandatory non-empty reason, validated at start-up and loudly printed as the audit record, mirroring `--mode-a-exempt`) — an owner-approved decision, never a silent auto-detection.

### 10.11 Hotfix deploy (#1881)

§10.10 keeps `main` deployable by default through reverts, scoped prerequisites and a global hold only for universal hazards. When prod needs one already-merged fix without everything else merged since, this section narrows the release unit from `deployedSha..origin/main` (§10.2) to a selected hotfix. Prerequisites of unrelated PRs are outside that unit; valid global holds remain.

**The rule.** `pnpm deploy:prod --ref <sha>` ships exactly `<sha>` instead of `origin/main`. `<sha>` is the head of a `hotfix/<N>-<slug>` branch cut FROM the SHA running in production, carrying cherry-picks of already-merged squash commits and nothing else. The branch is deleted once the deploy is recorded. This is a narrowing of the deploy range, never a widening of what is deployable.

**Pre-flight invariants** (`tools/deploy/hotfix-ref.mjs` — pure verdicts, unit-tested; wiring in `tools/deploy/prod.mjs`). All fail-closed, all before any ship step:

1. `--ref` takes a commit SHA, not a branch name or tag, and never combines with `--rollback`. Both are usage errors raised before the clean-tree, fetch and CI probes — the same fail-fast contract `--release-gate-exempt` validates under.
2. The target resolves after `git fetch origin` AND is reachable from an `origin/*` branch. A commit that exists only locally must never reach prod.
3. The target is a STRICT descendant of the LIVE deployed SHA (`GET /v1/health → {version}`, the §10.10 ground-truth basis — not the Deployment record, which an app-only rollback leaves ahead of reality). Rewinding prod is `--rollback`.
4. Every commit in `deployed..target` has an equivalent commit on `origin/main` (native `git cherry` equivalence, or exactly one canonical `cherry-pick -x` receipt naming a single-parent source on `origin/main`, whose clean Git three-way replay onto the target parent produces exactly the target tree; merge commits, conflicts, extra edits, malformed/ambiguous receipts and Git errors refuse). This is the invariant that keeps "production runs reviewed, merged code" true — a hotfix may only re-order what has already passed review and CI, never introduce it.
5. Green CI for the target SHA, the live-эфир hold (§10.4 item 7) and the release gate (§10.10) run unchanged. The release gate needs no special casing: its basis is already the LIVE deployed SHA, so its range IS `deployed..target`.

The CI run for a hotfix branch is raised with `gh workflow run ci.yml --ref hotfix/<N>-<slug>` — the existing `workflow_dispatch` trigger takes the same non-PR path as `push: main`. No workflow change is needed, and no hotfix ships on `--skip-ci-check` as a matter of routine.

**Release-cut rule.** A deploy still cuts one `release-*` tag at the deployed SHA (§10.5), but the base tag can no longer be "the newest tag by tag order": a hotfix release sits off `main`, so the NEXT ordinary `origin/main` deploy would find a newest tag that is not its ancestor and cut nothing. The base is therefore the newest `release-*` tag that IS an ANCESTOR of the target (`tools/release/cut-release.mjs` → `pickBaseReleaseTag`), which gives the hotfix release the deployed release as its base and the next main release the last main-ancestor release as its base. Release notes range from that base tag (`--notes-start-tag`). The empty-range / redeploy guard is unchanged. A hotfix cut is MARKED — the Release title carries `— Hotfix` and the notes name the cherry-picked PRs — because such a release deliberately does not contain everything on `main` at cut time, and an unmarked one would misrepresent shipped scope.

**Downstream consequence.** With prod on an off-main SHA, the `## Project reality` merged-not-deployed delta cannot range from the deployed SHA to `origin/main` — that range crosses a fork point. It is anchored on `git merge-base origin/main <deployedSha>` instead, and the rendered line says so («deployed SHA is a hotfix off main; delta anchored on merge-base <sha>»), so the count is never read as an exact missing-PR list without the caveat (`tools/project-reality.ts`, step 4).

**What stays forbidden.** Deploying an arbitrary branch; deploying a feature preview; deploying any commit not already merged to `main`; and using `--ref` to route around a `release-blocker` (the gate runs in this mode too). The hotfix path exists because the release unit is coarse, not because review is optional.
