...
4. Every commit in `deployed..target` has an equivalent commit on `origin/main` (`git cherry origin/main <target> <deployed>`; any `+` line refuses, naming the commit). This is the invariant that keeps "production runs reviewed, merged code" true — a hotfix may only re-order what has already passed review and CI, never introduce it.
