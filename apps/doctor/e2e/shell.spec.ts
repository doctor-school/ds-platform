...
/** ONE combined control on both hosts (canvas `ds-shell.dc.html` line 220, 017 US-7). */
...
test.describe("017 EARS-1: the doctor storefront mounts the shared chrome", () => {
  ...
  test("017 EARS-1.11: on desktop the search caps at the canvas width and the BBM band is the canvas micro-size", async ({
    page,
  }) => {
    // Canvas `ds-shell.dc.html` line 24: `flex:1;max-width:440px` — the search
    // grows into free bar width but never swallows the nav and the auth cluster
    // (the #2198 Stage-B finding: an uncapped `flex-1` spanning the bar). Line
    // 16: the band is `9px / 700 / .22em` — 13px caption ink was the second
    // finding. Both are build-time utilities from new tokens, so only the
    // rendered stylesheet proves they exist (cf. EARS-1.10).
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/");
    const search = page.getByTestId("shell-search");
    await expect(search).toHaveCSS("max-width", "440px");
  ...
  test("017 EARS-1.10: the footer, hero and catalog content columns cap at the design-system container width on desktop", async ({
    page,
  }) => {
    // `--container-content` is 69rem = 1104px (canvas `main{max-width}`, #1080).
    // The cap is a Tailwind UTILITY resolved at build time, so only a rendered
    // page can prove it reached the stylesheet: a misspelled class name (the
    // `--container-*` namespace yields `max-w-content`, not
    // `max-w-container-content`) leaves the column at `max-width: none` with
    // every unit test green — the giant wordmark then spans the viewport.
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/");
    for (const column of [
      page.getByTestId("storefront-footer").locator("> div").first(),
      page.getByTestId("storefront-hero").locator("> div").first(),
      page.getByTestId("specialty-catalog").locator("> div").first(),
    ]) {
      await expect(column).toHaveCSS("max-width", "1104px");
    }
  });
});
...
test.describe("017 EARS-2: every configured chrome target is a real route", () => {
  ...
  test("017 EARS-2.3: the ONE guest control navigates to the shipped sign-in surface", async ({
    page,
  }) => {
    await page.goto("/");
    const cluster = page.getByTestId("shell-auth-cluster");
    // Exactly one link in the guest cluster — the combined label, never a
    // «Войти» + «Регистрация» pair (canvas line 220, 017 US-7).
    await expect(cluster.getByRole("link")).toHaveCount(1);
    await cluster
      .getByRole("link", { name: "Войти / Регистрация", exact: true })
      .click();
    // `/login` is the shipped sign-in surface, and it carries the way to
    // `/register`; the combined canvas surface (`auth.dc.html`) is #2080.
