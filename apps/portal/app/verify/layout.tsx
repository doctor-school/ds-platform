...
/**
 * #675 — the signed-in guard for `/verify`, taken SERVER-side in the layout
 * before the surface renders: `resolveServerAuth` reads the session off the
 * incoming headers and `guardAuthRoute` (`@ds/auth-flow/server`) redirects a
 * signed-in visitor to this host’s `routes.account`. `/register` and `/reset`
 * run the same pair in their own layouts; `/login` has no layout of its own
 * because the package `LoginRoute` runs the guard inside the route itself.
 */
