...
// The shared portal theme toggle (006 EARS-12 / 008 EARS-3): the canvas 44×44
// icon-button (ADR-0013 canvas-wins — never the DS form switch). A `<button>`
// whose `aria-pressed` reflects the dark state, glyph ☾︎ in light / ☀︎ in dark,
// the accessible name injected from the message catalog. Activating it toggles
// `.dark` on `<html>` AND persists the explicit choice in `localStorage`
// (`ds-theme`).
...
describe("portal theme toggle — canvas icon-button flips `.dark` and persists the choice", () => {
