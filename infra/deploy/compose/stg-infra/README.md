...
2. `ssh deploy@$(terraform output -raw stage_1_public_ip)` works; cloud-init has
   ...
   completed. `cloud-init` now finishes within seconds — the real provisioning (system
   upgrade, docker, Node 22, Playwright host libraries, the Actions runner) runs in
   `ds-stage-bootstrap.service` and takes **several minutes** on a cold box. Check all
   three:

   ```bash
   systemctl is-enabled ds-stage-bootstrap   # enabled
   systemctl is-active ds-stage-bootstrap    # inactive  (activating = still working)
   test -f /var/lib/ds-platform/stage-bootstrap.done && echo bootstrap-complete
   ```

   `is-enabled` matters on its own: `is-active` prints `inactive` both for a finished
   run and for a unit that was never enabled. `failed` ⇒ read
   `journalctl -u ds-stage-bootstrap`. At least one provider-side reboot in the first
   minutes is expected on Timeweb (the floating IP and firewall attach while the box
   is still booting) — `cloud-init` does not resume its `runcmd` after that reboot,
   which is precisely why the slow steps live in this unit: every step carries its own
   marker under `/var/lib/ds-platform`, so a boot resumes at the step that was cut off
   and the unit re-runs until the aggregate marker exists (#2121).
...
above first. Note that an unmet start condition is **silent**: `systemctl enable --now`
still exits `0`, and only `systemctl status ds-actions-runner` (or the journal) shows
`Condition check resulted in … being skipped`. Read `status`, never the exit code.
