...
| `@ds/auth-flow/register`       | The sign-up door (#2027 PR 1.6): `RegisterDoor`, `RegistrationConfirmation` (the doctor host's inline confirmation step), `resolveConfirmLanding`, `RegisterGlyph`.                                                                                                                                                                                                                                                                                                 |
...
| `copy.register`                                     | The sign-up door's words — headings, field labels, the «Уже есть аккаунт? Войти» way out (#2331), and the optional `confirm` templates the doctor host's inline confirmation step renders.                                                                                                           |
...
| `register.form?`                                    | This host's shipped submit-group ORDER, vertical rhythm and pending affordance — stated, not derived, because neither may move without a Stage-B re-confirmation. Absent = the Academy's shipped render.                                                                                             |
| `consents`                                          | The whole consent block of the sign-up door as DATA: `tiers`, `medicalWorkerDeclaration`, `partnerDataItem`, `marketingOptIn`, `wordingVersion`, `managerNote`, `accessGroupHeading`, and the read-only `statement` a host renders instead of controls. Absent = this host asks for no consent here. |
