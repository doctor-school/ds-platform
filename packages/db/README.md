...
| `experts`                   | soft-removable                                 | `taxonomy_status` + `deleted_at`            | Structured family/given/patronymic names, nullable unique restrictive `user_id` link, and `content_removed_at`; display name is derived, never stored.                                                                                                                                                                                                                    |
...
| `doctor_specialties`        | soft-removable                                 | `record_status` + `deleted_at`              | 017 LD-1 link row: a re-choice retires the standing row and inserts the new one, preserving prior targeting evidence instead of overwriting it.                                                                                                                                                                                                                           |
...
| `speaker_migration_cutover` | **immutable/append-only** (retained singleton) | none — by contract                          | 012 EARS-24 SSOT of the legacy-speaker cutover. Exactly one row, seeded by migration 0032 and never deleted: the `event_speakers` fence fails CLOSED when it is missing, and `deploy:prod --rollback` reads its `minimum_compatible_release_*` floor. `phase` and both release ordinals are monotonic, so its trigger refuses DELETE, a second row and any backward move. |
