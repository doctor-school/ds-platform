 ...
 *
 * `specialties_minzdrav.id` is generated by the book seed
 * (`seedSpecialtiesMinzdrav`), not pinned here: the book is derived from the
 * nomenclature order and its ids differ per database. Pinning one would make the
 * golden seed depend on a value it does not own. The link therefore carries the
 * stable `name`, and the planner resolves it to an id at seed time.
 */
