...
| `LegalDocument` (`./legal-document`) | Canonical legal/document reading surface (028) — hero poster with back link, title and «редакция от <дата>», one-source table of contents (sticky aside at `xl`, collapsed `<details>` list below), Markdown body, and the «Другие документы» catalogue with a per-row «обновлено» chip; all four `state` values (`normal` / `loading` / `error` / `not-found`) render inside the SAME shell, host apps own routing, copy overrides and the `updated` fact |
| `ContactChip` (`./contact-chip`)     | Contact/social link chip — channel icon plus label; `mailto:` / `tel:` stay in place, `https:` opens with `target="_blank" rel="noopener noreferrer"`                                                                                                                                                                                                                                                                                                      |
