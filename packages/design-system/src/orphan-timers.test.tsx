...
/**
 * #441 — orphan-timer guard (the vitest.setup.ts tracking installed for every
 * `@ds/design-system` suite). `input-otp@1.4.2` schedules a 0/10/50ms `setTimeout`
 * triple on every value/focus change (its `syncTimeouts` helper) and returns NO
 * cleanup from the scheduling effect, so a timer scheduled by the last keystrokes
 * of a suite outlives the file's JSDOM environment and throws
 * `ReferenceError: window is not defined` inside React's state dispatch — the
 * intermittent `unit`-job red this guard makes impossible (same class as
 * #366/#405/#408, a different root timer than those interval guards cover).
 *
 ...
 * defect (`input-otp` frames) versus a foreign leak our own code must fix. The
 * guard is adopted from the portal's #434 sibling (apps/portal/orphan-timers.*).
 */
...
describe("#441 orphan-timer tracking (vitest.setup.ts)", () => {
  it("#441: a pending timeout is reported as an orphan and never fires after the flush", async () => {
    const cb = vi.fn();
    setTimeout(cb, 30);
  ...
  it("#441: a fired timeout is untracked — nothing to flush", async () => {
    const cb = vi.fn();
    setTimeout(cb, 0);
    await wait(10);
  ...
  it("#441: clearTimeout untracks the handle", () => {
    const cb = vi.fn();
  ...
  it("#441: tracking survives a fake-timer cycle (vi.useFakeTimers/useRealTimers)", () => {
    vi.useFakeTimers();
    try {
  ...
  it("#441: input-otp's uncleaned syncTimeouts survive unmount and are classified as the known upstream defect", async () => {
