 ...
 *
 * Width: the canvas desktop search is `flex:1;max-width:440px` (line 24) — it
 * grows into the free bar width and stops at the `search` container token, so
 * the nav and the auth cluster keep their place at the right. Below `layout` it
 * re-flows onto its own full-width row (canvas line 62).
 */
export function ShellSearch({
  ...
  return (
    <form
      ...
      className="order-last w-full min-w-0 layout:order-none layout:w-auto layout:max-w-search layout:flex-1 layout:basis-40"
    >
      <Input
        type="search"
        name="q"
