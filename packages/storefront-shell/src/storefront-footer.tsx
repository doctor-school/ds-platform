...
function FooterChrome({ config }: { config: StorefrontShellConfig }) {
  ...
  return (
    ...
    >
      {/*
        `max-w-content` is the `--container-content` token (69rem = 1104px) —
        Tailwind's `--container-*` namespace drops its prefix in the utility
        name, so `max-w-container-content` never existed and the column ran
        edge to edge (the #2180 Stage-B finding).
      */}
      <div className="mx-auto grid w-full max-w-content gap-8 layout:grid-cols-4 layout:gap-10">
        <div>
          {/*
