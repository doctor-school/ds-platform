 ...
 *      head — unless the main-side delta since the tested base is disjoint
 *      from the PR files AND outside `ALWAYS_OVERLAPPING_PATHS`, which the
 *      disjoint-changes rule accepts with a printed evidence block (#2124).
 *      Anything else: another session landing while CI runs requires
 *      rebase/recheck.
...
/**
 * Paths whose change on main invalidates a tested PR head even when the PR does
 * not touch the same file (#2124). The disjoint-changes rule below accepts an
 * advanced main only when its delta misses BOTH the PR file set and this list.
 * `kind`: `exact` (repo-relative path) · `dir` (prefix, trailing `/`) ·
 * `basename` (file name in any directory) · `root-regex` (repo-root file only) ·
 * `basename-regex` (file name in any directory).
 */
export const ALWAYS_OVERLAPPING_PATHS = [
  {
    kind: "exact",
    value: "pnpm-lock.yaml",
    reason:
      "a lockfile generated against the old tree goes red on main after the merge (#218)",
  },
  {
    kind: "basename",
    value: "package.json",
    reason:
      "the root and every workspace manifest feed the lockfile and the turbo graph",
  },
  {
    kind: "exact",
    value: "pnpm-workspace.yaml",
    reason: "workspace topology the tested install resolved against",
  },
  {
    kind: "exact",
    value: "turbo.json",
    reason: "task-pipeline topology the tested run executed",
  },
  {
    kind: "dir",
    value: ".github/workflows/",
    reason: "the CI definition the tested run used",
  },
  {
    kind: "dir",
    value: "tools/gh/",
    reason: "the merge-gate and landing tooling deciding this very merge",
  },
  {
    kind: "exact",
    value: "tools/lint/guard-policy.mjs",
    reason: "the guard severity map CI reads",
  },
  {
    kind: "dir",
  ...
  {
    kind: "dir",
    value: "packages/schemas/",
    reason: "the API contract SSOT every app depends on",
  },
  {
    kind: "exact",
    value: "eslint.config.js",
    reason: "the lint rules the tested run used",
  },
  {
    kind: "root-regex",
    value: "^eslint\\..+\\.config\\.mjs$",
    reason: "a root lint-config projection the tested run used",
  },
  {
    kind: "basename-regex",
    value: "^tsconfig.*\\.json$",
    reason: "the compiler settings the tested run used",
  },
];

/**
 * The always-overlapping entry matching a `git diff --name-only` path, or null.
 * Paths are POSIX repo-relative as git prints them.
 */
export function matchAlwaysOverlapping(path) {
  ...
  for (const entry of ALWAYS_OVERLAPPING_PATHS) {
    if (entry.kind === "exact" && normalized === entry.value) return entry;
    if (entry.kind === "dir" && normalized.startsWith(entry.value))
      return entry;
    if (entry.kind === "basename" && basename === entry.value) return entry;
    if (
      entry.kind === "root-regex" &&
      atRoot &&
      new RegExp(entry.value).test(normalized)
    )
      return entry;
    if (
      entry.kind === "basename-regex" &&
      new RegExp(entry.value).test(basename)
    )
      return entry;
  }
  return null;
...
/**
 * Disjoint-changes rule (#2124): classify a main that advanced past the tested
 * head. `disjoint` is the only accepting state; everything else stays RED.
 * Fails closed — an unusable file list (a git failure the caller could not read,
 * or an advance with nothing to compare) is `unknown`, never `disjoint`.
 */
export function classifyMainAdvance({ mainFiles, prFiles } = {}) {
  const clean = (list) =>
    Array.isArray(list)
      ? list.filter((path) => typeof path === "string" && path.trim() !== "")
      : null;
  const mainList = clean(mainFiles);
  const prList = clean(prFiles);
  const unusable = !mainList || mainList.length === 0 || !prList;
  if (unusable) return { state: "unknown", overlapping: [], protectedHits: [] };
  const prSet = new Set(prList);
  const overlapping = [...new Set(mainList.filter((path) => prSet.has(path)))];
  const protectedHits = [];
  for (const path of mainList) {
    const entry = matchAlwaysOverlapping(path);
    if (entry) protectedHits.push({ path, reason: entry.reason });
  }
  if (protectedHits.length > 0)
    return { state: "protected", overlapping, protectedHits };
  if (overlapping.length > 0)
    return { state: "overlap", overlapping, protectedHits };
  return { state: "disjoint", overlapping, protectedHits };
}

/** Render a capped, indented evidence file list for the gate's stdout. */
function formatEvidencePaths(paths, limit = 40) {
  const lines = paths.slice(0, limit).map((path) => `${TAG}     ${path}`);
  if (paths.length > limit)
    lines.push(`${TAG}     … and ${paths.length - limit} more`);
...
function assertCurrentMainAncestry(headSha) {
  ...
  const advanced = !ancestry.error && ancestry.status === 1;
  if (ancestry.error || (ancestry.status !== 0 && !advanced))
    refuse(`git ancestry check failed for main ${mainSha.slice(0, 12)}`);
  if (advanced) {
    // #2124 — main advanced past the tested head. That is accepted only when
    // main's own delta since the tested base touches neither a PR file nor an
    // always-overlapping path; anything else keeps the historical refusal, now
    // naming what collides so the lander knows a rebase is unavoidable.
    const advanceRefusal = `main advanced to ${mainSha.slice(0, 12)} outside the PR ancestry`;
    ...
    const mainFiles = filesOf(mainSha);
    const prFiles = filesOf(headSha);
    const verdict = classifyMainAdvance({ mainFiles, prFiles });
    if (verdict.state !== "disjoint") {
      for (const path of verdict.overlapping)
        process.stderr.write(`${TAG} overlapping path: ${path}\n`);
      for (const hit of verdict.protectedHits)
        process.stderr.write(
          `${TAG} always-overlapping path: ${hit.path} — ${hit.reason}\n`,
        );
    ...
    process.stdout.write(
      `${TAG} #2124 disjoint-changes rule: accepting the advanced main.\n` +
        `${TAG}   tested base ${base.slice(0, 12)}, main advanced to ${mainSha.slice(0, 12)}.\n` +
        `${TAG}   main-side files (${mainFiles.length}):\n` +
        formatEvidencePaths(mainFiles) +
        `${TAG}   PR-side files (${prFiles.length}):\n` +
        formatEvidencePaths(prFiles) +
        `${TAG}   Safety net: ci.yml runs on push to main — a semantic conflict between disjoint changes surfaces on main within one CI run.\n`,
    );
    return;
  }
  process.stdout.write(
