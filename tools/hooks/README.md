...
Local hook coverage is host-specific. Hosted tools do not use the documented local hook path, some specialized tools can opt out, and `write_stdin` does not receive a new PreToolUse check. `functions.exec` and its nested tools must be observed in the active host before asserting enforcement or bypass. The broad child-hook registration records names even when no child budget can be measured.
