...
const base = {
  toolName: "Read",
  toolInput: { file_path: `${ROOT}\\tools\\agent-bootstrap.ts` },
  cwd: ROOT,
  sessionId: "self",
  projectDir: ROOT,
  flag: flagWith([{ id: "peer", logPath: "C:\\logs\\peer.jsonl" }]),
...
describe("main-tree-read-guard decideWarn()", () => {
  ...
  it("ignores targets outside the repo (scratchpad, auto-memory)", () => {
    const toolInput = {
      file_path: "C:\\Users\\sidor\\.claude\\projects\\x\\memory\\MEMORY.md",
    };
  ...
  it("counts only live foreign sessions in liveCount", () => {
    const flag = flagWith([
      { id: "peer-a", logPath: "C:\\logs\\a.jsonl" },
      { id: "peer-b", logPath: "C:\\logs\\b.jsonl" },
      { id: "self", logPath: "C:\\logs\\self.jsonl" },
    ]);
...
describe("main-tree-read-guard path seams", () => {
  it("targetPath resolves a relative path against the cwd", () => {
    expect(targetPath({ path: "tools" }, ROOT).replace(/\\/g, "/")).toBe(
      "C:/Users/sidor/repos/ds-platform/tools",
    );
  });
  ...
  it("inWorktree matches the worktree root itself and its children", () => {
    expect(inWorktree(`${ROOT}\\.claude\\worktrees\\823`)).toBe(true);
    expect(inWorktree(`${ROOT}\\.claude\\worktrees\\823\\tools`)).toBe(true);
...
describe("bootstrap flag + directive (#823)", () => {
  ...
  it("buildParallelFlag carries the session list + timestamp", () => {
      ...
      [
        {
          id: "peer",
          mtimeMs: NOW,
          inSharedMainTree: true,
          logPath: "C:\\logs\\peer.jsonl",
        },
      ],
    ...
    expect(flag.sessions).toEqual([
      { id: "peer", logPath: "C:\\logs\\peer.jsonl", inSharedMainTree: true },
    ]);
  });
