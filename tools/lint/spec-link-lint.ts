 ...
 *   2. Each linked Issue carries a milestone (the track release — any title;
...
async function main(): Promise<void> {
  ...
  // 1. + 2. — `Closes #N`, and each linked Issue carries a (track-release) milestone.
  ...
  for (const num of linked) {
    ...
    info(
      `Issue #${num} → milestone \`${issue.milestone.title}\` (track release) OK`,
    );
  }
